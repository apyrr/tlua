...
description: >
  ...
  tlua repository. Covers test file formats, directives, markers,
  baseline management, and the fourslash verification API.
---

# Writing, Running, and Debugging Compiler Tests and Fourslash Tests

This guide covers the complete testing workflow for the tlua repository, including compiler tests (type-checking, emit, diagnostics) and fourslash tests (language server features like completions, hover, go-to-definition).
...
```go
import . "github.com/apyrr/tlua/internal/fourslash/tests/util"
