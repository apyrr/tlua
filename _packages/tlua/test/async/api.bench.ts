...
export async function runBenchmarks(options?: { filter?: string; singleIteration?: boolean; cpuprofile?: boolean; }) {
    ...
    // These benchmarks load the upstream src/compiler project, whose .ts
    // sources tlua no longer consumes (tlua sources are .tlua), so the
    // project resolves to zero files. Skip until a tlua-sized corpus exists.
    const skipUpstreamCorpus = true;
    if (skipUpstreamCorpus) {
        console.warn("Warning: benchmarks target the upstream .ts corpus, which tlua no longer compiles; skipping benchmarks.");
        return;
    }
