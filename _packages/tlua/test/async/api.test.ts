...
} from "@tlua/compiler/unstable/async"; // @sync: } from "@tlua/compiler/unstable/sync";
...
describe("Multiple snapshots", () => {
    test("two snapshots work independently", async () => {
        ...
        try {
            ...
            const sf1 = await snap1.getProject("/tsconfig.json")!.program.getSourceFile("/src/index.tlua");
            const sf2 = await snap2.getProject("/tsconfig.json")!.program.getSourceFile("/src/index.tlua");
            assert.ok(sf1);
            assert.ok(sf2);
            ...
            const symbol = await snap2.getProject("/tsconfig.json")!.checker.getSymbolAtPosition("/src/index.tlua", 7);
            assert.ok(symbol);
            assert.equal(symbol.name, "foo");
        }
        finally {
    ...
    test("each snapshot has its own server-side lifecycle", async () => {
        ...
        try {
            ...
            const sf1 = await snap1.getProject("/tsconfig.json")!.program.getSourceFile("/src/foo.tlua");
            assert.ok(sf1);
            ...
            const sf2 = await snap2.getProject("/tsconfig.json")!.program.getSourceFile("/src/foo.tlua");
            assert.ok(sf2);
            ...
            const sf3 = await snap3.getProject("/tsconfig.json")!.program.getSourceFile("/src/foo.tlua");
            assert.ok(sf3);
    ...
    test("adding a new file is reflected in the next snapshot", async () => {
        ...
        try {
            ...
            const sf = await snap2.getProject("/tsconfig.json")!.program.getSourceFile("/src/bar.tlua");
            assert.ok(sf);
            assert.equal(sf.text, `local bar = true;\nreturn { bar = bar };`);

            // Original snapshot shouldn't have the new file
            const sfOld = await snap1.getProject("/tsconfig.json")!.program.getSourceFile("/src/bar.tlua");
    ...
    test("multiple sequential edits produce correct snapshots", async () => {
        ...
        try {
            ...
            for (const version of versions) {
                ...
                const sf = await snap.getProject("/tsconfig.json")!.program.getSourceFile("/src/foo.tlua");
                assert.ok(sf);
