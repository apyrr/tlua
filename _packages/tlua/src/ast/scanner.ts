...
export const textToKeywordObj: Record<string, KeywordSyntaxKind> = {
    ...
    undefined: SyntaxKind.NilKeyword,
        ...
        switch (ch) {
            ...
            case CharacterCodes.minus:
                if (text.charCodeAt(pos + 1) === CharacterCodes.minus) {
                    ...
                    pos += 2;
                    const opener = scanLuaLongBracketStart(text, pos);
                    if (opener) {
                        pos = scanLuaLongBracketEnd(text, opener.contentStart, opener.level).end;
                    }
                    else {
...
function scanLuaLongBracketStart(text: string, pos: number): { level: number; contentStart: number; } | undefined {
    if (text.charCodeAt(pos) !== CharacterCodes.openBracket) {
        return undefined;
    }
    let level = 0;
    pos++;
    while (text.charCodeAt(pos) === CharacterCodes.equals) {
        level++;
        pos++;
    }
    return text.charCodeAt(pos) === CharacterCodes.openBracket ? { level, contentStart: pos + 1 } : undefined;
}

function luaLongBracketClose(level: number): string {
    return "]" + "=".repeat(level) + "]";
}

function scanLuaLongBracketEnd(text: string, pos: number, level: number): { end: number; closed: boolean; } {
    const close = luaLongBracketClose(level);
    const closePos = text.indexOf(close, pos);
    return closePos < 0 ? { end: text.length, closed: false } : { end: closePos + close.length, closed: true };
}
        ...
        switch (ch) {
            ...
            case CharacterCodes.minus:
                ...
                }
                {
                    const startPos = pos;
                    pos += 2;
                    let kind: CommentKind = SyntaxKind.SingleLineCommentTrivia;
                    let hasTrailingNewLine = false;
                    const opener = scanLuaLongBracketStart(text, pos);
                    if (opener) {
...
export function createScanner(
    ...
    function checkForIdentifierStartAfterNumericLiteral() {
        ...
        // An identifier (including a lone `n`, the former bigint suffix) is never
        // valid immediately after a numeric literal; leave it to be scanned separately.
            ...
            switch (ch) {
                ...
                case CharacterCodes.exclamation:
                    if (charCodeUnchecked(pos + 1) === CharacterCodes.equals) {
                        // Lua not-equal is `~=`; `!=`/`!==` scan to the same
                        // not-equal token as lenient aliases for it.
                        if (charCodeUnchecked(pos + 2) === CharacterCodes.equals) {
                            return pos += 3, token = SyntaxKind.TildeEqualsToken;
                        }
                        return pos += 2, token = SyntaxKind.TildeEqualsToken;
                    }
                    pos++;
                ...
                case CharacterCodes.minus:
                    if (charCodeUnchecked(pos + 1) === CharacterCodes.minus) {
                        ...
                        const opener = scanLuaLongBracketStart(text, pos);
                        if (opener) {
                            pos = opener.contentStart;
                            const close = luaLongBracketClose(opener.level);
                        ...
                        // A malformed long-bracket opener remains line-comment text.
                ...
                case CharacterCodes.equals:
                    ...
                    if (charCodeUnchecked(pos + 1) === CharacterCodes.equals) {
                        // Lua equality is `==` (strict, no coercion); `===` scans
                        // to the same equality token as a lenient alias.
                        if (charCodeUnchecked(pos + 2) === CharacterCodes.equals) {
                            return pos += 3, token = SyntaxKind.EqualsEqualsToken;
                        }
