...
func (tx *TypeEraserTransformer) visit(node *ast.Node) *ast.Node {
	...
	case ast.KindModuleDeclaration:
		// Module declarations are always elided: ambient modules and `declare
		// global` have no runtime form, and identifier namespaces are deleted
		// from the grammar.
		return tx.elide(node)
	...
	case ast.KindFunctionDeclaration:
		...
		return tx.Factory().UpdateFunctionDeclaration(n, tx.Visitor().VisitModifiers(n.Modifiers()), tx.Visitor().VisitNode(n.Target), n.ColonToken, tx.Visitor().VisitNode(n.Name()), nil, tx.Visitor().VisitNodes(n.Parameters), nil, nil, tx.Visitor().VisitNode(n.Body))

	case ast.KindFunctionExpression:
		n := node.AsFunctionExpression()
		return tx.Factory().UpdateFunctionExpression(n, tx.Visitor().VisitModifiers(n.Modifiers()), tx.Visitor().VisitNode(n.Name()), nil, tx.Visitor().VisitNodes(n.Parameters), nil, nil, tx.Visitor().VisitNode(n.Body))
