...
func BenchmarkPopulateSymlinksFromResolutions(b *testing.B) {
	...
	for i := range 50 {
		deps[i].orig = "/project/node_modules/pkg" + string(rune('A'+i)) + "/index.lua"
		deps[i].resolved = "/real/pkg" + string(rune('A'+i)) + "/index.lua"
	}
...
func BenchmarkSetFile(b *testing.B) {
	cache := NewKnownSymlink("/project", true)
	symlink := "/project/file.tlua"
	path := tspath.ToPath(symlink, "/project", true)

	for b.Loop() {
		cache.SetFile(symlink, path, "/real/file.tlua")
	}
}
...
func BenchmarkGuessDirectorySymlink(b *testing.B) {
	...
	for b.Loop() {
		cache.guessDirectorySymlink(
			"/real/node_modules/package/dist/index.lua",
			"/project/symlink/package/dist/index.lua",
			"/project",
		)
	}
}
...
func BenchmarkConcurrentAccess(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		i := 0
		for pb.Next() {
			symlink := "/project/file" + string(rune('A'+(i%26))) + ".tlua"
			path := tspath.ToPath(symlink, "/project", true)
			cache.SetFile(symlink, path, "/real/file.tlua")
