...
func TestSetFile(t *testing.T) {
	t.Parallel()
	cache := NewKnownSymlink("/test/dir", true)
	symlink := "/test/symlink/file.tlua"
	symlinkPath := tspath.ToPath(symlink, "/test/dir", true)
	realpath := "/real/path/file.tlua"
...
func TestProcessResolution(t *testing.T) {
	...
	originalPath := "/test/original/file.tlua"
	resolvedPath := "/test/resolved/file.tlua"
...
func TestGuessDirectorySymlink(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			a:        "/test/path/file.tlua",
			b:        "/test/path/file.tlua",
			cwd:      "/test/dir",
			expected: [2]string{"/", "/"},
		},
		{
			name:     "different files same directory",
			a:        "/test/path/file1.tlua",
			b:        "/test/path/file2.tlua",
			cwd:      "/test/dir",
			expected: [2]string{"", ""},
		},
		{
			name:     "different directories",
			a:        "/test/path1/file.tlua",
			b:        "/test/path2/file.tlua",
			cwd:      "/test/dir",
			expected: [2]string{"/test/path1", "/test/path2"},
		},
		{
			name:     "node_modules paths",
			a:        "/test/node_modules/pkg/file.tlua",
			b:        "/test/node_modules/pkg/file.tlua",
			cwd:      "/test/dir",
			expected: [2]string{"/test/node_modules/pkg", "/test/node_modules/pkg"},
		},
		{
			name:     "scoped package paths",
			a:        "/test/node_modules/@scope/pkg/file.tlua",
			b:        "/test/node_modules/@scope/pkg/file.tlua",
			cwd:      "/test/dir",
...
func TestSetSymlinksFromResolutions(t *testing.T) {
	...
	resolvedModules := []struct {
		...
		{
			originalPath: "/test/original/file1.tlua",
			resolvedPath: "/test/resolved/file1.tlua",
			moduleName:   "module1",
			mode:         core.ResolutionModeNone,
			filePath:     tspath.ToPath("/test/source.tlua", "/test/dir", true),
		},
		{
			originalPath: "/test/original/file2.tlua",
			resolvedPath: "/test/resolved/file2.tlua",
			moduleName:   "module2",
			mode:         core.ResolutionModeNone,
			filePath:     tspath.ToPath("/test/source.tlua", "/test/dir", true),
		},
