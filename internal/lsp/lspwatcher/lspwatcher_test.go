...
func TestWatcher_KindFilter(t *testing.T) {
	...
	backend.emitAll([]fswatch.Event{
		{Kind: fswatch.EventUpdate, Path: filepath.FromSlash(filepath.Join(dirNorm, "x.tlua"))},
		{Kind: fswatch.EventDelete, Path: filepath.FromSlash(filepath.Join(dirNorm, "x.tlua"))},
	}, nil)
...
func TestWatcher_BookkeepingAndOverflow(t *testing.T) {
	...
	backend.emitAll([]fswatch.Event{
		{Kind: fswatch.EventUpdate, Path: filepath.FromSlash(filepath.Join(dirNorm, "a.tlua"))},
	}, fswatch.ErrOverflow)
	waitFor(t, func() bool {
		mu.Lock()
		defer mu.Unlock()
...
func TestWatcher_RealBackend_MissingThenCreate(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(base, "pkg", "index.tlua"), []byte("export {}"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	waitFor(t, func() bool {
		for _, e := range collected() {
			if strings.HasSuffix(string(e.Uri), "/pkg/index.tlua") {
				return true
			}
		}
		return false
...
func TestWatcher_MissingDirectoryPromotesOnCreate(t *testing.T) {
	...
	waitFor(t, func() bool {
		...
		for _, e := range got {
			...
			if strings.HasSuffix(s, "/pkg/index.tlua") {
				sawChild = true
			}
		}
...
func TestWatcher_SyntheticCreateDepth(t *testing.T) {
	...
	for _, recursive := range []bool{false, true} {
		t.Run(map[bool]string{false: "non-recursive", true: "recursive"}[recursive], func(t *testing.T) {
			...
			if err := os.WriteFile(filepath.Join(base, "pkg", "top.tlua"), []byte("export {}"), 0o644); err != nil {
				t.Fatal(err)
			}
			if err := os.WriteFile(filepath.Join(base, "pkg", "sub", "deep.tlua"), []byte("export {}"), 0o644); err != nil {
				t.Fatal(err)
			}
			...
			waitFor(t, func() bool {
				for s := range created() {
					if strings.HasSuffix(s, "/pkg/top.tlua") {
						return true
					}
				}
				return false
			...
			sawDeep := func() bool {
				for s := range created() {
					if strings.HasSuffix(s, "/pkg/sub/deep.tlua") {
						return true
					}
				}
				return false
...
func TestWatcher_WatchTerminatedDoesNotDropEvents(t *testing.T) {
	...
	backend.emitAll([]fswatch.Event{
		{Kind: fswatch.EventUpdate, Path: filepath.FromSlash(filepath.Join(dirNorm, "b.tlua"))},
