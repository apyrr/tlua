...
func TestServerShutdownNoDeadlock(t *testing.T) {
	...
	server.session.DidOpenFile(ctx, "file:///test/index.tlua", 1, "local x = 1;", lsproto.LanguageKindTypeScript)
	...
	server.session.DidChangeFile(ctx, "file:///test/index.tlua", 2, []lsproto.TextDocumentContentChangePartialOrWholeDocument{
		{
	...
	_, _ = server.session.GetLanguageService(ctx, "file:///test/index.tlua")
	server.session.WaitForBackgroundTasks()
