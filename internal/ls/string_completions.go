...
func (l *LanguageService) getStringLiteralCompletionEntries(
	...
	case ast.KindComputedPropertyName:
		// The bracket key of a table field: `{ ["/*cursor*/"] = v }`. Offer
		// the property names of the literal's contextual type, like the old
		// bare quoted-name spelling did.
		grandparent := walkUpParentheses(parent.Parent)
		if ast.IsPropertyAssignment(grandparent) &&
			grandparent.Name() == parent && ast.IsObjectLiteralExpression(grandparent.Parent) {
			return &stringLiteralCompletions{
				fromProperties: stringLiteralCompletionsForObjectLiteral(typeChecker, grandparent.Parent),
			}
		}
		return nil
...
func (l *LanguageService) getStringLiteralCompletionsFromModuleNamesWorker(
	...
	if isPathRelativeToScript(literalValue) ||
		tspath.IsRootedDiskPath(literalValue) || tspath.IsUrl(literalValue) {
...
func (l *LanguageService) getCompletionEntriesForNonRelativeModules(
	...
	// String-named ambient modules are deleted, so declarations contribute no
...
func getSupportedExtensionsForModuleResolution(options *core.CompilerOptions, checker *checker.Checker) []string {
	// String-named ambient modules are deleted, so no `*.css`-style wildcard
	// extensions can be contributed by declarations.
...
func getStringLiteralTypes(t *checker.Type, uniques *collections.Set[string], typeChecker *checker.Checker) []*checker.StringLiteralType {
	...
	if t.IsStringLiteral() && uniques.AddIfAbsent(t.AsLiteralType().Value().(string)) {
