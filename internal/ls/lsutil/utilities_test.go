...
// A word-spelled operator is reserved but scans to a punctuation kind rather
// than a keyword kind, so the keyword guard alone lets it through: a module
// named `and.tlua` would give the local name `and` and an unparseable
// `import and from "./and"`.
func TestModuleSpecifierToValidIdentifierEscapesWordOperators(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{"or", "_or"},
		{"not", "_not"},
		{"while", "_while"}, // keyword, the case the guard already covered
		{"andThen", "andThen"},
		{"notify", "notify"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.specifier, func(t *testing.T) {
			t.Parallel()
			if got := ModuleSpecifierToValidIdentifier(tt.specifier, false /*forceCapitalize*/); got != tt.want {
				t.Fatalf("ModuleSpecifierToValidIdentifier(%q) = %q, want %q", tt.specifier, got, tt.want)
			}
		})
	}
}
