...
func ModuleSpecifierToValidIdentifier(moduleSpecifier string, forceCapitalize bool) string {
	...
	// A word operator is reserved but not a keyword kind, so IsNonContextualKeyword
	// misses it: a module named `and.ts` must not yield `import and from "./and"`.
	if resString != "" && !IsNonContextualKeyword(scanner.StringToToken(resString)) && !scanner.IsWordOperatorText(resString) {
		return resString
	}
	return "_" + resString
}
