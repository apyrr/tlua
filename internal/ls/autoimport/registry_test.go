...
import (
	...
	"github.com/apyrr/tlua/internal/testutil/autoimporttestutil"
...
func TestRegistryLifecycle(t *testing.T) {
	...
	t.Run("bucket updates on same-file change when new files added to the program", func(t *testing.T) {
		...
		files := map[string]any{
			...
			projectRoot + "/index.tlua": "",
			projectRoot + "/utils.tlua": `local foo = 1;
local bar = 2;
return { foo = foo, bar = bar };`,
		...
		indexURI := lsproto.DocumentUri("file://" + projectRoot + "/index.tlua")

		// Open the index.tlua file
		...
		// Edit index.tlua to require utils.tlua
		newContent := `local utils = require("utils");`
	...
	t.Run("node_modules bucket dependency selection changes with open files", func(t *testing.T) {
		...
		monorepoIndex := tspath.CombinePaths(monorepoRoot, "index.lua")
		packageAIndex := tspath.CombinePaths(packageADir, "index.lua")
		...
		fixture := autoimporttestutil.SetupMonorepoLifecycleSession(t, autoimporttestutil.MonorepoSetupConfig{
			...
			ExtraFiles: []autoimporttestutil.TextFileSpec{
				{Path: monorepoIndex, Content: "export local monorepoIndex = 1;\n"},
				{Path: packageAIndex, Content: "export local pkgA = 2;\n"},
			},
		})
		session := fixture.Session()
	...
	t.Run("node_modules bucket includes resolved packages from all projects", func(t *testing.T) {
		...
		packageAIndex := tspath.CombinePaths(packageADir, "index.tlua")
		packageBIndex := tspath.CombinePaths(packageBDir, "index.tlua")
		...
		fixture := autoimporttestutil.SetupMonorepoLifecycleSession(t, autoimporttestutil.MonorepoSetupConfig{
			...
			ExtraFiles: []autoimporttestutil.TextFileSpec{
				// project-a directly requires pkg-unlisted (not in package.json)
				{Path: packageAIndex, Content: "local pkg_unlisted = require(\"pkg-unlisted\");\nreturn { a = pkg_unlisted };\n"},
				// project-b does not require pkg-unlisted
				{Path: packageBIndex, Content: "local b = 1;\nreturn { b = b };\n"},
			},
		})
		session := fixture.Session()
		packageAHandle := fixture.ExtraFile(packageAIndex)
	...
	t.Run("symlinked monorepo invalidates on source file change", func(t *testing.T) {
		...
		projectAIndex := tspath.CombinePaths(projectADir, "src", "index.tlua")
		projectBSrcIndex := tspath.CombinePaths(projectBDir, "src", "index.tlua")
		projectBDistIndex := tspath.CombinePaths(projectBDir, "dist", "index.d.tlua")
...
`,
		...
		newProjectBContent := `export local projectBValue: number = 42;`
	...
	t.Run("pnpm-style symlinks only grant granular updates to workspace packages", func(t *testing.T) {
		...
		otherPkgIndex := tspath.CombinePaths(pnpmStoreDir, "index.d.tlua")
...
`,
		...
		session.DidOpenFile(ctx, projectBURI, 1, projectBContent, lsproto.LanguageKindTypeScript)
		session.DidChangeFile(ctx, projectBURI, 2, []lsproto.TextDocumentContentChangePartialOrWholeDocument{
			{WholeDocument: &lsproto.TextDocumentContentChangeWholeDocument{Text: `export local projectBValue: number = 42;`}},
		})
	...
	t.Run("changed fileExcludePatterns triggers bucket rebuild", func(t *testing.T) {
		...
		newPreferences.AutoImportFileExcludePatterns = []string{"**/node_modules/**/*.d.tlua"}
	...
	t.Run("dedupes packages that resolve to same realpath across ancestor node_modules buckets", func(t *testing.T) {
		...
		appIndex := tspath.CombinePaths(appDir, "src", "index.tlua")
		...
		files := map[string]any{
			...
			appIndex: "export local app = 1;\n",
			tspath.CombinePaths(sharedPkgDir, "package.json"): `{
				"name": "shared",
				"version": "1.0.0",
				"types": "index.d.tlua"
			}`,
			tspath.CombinePaths(sharedPkgDir, "index.d.tlua"):     "export declare local sharedValue: 1;\n",
...
func TestHiddenDirectoriesInNodeModules(t *testing.T) {
	...
	t.Run("deep import through subdirectory package.json in hidden store", func(t *testing.T) {
		...
		files := map[string]any{
			...
			projectRoot + "/index.tlua": `import { debug } from "some-pkg/debug";`,

			// Real package that should be indexed normally
			projectRoot + "/node_modules/real-package/package.json": `{"name":"real-package","version":"1.0.0","types":"index.d.tlua"}`,
			projectRoot + "/node_modules/real-package/index.d.tlua": "export declare local realExport: number;\n",
			...
			pkgStoreDir + "/package.json": `{"name":"some-pkg","version":"1.0.0","types":"index.d.tlua"}`,
			pkgStoreDir + "/index.d.tlua": "export declare local something: number;\n",
			...
			pkgStoreDir + "/debug/package.json": `{"main":"./debug.lua","types":"./debug.d.tlua"}`,
			pkgStoreDir + "/debug/debug.d.tlua": "export declare function debug(msg: string): void;\n",
			pkgStoreDir + "/debug/debug.lua":    "exports.debug = function(msg) { console.log(msg); };\n",

			// Other content in the hidden store that should never be crawled
			storeDir + "/other-pkg-npm-2.0.0-def456/package/package.json": `{"name":"other-pkg","version":"1.0.0","types":"index.d.tlua"}`,
			storeDir + "/other-pkg-npm-2.0.0-def456/package/index.d.tlua": "export declare local other: string;\n",
		}
		...
		ctx := context.Background()
		indexURI := lsproto.DocumentUri("file://" + projectRoot + "/index.tlua")
		session.DidOpenFile(ctx, indexURI, 1, files[projectRoot+"/index.tlua"].(string), lsproto.LanguageKindTypeScript)
...
func TestAutoImportEntrypointDirectorySearch(t *testing.T) {
	...
	files := map[string]any{
		...
		projectRoot + "/index.tlua": `local pkg = require("my-pkg");`,
		// Package with NO "exports" field and multiple files.
		// Without the preference, only the main entrypoint is found.
		// With the preference, directory search finds all .d.tlua files.
		pkgDir + "/package.json":         `{"name":"my-pkg","version":"1.0.0","types":"index.d.tlua"}`,
		pkgDir + "/index.d.tlua":         "export declare local main: number;\n",
		pkgDir + "/extra.d.tlua":         "export declare local extra: string;\n",
		pkgDir + "/nested/deep.d.tlua":   "export declare local deep: boolean;\n",
		pkgDir + "/nested/deeper.d.tlua": "export declare local deeper: boolean;\n",
	}
	...
	t.Run("default limits to main entrypoint", func(t *testing.T) {
		...
		stats := autoImportStats(t, session)
		nodeModulesBucket := singleBucket(t, stats.NodeModulesBuckets)
		// Without the preference, only the main entrypoint (index.d.tlua) should be found
	...
	t.Run("autoImportEntrypointDirectorySearch enables all files", func(t *testing.T) {
		...
		stats := autoImportStats(t, session)
		nodeModulesBucket := singleBucket(t, stats.NodeModulesBuckets)
		// With the preference, all 4 .d.tlua files should be found via directory search
	...
	t.Run("changing preference triggers rebuild", func(t *testing.T) {
		...
		isPrepared := snapshot.AutoImportRegistry().IsPreparedForImportingFile(
			projectRoot+"/index.tlua", projectPath, prefs,
		)
	...
	t.Run("deep import from program update enables recursive search for that package", func(t *testing.T) {
		t.Parallel()
		// The deep-import detection keys on '/'-subpath specifiers
		// (module.ParsePackageName), which are no longer valid Lua module
		// names and never resolve. Re-expressing deep detection for dotted
		// names (a resolved file deeper than its package root) belongs to
		// the auto-import redesign; until then the trigger is unreachable.
		t.Skip("deep-import detection is unreachable under Lua dotted module names; auto-import stack debt")
		session, _ := projecttestutil.Setup(files)
		t.Cleanup(session.Close)
		...
		// Now update the program to add a deep require from the same package
		newContent := "local pkg = require(\"my-pkg\");\nlocal deep = require(\"my-pkg/nested/deep\");\n"
		session.DidChangeFile(ctx, indexURI, 2, []lsproto.TextDocumentContentChangePartialOrWholeDocument{
			{WholeDocument: &lsproto.TextDocumentContentChangeWholeDocument{Text: newContent}},
		})

		// After the program update, auto-imports should detect the deep import and
		// enable recursive directory search for my-pkg, finding all .d.tlua files.
		_, err = session.GetCurrentLanguageServiceWithAutoImports(ctx, indexURI)
		assert.NilError(t, err)
...
func TestUpdateIndexesConcurrentMapSafety(t *testing.T) {
	...
	for i := range packageCount {
		...
		files[pkgDir+"/package.json"] = fmt.Sprintf(`{"name":"pkg%d","version":"1.0.0","main":"index.lua"}`, i)
		files[pkgDir+"/index.lua"] = "module.exports = {};\n"
		typesDir := fmt.Sprintf("%s/node_modules/@types/pkg%d", projectRoot, i)
		files[typesDir+"/package.json"] = fmt.Sprintf(`{"name":"@types/pkg%d","version":"1.0.0","types":"index.tlua"}`, i)
		files[typesDir+"/index.tlua"] = fmt.Sprintf("local foo%d: number = %d;\nreturn { foo%d = foo%d };\n", i, i, i, i)
	}
