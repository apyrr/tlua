...
func getNewImports(
	...
	if defaultImport != nil || len(namedImports) > 0 {
		...
		topLevelTypeOnly := (defaultImport == nil || needsTypeOnly(defaultImport.addAsTypeOnly)) &&
			...
			preferences.PreferTypeOnlyAutoImports.IsTrue() &&
...
func getAddAsTypeOnly(isValidTypeOnlyUseSite bool, export *Export, compilerOptions *core.CompilerOptions) lsproto.AddAsTypeOnly {
	...
	if export.IsTypeOnly && export.Flags&ast.SymbolFlagsValue != 0 {
...
func detectSyntaxIndicators(file *ast.SourceFile, options *core.CompilerOptions) (hasESM bool, hasCJS bool) {
	...
	// Every file is a module in tlua, so ExternalModuleIndicator is set to
...
func promoteFromTypeOnly(
	...
	convertExistingToTypeOnly := core.TSUnknown
