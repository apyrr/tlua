...
import (
	...
	"github.com/apyrr/tlua/internal/ls/change"
...
func (f *missingMemberFixer) createSignatureDeclarationFromSignature(signature *checker.Signature, kind ast.Kind, sourceFile *ast.SourceFile, enclosingDeclaration *ast.Node, body *ast.FunctionBody, modifiers *ast.ModifierList, name *ast.PropertyName, optional bool) *ast.Node {
	...
	if typeParameters != nil && len(typeParameters.Nodes) > 0 {
		...
		for _, tp := range typeParameters.Nodes {
			...
			if ast.IsTypeParameterDeclaration(tp) {
				...
				nodes = append(nodes,
					f.changeTracker.NodeFactory.UpdateTypeParameterDeclaration(typeParameter, typeParameter.Modifiers(), typeParameter.DotDotDotToken, typeParameter.Name(), constraint, typeParameter.Expression, defaultType))
			} else {
	...
	case ast.KindFunctionExpression:
		...
		return f.changeTracker.NodeFactory.UpdateFunctionExpression(fn, modifiers, core.IfElse(name != nil && ast.IsIdentifier(name), name, nil), typeParameters, parameters, typeNode, fn.FullSignature, core.OrElse(body, fn.Body))
	...
	case ast.KindMethodDeclaration:
		...
		return f.changeTracker.NodeFactory.UpdateMethodDeclaration(method, modifiers, methodName, questionToken, typeParameters, parameters, typeNode, method.FullSignature, body)

	case ast.KindFunctionDeclaration:
		fn := signatureDeclaration.AsFunctionDeclaration()
		return f.changeTracker.NodeFactory.UpdateFunctionDeclaration(fn, modifiers, fn.Target, fn.ColonToken, core.IfElse(name != nil && ast.IsIdentifier(name), name, nil), typeParameters, parameters, typeNode, fn.FullSignature, core.OrElse(body, fn.Body))
	}
...
func (f *missingMemberFixer) createSignatureDeclarationFromSignatures(signatures []*checker.Signature, name *ast.PropertyName, optional bool, modifiers *ast.ModifierList, quotePreference lsutil.QuotePreference, body *ast.FunctionBody, enclosingDeclaration *ast.Node) *ast.Node {
	...
	if hasRestParameter {
		// A tlua vararg: no name, and the annotation is the pack's *element* type.
		// Emitting the TS form (`...rest: unknown[]`) would generate a parameter
		// that does not compile -- a pack of arrays, which cannot implement the
		// member it was generated for.
		parameters.Nodes = append(parameters.Nodes, f.changeTracker.NodeFactory.NewParameterDeclaration(
			...
			f.changeTracker.NodeFactory.NewIdentifier(ast.VarargParameterName), nil, /*questionToken*/
			f.changeTracker.NodeFactory.NewKeywordTypeNode(ast.KindUnknownKeyword), nil, /*initializer*/
		))
	}
	...
	return f.changeTracker.NodeFactory.NewMethodDeclaration(
		modifiers, methodName, core.IfElse(optional, f.changeTracker.NodeFactory.NewToken(ast.KindQuestionToken), nil),
