		...
		for _, element := range bindingPattern.AsBindingPattern().Elements.Nodes {
			...
			if be.PropertyName != nil && ast.IsComputedPropertyName(be.PropertyName) {
				...
				*newNodes = append(*newNodes, compVarStmt)
				...
				accessExpr = factory.NewPropertyAccessExpression(baseExpr, nil, nil /*colonToken*/, factory.NewIdentifier(propText), ast.NodeFlagsNone)
			} else if ast.IsIdentifier(name) {
				accessExpr = factory.NewPropertyAccessExpression(baseExpr, nil, nil /*colonToken*/, factory.NewIdentifier(name.Text()), ast.NodeFlagsNone)
			} else {
				continue
			}
...
func (f *isolatedDeclarationsFixer) relativeType(node *ast.Node) *ast.TypeNode {
	...
	// Upstream also unioned the branches of a conditional expression here; tlua
	// has none in source, so an unsupported node falls through.
	return nil
}
