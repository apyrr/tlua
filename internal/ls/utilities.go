...
func getPossibleTypeArgumentsInfo(tokenIn *ast.Node, sourceFile *ast.SourceFile) *PossibleTypeArgumentInfo {
	...
	for token != nil {
		...
		case ast.KindEqualsGreaterThanToken, ast.KindIdentifier, ast.KindStringLiteral, ast.KindNumericLiteral,
			ast.KindTrueKeyword, ast.KindFalseKeyword, ast.KindTypeOfKeyword, ast.KindExtendsKeyword,
...
func getAdjustedLocation(node *ast.Node, forRename bool, sourceFile *ast.SourceFile) *ast.Node {
	...
	parent := node.Parent
	// /**/<modifier> [|name|] ...
	// /**/<modifier> <class|interface|type|module|namespace|function|get|set> [|name|] ...
	// /**/<class|interface|type|module|namespace|function|get|set> [|name|] ...
	// /**/import [|name|] = ...
	//
	...
	// /**/local [|name|] ...
	if node.Kind == ast.KindLocalKeyword &&
	...
	if !forRename {
		...
		// for k, v /**/in [|explist|] -- a Lua generic-for (KindForOfStatement)
		// keys on the `in` keyword.
		if node.Kind == ast.KindInKeyword && parent.Kind == ast.KindForOfStatement {
			return ast.SkipOuterExpressions(parent.Expression(), ast.OEKAll)
		}
	}
...
func getMeaningFromDeclaration(node *ast.Node) ast.SemanticMeaning {
	...
	case ast.KindNamedImports, ast.KindImportSpecifier, ast.KindImportEqualsDeclaration, ast.KindImportDeclaration,
...
// Resolves a `goto name` to the name in the `::name::` it targets, through the
// same walk the checker resolves with.
func getTargetLabel(referenceNode *ast.Node, labelName string) *ast.Node {
	if label := ast.FindTargetLabel(referenceNode, labelName); label != nil {
		return label.Label()
	}
	return nil
}
