...
func parameterDocComments(parameters []*ast.ParameterDeclarationNode, isJavaScriptFile bool, indentation, newLine string) string {
	...
	for i, parameter := range parameters {
		...
		// A vararg's `...` name is synthetic and unwritable, so it falls back to
		// the positional placeholder rather than emitting `@param ...`.
		if ast.IsIdentifier(parameter.Name()) && !ast.IsVarargParameter(parameter) {
