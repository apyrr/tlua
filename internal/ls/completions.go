...
const (
	...
	// Nothing assigns this sort text any more -- spread no longer parses -- but
	// generated fourslash tests still reference the constant.
...
func (l *LanguageService) getCompletionData(
	...
	getTypeScriptMemberSymbols := func() {
					...
					if !isTypeLocation && !insideJSDocTagTypeExpression &&
						core.Some(
							...
							func(decl *ast.Declaration) bool {
								return decl.Kind != ast.KindSourceFile && decl.Kind != ast.KindModuleDeclaration
							},
						) {
		...
		if !isTypeLocation || checker.IsInTypeQuery(node) {
			...
			typeChecker.TryGetThisTypeAtEx(node, false /*includeGlobalEnvironment*/, nil)
	...
	tryGetObjectLikeCompletionSymbols := func() (globalsSearch, error) {
		...
		if len(typeMembers) > 0 {
			...
			filteredMembers := filterObjectMembersList(
				typeMembers,
	...
	getGlobalCompletions := func() (globalsSearch, error) {
		...
		// The cursor position, not scopeNode's own range, decides which
		// position-gated Lua locals are visible.
		symbols = append(symbols, typeChecker.GetSymbolsInScopeAtPosition(scopeNode, adjustedPosition, symbolMeanings)...)
		...
		if scopeNode.Kind != ast.KindSourceFile {
			thisType := typeChecker.TryGetThisTypeAtEx(
				scopeNode,
				false, /*includeGlobalEnvironment*/
				nil,   /*container*/
			)
...
func (l *LanguageService) getCompletionEntriesFromSymbols(
	...
	for index, symbol := range data.symbols {
		...
		uniqueKey := name
		if isNumberKeyCompletion(symbol, origin) {
			uniqueKey = ast.SymbolName(symbol)
		}
		if name == "" ||
			uniques[uniqueKey] && (origin == nil || !originIsObjectLiteralMethod(origin)) ||
		...
		uniques[uniqueKey] = shouldShadowLaterSymbols
		var sym *ast.Symbol
		if includeSymbols {
			sym = symbol
		}
...
func (l *LanguageService) createCompletionItem(
	...
	// A number key inserts as an unquoted element access ("[1]"), never as a
	// quoted string key, because t[1] is disjoint from t["1"].
	isNumberKey := isNumberKeyCompletion(symbol, origin)
	if originIsThisTypeNode(origin) {
		...
		if useBraces {
			...
			case needsConvertPropertyAccess:
...
func getCompletionEntryDisplayNameForSymbol(
	...
	// A number key (t[1]) is disjoint from the string key t["1"], so its entry
	// must never render or insert as a string key. Handle it per completion
	// kind directly instead of decoding into the shared identifier/string path.
	if value, ok := ast.NumberKeyValue(name); ok {
		switch completionKind {
		case CompletionKindObjectPropertyDeclaration:
			// `1: v` is a numeric-literal member; `-1:` does not parse, so a
			// negative key must be written as the computed field `[-1] = v`.
			if strings.HasPrefix(value, "-") {
				return "[" + value + "]", false
			}
			return value, false
		case CompletionKindPropertyAccess, CompletionKindGlobal:
			// Convert `t.` to `t[1]`. The label stays the bare number (so tuple
			// element entries read as 0, 1, ...); createCompletionItem adds the
			// brackets to the insert text.
			return value, true
		case CompletionKindString, CompletionKindMemberLike:
			// A number key has no string-literal spelling.
			return "", false
		default:
			return value, false
		}
	...
	if name == "" ||
		...
		// expression can name it, so it is never a completion.
		ast.IsVarargSymbolName(name) ||
...
func isCheckedFile(file *ast.SourceFile, compilerOptions *core.CompilerOptions) bool {
	return !ast.IsSourceFileJS(file) || ast.IsCheckJSEnabledForFile(file)
}
...
func isContextTokenValueLocation(contextToken *ast.Node) bool {
	return contextToken != nil && ((contextToken.Kind == ast.KindTypeOfKeyword &&
		contextToken.Parent.Kind == ast.KindTypeQuery) ||
...
func isContextTokenTypeLocation(contextToken *ast.Node) bool {
	if contextToken != nil {
		...
		case ast.KindColonToken:
			return parentKind == ast.KindPropertySignature ||
...
func getContextualType(previousToken *ast.Node, position int, file *ast.SourceFile, typeChecker *checker.Checker) *checker.Type {
	...
	case ast.KindEqualsToken:
		...
		case ast.KindPropertyAssignment:
			...
			// to its constraint so literal candidates surface, and a value slot
			// whose own failed inference fixed the call's type parameters to the
			// error type is retried with those inferences blocked (like string
			// completions do).
			if initializer := parent.Initializer(); initializer != nil {
				t := typeChecker.GetContextualType(initializer, checker.ContextFlagsNone)
				if t == nil || t == typeChecker.GetErrorType() {
					t = typeChecker.GetContextualType(initializer, checker.ContextFlagsIgnoreNodeInferences)
				}
				if t != nil && t.IsTypeParameter() {
					if constraint := typeChecker.GetBaseConstraintOfType(t); constraint != nil {
						return constraint
					}
				}
				return t
...
// isNumberKeyCompletion reports whether a completion entry's symbol is a number
// key (t[1]), whose display name is already the element-access bracket form and
...
func generateIdentifierForArbitraryString(text string) string {
	...
	// `and`, `or` and `not` are identifier-shaped but scan as operators, so they
	// cannot be a binding name. The trailing underscore matches the `name as name_`
	// form used for a keyword-named import.
	if scanner.IsWordOperatorText(id) {
		return id + "_"
	}
...
var (
	...
	allKeywordCompletions   = sync.OnceValue(func() []*lsproto.CompletionItem {
		...
		for i := ast.KindFirstKeyword; i <= ast.KindLastKeyword; i++ {
			...
			// Some output-only kinds retain printable text after being de-reserved.
			if label == "" || scanner.StringToToken(label) != i {
				continue
			}
...
func getTypescriptKeywordCompletions(keywordFilter KeywordCompletionFilters) []*lsproto.CompletionItem {
	...
	result := core.Filter(allKeywordCompletions(), func(entry *lsproto.CompletionItem) bool {
		...
		case KeywordCompletionFiltersAll:
			return isFunctionLikeBodyKeyword(kind) ||
				...
				isTypeKeyword(kind) && kind != ast.KindNilKeyword
	...
	if keywordFilter == KeywordCompletionFiltersTypeAssertionKeywords {
		// `as const` survives the const keyword's deletion (the assertion is a
		// TypeReference named "const"), so the identifier spelling is offered
		// here by hand -- it no longer exists as a keyword kind to filter.
		result = append(result, &lsproto.CompletionItem{
			Label:    "const",
			Kind:     new(lsproto.CompletionItemKindKeyword),
			SortText: new(string(SortTextGlobalsOrKeywords)),
...
func computeCommitCharactersAndIsNewIdentifier(
	...
	case ast.KindCommaToken:
		...
		// var x: (s: string, list|
		// const obj = { x, |
		case ast.KindFunctionType, ast.KindObjectLiteralExpression:
...
// Determines if a type is exactly the same type resolved by the global 'self', 'global', or '_G'.
func isProbablyGlobalType(t *checker.Type, file *ast.SourceFile, typeChecker *checker.Checker) bool {
	...
	for _, name := range [...]string{"self", "global", "_G"} {
		symbol := typeChecker.GetGlobalSymbol(name, ast.SymbolFlagsValue, nil /*diagnostic*/)
		if symbol != nil && typeChecker.GetTypeOfSymbolAtLocation(symbol, file.AsNode()) == t {
			return true
		}
	}
	return false
...
func getConstraintOfTypeArgumentProperty(node *ast.Node, typeChecker *checker.Checker) *checker.Type {
	...
	case ast.KindPropertySignature:
		...
		if name := ast.GetPropertyNameForPropertyNameNode(reparsed.Name()); name != ast.InternalSymbolNameMissing {
...
func filterObjectMembersList(
	...
	for _, member := range existingMembers {
		...
		if ast.IsBindingElement(member) && member.PropertyName() != nil {
			...
			// Match against the identity (symbol-table) name so number-key
			// members ("1:" and "[1] =") filter their contextual symbol and do
			// not suppress the disjoint string key "1".
			if name != nil && (ast.IsPropertyNameLiteral(name) || ast.IsComputedPropertyName(name)) {
				if n := ast.GetPropertyNameForPropertyNameNode(name); n != ast.InternalSymbolNameMissing {
					existingName = n
				}
			}
		}
...
func tryGetObjectTypeDeclarationCompletionContainer(
	...
	default:
		if ast.IsObjectTypeDeclaration(location) {
			...
			isValidKeyword := isInterfaceOrTypeLiteralCompletionKeyword
...
func isSolelyIdentifierDefinitionLocation(
	...
	case ast.KindOpenParenToken:
		return ast.IsFunctionLikeKind(containingNodeKind)
	case ast.KindLessThanToken:
		return containingNodeKind == ast.KindInterfaceDeclaration || // interface A< |
	...
	case ast.KindAsteriskToken:
		return ast.IsFunctionLike(parent)
	}
	...
	return ast.IsDeclarationName(contextToken) &&
		...
		!((ast.IsInterfaceDeclaration(parent) || ast.IsTypeParameterDeclaration(parent)) &&
...
func getJSDocParameterCompletions(
	...
	return core.MapNonNil(fun.Parameters(), func(param *ast.ParameterDeclarationNode) *CompletionItem {
		...
		if ast.IsVarargParameter(param) {
			// A vararg has no name, so `@param` has nothing to document: its synthetic
			// `...` is unwritable and names no binding.
			return nil
		}
