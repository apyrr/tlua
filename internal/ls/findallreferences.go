...
func getContextNode(node *ast.Node) *ast.Node {
	...
	case ast.KindVariableDeclaration:
		if !ast.IsVariableDeclarationList(node.Parent) || len(node.Parent.AsVariableDeclarationList().Declarations.Nodes) != 1 {
			...
			return node.Parent.Parent
		} else if ast.IsForOfStatement(node.Parent.Parent) {
...
func getReferencedSymbolsSpecial(node *ast.Node, sourceFiles []*ast.SourceFile) []*SymbolAndEntries {
	...
	if ast.IsJumpStatementTarget(node) {
		...
		if labelDefinition := getTargetLabel(node.Parent, node.Text()); labelDefinition != nil {
			return getLabelReferencesInNode(labelScopeContainer(labelDefinition), labelDefinition)
		}
		return nil
	}

	if ast.IsLabelOfLabelStatement(node) {
		// it is a label definition and not a target, search the block that scopes it
		return getLabelReferencesInNode(labelScopeContainer(node), node)
	}
...
// A Lua label is visible in the block that declares it, so every goto that can
// reach it lives inside that block. labelIdentifier is the name in `::name::`.
func labelScopeContainer(labelIdentifier *ast.Node) *ast.Node {
	return labelIdentifier.Parent.Parent
}
...
func getLabelReferencesInNode(container *ast.Node, targetLabel *ast.Node) []*SymbolAndEntries {
	...
	references := core.MapNonNil(getPossibleSymbolReferenceNodes(sourceFile, labelName, container), func(node *ast.Node) *ReferenceEntry {
		...
		if node == targetLabel.AsNode() || (ast.IsJumpStatementTarget(node) && getTargetLabel(node, labelName) == targetLabel) {
