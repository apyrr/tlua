...
type sourceDefResolver struct {
	...
	// getRedirect returns the project-reference redirect for a containing file,
	// so a file inside a referenced project resolves against that project's root
	// rather than the top program's.
	getRedirect func(fileName string) module.ResolvedProjectReference
...
func (l *LanguageService) newSourceDefResolver(
	...
	return &sourceDefResolver{
		...
		getRedirect: func(fileName string) module.ResolvedProjectReference {
			// Return an untyped nil when there is no redirect, so the interface
			// value passed to ResolveModuleName is nil rather than a typed nil.
			if redirect := program.GetRedirectForResolution(ast.NewHasFileName(fileName, l.toPath(fileName))); redirect != nil {
				return redirect
			}
			return nil
		},
...
func (r *sourceDefResolver) resolveImplementationFrom(
	...
	for _, mode := range modes {
		resolved, _ := r.resolver.ResolveModuleName(moduleName, resolveFromFile, mode, r.getRedirect(resolveFromFile))
