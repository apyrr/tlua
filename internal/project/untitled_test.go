...
x++;`
	...
	files := map[string]any{
		"/Untitled-2.tlua": testContent,
	}

	session, _ := projecttestutil.Setup(files)

	ctx := projecttestutil.WithRequestID(context.Background())
	session.DidOpenFile(ctx, "file:///Untitled-2.tlua", 1, testContent, lsproto.LanguageKindTypeScript)

	// Get language service
	languageService, err := session.GetLanguageService(ctx, "file:///Untitled-2.tlua")
	assert.NilError(t, err)

	// Test the filename that the source file reports
	program := languageService.GetProgram()
	sourceFile := program.GetSourceFile("/Untitled-2.tlua")
	t.Logf("SourceFile.FileName() returns: '%s'", sourceFile.FileName())

	// Call ProvideReferences using the LSP method
	uri := lsproto.DocumentUri("file:///Untitled-2.tlua")
...
x++;`
	...
	// We expect to find 3 references, all inside Untitled-2. (Untitled-1's bare `x`
	// used to reference it through shared global scope, but every file is a module
	// now, so cross-file globals no longer exist.)
	assert.Assert(t, len(refs) == 3, "Expected 3 references, got %d", len(refs))
}
...
func TestImportsInUntitled(t *testing.T) {
	...
	files := map[string]any{
		...
		projecttestutil.TestTypingsLocation + "/node_modules/@types/somelib/index.d.tlua": `declare x: number;`,
	}
	session, _ := projecttestutil.Setup(files)
	content := `local mod = require("https://deno.land/std@0.208.0/path/mod.tlua");
		local rel = require("some.relative");
`
