...
func TestExtendedConfigCacheOwnership(t *testing.T) {
	...
	t.Run("transitive extended config ownership with new project", func(t *testing.T) {
		...
		session.DidCloseFile(context.Background(), "file:///user/username/projects/projectA/src/main.tlua")
		// Opening another file triggers cleanup of closed projects
		session.DidOpenFile(context.Background(), "file:///user/username/projects/other/src/main.tlua", 1, files["/user/username/projects/other/src/main.tlua"].(string), lsproto.LanguageKindTypeScript)

		// Close the other file too so only projectB remains
		session.DidCloseFile(context.Background(), "file:///user/username/projects/other/src/main.tlua")

		// Step 4: Trigger another snapshot clone for projectB
		session.DidChangeFile(context.Background(), "file:///user/username/projects/projectB/src/main.tlua", 2, []lsproto.TextDocumentContentChangePartialOrWholeDocument{
			{
		...
		_, err := session.GetLanguageService(context.Background(), "file:///user/username/projects/projectB/src/main.tlua")
