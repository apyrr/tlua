...
func TestProjectProgramUpdateKind(t *testing.T) {
	...
	t.Run("Cloned on single-file change", func(t *testing.T) {
		...
		_, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///src/index.tlua"))
		assert.NilError(t, err)
		session.DidChangeFile(context.Background(), "file:///src/index.tlua", 2, []lsproto.TextDocumentContentChangePartialOrWholeDocument{{
	...
	t.Run("NewFiles on root addition", func(t *testing.T) {
		...
		_, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///src/index.tlua"))
		assert.NilError(t, err)
		content := "export local y = 2;"
		err = utils.FS().WriteFile("/src/newfile.tlua", content)
		assert.NilError(t, err)
		session.DidChangeWatchedFiles(context.Background(), []*lsproto.FileEvent{{Uri: lsproto.DocumentUri("file:///src/newfile.tlua"), Type: lsproto.FileChangeTypeCreated}})
		session.DidOpenFile(context.Background(), "file:///src/newfile.tlua", 1, content, lsproto.LanguageKindTypeScript)
	...
	t.Run("SameFileNames when adding an unresolvable import with multi-file change", func(t *testing.T) {
		...
		_, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///src/index.tlua"))
		assert.NilError(t, err)
		// Change index.tlua to add an unresolvable require
		session.DidChangeFile(context.Background(), "file:///src/index.tlua", 2, []lsproto.TextDocumentContentChangePartialOrWholeDocument{{
...
func TestProject(t *testing.T) {
	...
	t.Run("commandLineWithTypingsFiles is reset on CommandLine change", func(t *testing.T) {
		t.Parallel()
		files := map[string]any{
			"/user/username/projects/project1/app.lua":      ``,
			"/user/username/projects/project1/package.json": `{"name":"p1","dependencies":{"jquery":"^3.1.0"}}`,
			"/user/username/projects/project2/app.lua":      ``,
		}
		...
		session, utils := projecttestutil.SetupWithTypingsInstaller(files, &projecttestutil.TypingsInstallerOptions{
			PackageToFile: map[string]string{
				...
				"jquery": `declare local $: { x: number }`,
			},
		})

		// 1) Open an inferred project file that triggers ATA
		uri1 := lsproto.DocumentUri("file:///user/username/projects/project1/app.lua")
		session.DidOpenFile(context.Background(), uri1, 1, files["/user/username/projects/project1/app.lua"].(string), lsproto.LanguageKindJavaScript)
		...
		uri2 := lsproto.DocumentUri("file:///user/username/projects/project2/app.lua")
	...
	t.Run("inferred project with a node_modules dependency that ships .d.ts types does not crash", func(t *testing.T) {
		t.Parallel()
		// Regression: a locally-installed dependency that ships its own `.d.ts` typings (the
		// npm norm, e.g. `@types/*` or a typed dev dependency) was discovered by ATA and
		// added to the inferred program as a root file. tlua renamed `.d.ts` away, so the
		// parser had no ScriptKind for it and panicked ("ScriptKind must be specified"),
		// crashing the language server on any project whose node_modules held such a package.
		files := map[string]any{
			"/user/username/projects/project1/app.lua":                       ``,
			"/user/username/projects/project1/package.json":                  `{"name":"p1","dependencies":{"foo":"^1.0.0"}}`,
			"/user/username/projects/project1/node_modules/foo/package.json": `{"name":"foo","version":"1.0.0","types":"./index.d.ts"}`,
			"/user/username/projects/project1/node_modules/foo/index.d.ts":   `export declare local x: number;`,
		}
		session, _ := projecttestutil.SetupWithTypingsInstaller(files, &projecttestutil.TypingsInstallerOptions{})

		uri := lsproto.DocumentUri("file:///user/username/projects/project1/app.lua")
		session.DidOpenFile(context.Background(), uri, 1, ``, lsproto.LanguageKindJavaScript)
		session.WaitForBackgroundTasks()

		ls, err := session.GetLanguageService(context.Background(), uri)
		assert.NilError(t, err)
		program := ls.GetProgram()
		// The foreign `.d.ts` must not be pulled into the program (and building it must not panic).
		assert.Assert(t, program.GetSourceFile("/user/username/projects/project1/node_modules/foo/index.d.ts") == nil)
	})
	...
	t.Run("inferred project rebuilt twice in one snapshot after typings install does not crash", func(t *testing.T) {
		...
		files := map[string]any{
			"/user/username/projects/project1/a.lua":        ``,
			"/user/username/projects/project1/package.json": `{"name":"p1","dependencies":{"jquery":"^3.1.0"}}`,
			"/user/username/projects/project1/b.tlua":       `export local y = 1;`,
		}
		...
		aURI := lsproto.DocumentUri("file:///user/username/projects/project1/a.lua")
		bURI := lsproto.DocumentUri("file:///user/username/projects/project1/b.tlua")
		...
		session.DidOpenFile(context.Background(), bURI, 1, files["/user/username/projects/project1/b.tlua"].(string), lsproto.LanguageKindTypeScript)
...
func TestPushDiagnostics(t *testing.T) {
	...
	t.Run("clears diagnostics when project is removed", func(t *testing.T) {
		...
		session.DidCloseFile(context.Background(), "file:///src/index.tlua")
		session.DidOpenFile(context.Background(), "file:///src2/index.tlua", 1, files["/src2/index.tlua"].(string), lsproto.LanguageKindTypeScript)
		_, err = session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///src2/index.tlua"))
		assert.NilError(t, err)
		session.WaitForBackgroundTasks()
	...
	t.Run("does not publish for inferred projects", func(t *testing.T) {
		t.Parallel()
		files := map[string]any{
			"/src/index.tlua": "local x: number = 'not a number';",
		}
		session, utils := projecttestutil.Setup(files)
		session.DidOpenFile(context.Background(), "file:///src/index.tlua", 1, files["/src/index.tlua"].(string), lsproto.LanguageKindTypeScript)
		_, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///src/index.tlua"))
...
use(s);`,
	...
	t.Run("cleans tsconfig diagnostics after TS files close and restores them after TS file is reopened", func(t *testing.T) {
		...
		session.DidOpenFile(context.Background(), uri, 2, files["/src/index.tlua"].(string), lsproto.LanguageKindTypeScript)
		_, err = session.GetLanguageService(context.Background(), uri)
		assert.NilError(t, err)
		session.WaitForBackgroundTasks()
...
func TestDisplayName(t *testing.T) {
	...
	t.Run("configured project returns relative config path", func(t *testing.T) {
		...
		_, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///home/projects/index.tlua"))
		assert.NilError(t, err)
	...
	t.Run("configured project with nested config", func(t *testing.T) {
		...
		_, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///home/projects/sub/index.tlua"))
		assert.NilError(t, err)
...
func TestProgressNotifications(t *testing.T) {
	...
	t.Run("each start has a matching finish", func(t *testing.T) {
		...
		_, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///home/projects/a.tlua"))
		assert.NilError(t, err)
