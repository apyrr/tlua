...
func (ti *TypingsInstaller) installTypings(
	...
	if packageNames, ok := ti.installWorker(projectID, requestID, scopedTypings, logger); ok {
		...
		// The typings location must be passed so installed typings resolve
		// through the typings-location fallback; the Lua module resolver never
		// probes node_modules/@types on its own. Note the fallback accepts
		// .d.tlua only — real npm @types packages ship .d.ts and will not
		// resolve; ATA is useful only for tlua-native typings packages.
		resolver := module.NewResolver(ti.host, &core.CompilerOptions{Module: core.ModuleKindNodeNext}, ti.typingsLocation, "")
...
func (ti *TypingsInstaller) typingToFileName(resolver *module.Resolver, packageName string) string {
	result, _ := resolver.ResolveModuleName(packageName, tspath.CombinePaths(ti.typingsLocation, "index"+tspath.ExtensionDts), core.ModuleKindNone, nil)
