...
func TestATA(t *testing.T) {
	...
	t.Run("configured projects", func(t *testing.T) {
		...
		session.DidOpenFile(context.Background(), lsproto.DocumentUri("file:///user/username/projects/project/app.lua"), 1, files["/user/username/projects/project/app.lua"].(string), lsproto.LanguageKindJavaScript)
	...
	t.Run("inferred projects", func(t *testing.T) {
		...
		ls, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///user/username/projects/project/app.lua"))
		assert.NilError(t, err)
		program := ls.GetProgram()
		jqueryTypesFile := program.GetSourceFile(projecttestutil.TestTypingsLocation + "/node_modules/@types/jquery/index.d.tlua")
	...
	t.Run("discover from node_modules", func(t *testing.T) {
		...
		files := map[string]any{
			...
			"/user/username/projects/project/node_modules/commander/index.lua":        "",
			"/user/username/projects/project/node_modules/commander/package.json":     `{ "name": "commander" }`,
			"/user/username/projects/project/node_modules/jquery/index.lua":           "",
		...
		session, utils := projecttestutil.SetupWithTypingsInstaller(files, &projecttestutil.TypingsInstallerOptions{
			...
			PackageToFile: map[string]string{
				"jquery": "declare local jquery: { x: number }",
			},
		})
	...
	t.Run("discover from node_modules empty types has import", func(t *testing.T) {
		...
		files := map[string]any{
			"/user/username/projects/project/app.lua":                                 `import "jquery";`,
	...
	t.Run("discover from bower_components", func(t *testing.T) {
		...
		files := map[string]any{
			"/user/username/projects/project/app.lua":                            ``,
			"/user/username/projects/project/jsconfig.json":                      `{}`,
			"/user/username/projects/project/bower_components/jquery/index.lua":  "",
		...
		ls, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///user/username/projects/project/app.lua"))
		assert.NilError(t, err)
		jqueryTypesFile := ls.GetProgram().GetSourceFile(projecttestutil.TestTypingsLocation + "/node_modules/@types/jquery/index.d.tlua")
		assert.Assert(t, jqueryTypesFile != nil, "jquery types should be installed")
	})
	...
	t.Run("Malformed package.json should be watched", func(t *testing.T) {
		...
		session, utils := projecttestutil.SetupWithTypingsInstaller(files, &projecttestutil.TypingsInstallerOptions{
			PackageToFile: map[string]string{
				"commander": "export local x: number",
			},
		})

		uri := lsproto.DocumentUri("file:///user/username/projects/project/app.lua")
		session.DidOpenFile(context.Background(), uri, 1, files["/user/username/projects/project/app.lua"].(string), lsproto.LanguageKindJavaScript)
		session.WaitForBackgroundTasks()
		...
		assert.NilError(t, err)
		program := ls.GetProgram()
		assert.Assert(t, program.GetSourceFile(projecttestutil.TestTypingsLocation+"/node_modules/@types/commander/index.d.tlua") != nil)
	})

	t.Run("should redo resolution that resolved to '.lua' file after typings are installed", func(t *testing.T) {
		t.Parallel()

		files := map[string]any{
			"/user/username/projects/project/app.lua":                  `local commander = require("commander");`,
			"/user/username/projects/node_modules/commander/index.lua": "return 0",
		}

		session, utils := projecttestutil.SetupWithTypingsInstaller(files, &projecttestutil.TypingsInstallerOptions{
			PackageToFile: map[string]string{
				"commander": "export local commander: number",
			},
		})
		...
		assert.Assert(t, program.GetSourceFile("/user/username/projects/node_modules/commander/index.lua") == nil)
	})
	...
	t.Run("expired cache entry (inferred project, should install typings)", func(t *testing.T) {
		...
		files := map[string]any{
			...
			projecttestutil.TestTypingsLocation + "/node_modules/@types/jquery/index.d.tlua": "export local x = 10;",
		...
		assert.Equal(t, program.GetSourceFile(projecttestutil.TestTypingsLocation+"/node_modules/@types/jquery/index.d.tlua").Text(), "export local y = 10")
	})
	...
	t.Run("non-expired cache entry (inferred project, should not install typings)", func(t *testing.T) {
		...
		assert.Equal(t, program.GetSourceFile(projecttestutil.TestTypingsLocation+"/node_modules/@types/jquery/index.d.tlua").Text(), "export local x = 10;")
	})
	...
	t.Run("deduplicate from local @types packages", func(t *testing.T) {
		...
		files := map[string]any{
			"/user/username/projects/project/app.lua":                               "",
			"/user/username/projects/project/node_modules/@types/node/index.d.tlua": "declare local node;",
			"/user/username/projects/project/jsconfig.json": `{
		...
		assert.NilError(t, err)
		program := ls.GetProgram()
		assert.Assert(t, program.GetSourceFile("/user/username/projects/project/node_modules/@types/node/index.d.tlua") != nil)
	})
	...
	t.Run("should install typings for unresolved imports", func(t *testing.T) {
		...
		files := map[string]any{
			"/user/username/projects/project/app.lua": `
				...
				local component = require("@ember/component");
			`,
		}

		session, utils := projecttestutil.SetupWithTypingsInstaller(files, &projecttestutil.TypingsInstallerOptions{
			PackageToFile: map[string]string{
				"node":             "export local node: number",
				"commander":        "export local commander: number",
				"ember__component": "export local ember__component: number",
			},
		})
		...
		assert.NilError(t, err)
		program := ls.GetProgram()
		nodeTypesFile := program.GetSourceFile(projecttestutil.TestTypingsLocation + "/node_modules/@types/node/index.d.tlua")
		assert.Assert(t, nodeTypesFile != nil, "node types should be installed")
		commanderTypesFile := program.GetSourceFile(projecttestutil.TestTypingsLocation + "/node_modules/@types/commander/index.d.tlua")
		assert.Assert(t, commanderTypesFile != nil, "commander types should be installed")
		emberComponentTypesFile := program.GetSourceFile(projecttestutil.TestTypingsLocation + "/node_modules/@types/ember__component/index.d.tlua")
