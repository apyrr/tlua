...
func DiscoverTypings(
	...
	for typing, inferred := range inferredTypings {
		if inferred != "" {
			// tlua can only consume its own `.d.tlua` declaration files. A package that
			// ships `.d.ts` typings (the npm norm, e.g. `@types/*` or a typed dependency)
			// resolves to a file the parser cannot assign a ScriptKind to, so including it
			// panics the compiler. Drop any inferred typing that is not a tlua declaration.
			if tspath.FileExtensionIs(inferred, tspath.ExtensionDts) {
				cachedTypingPaths = append(cachedTypingPaths, inferred)
			} else {
				logger.Log(fmt.Sprintf("ATA:: Ignoring non-tlua typings file for '%s': %s", typing, inferred))
			}
		} else {
