...
func (b *ProjectCollectionBuilder) updateInferredProjectRoots(rootFileNames []string, logger *logging.LogTree) bool {
	...
	if b.inferredProject.Value() == nil {
		...
		// Client-supplied options without RootDir must still anchor resolution at
		// the inferred project's directory, not the process cwd.
		newCompilerOptions = ensureInferredProjectRootDir(newCompilerOptions, b.sessionOptions.CurrentDirectory)
