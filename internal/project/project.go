...
func NewInferredProject(
	...
	// An inferred project has no tsconfig to anchor the Lua module search root,
	// so default RootDir to the project's own directory rather than letting it
	// fall back to the process cwd at resolution time.
	compilerOptions = ensureInferredProjectRootDir(compilerOptions, currentDirectory)
...
// ensureInferredProjectRootDir returns compiler options whose RootDir anchors
// the Lua module search root at the inferred project's own directory. Options
// that already set RootDir are returned unchanged; otherwise the value is
// cloned before mutation so a shared (client-supplied) options value is never
// modified in place.
func ensureInferredProjectRootDir(options *core.CompilerOptions, rootDir string) *core.CompilerOptions {
	if options.RootDir != "" {
		return options
	}
	clone := options.Clone()
	clone.RootDir = rootDir
	return clone
}
