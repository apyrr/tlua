...
func TestProcessChanges(t *testing.T) {
	...
	createOverlayFS := func() *overlayFS {
		testFS := vfstest.FromMap(map[string]string{
			"/test1.tlua": "// existing content",
			"/test2.tlua": "// existing content",
	...
	// Test URI constants
	const (
		testURI1 = lsproto.DocumentUri("file:///test1.tlua")
		testURI2 = lsproto.DocumentUri("file:///test2.tlua")
	)
	...
	t.Run("open falls back to file extension for unknown language kind", func(t *testing.T) {
		t.Parallel()
		fs := createOverlayFS()
		uri := lsproto.DocumentUri("file:///test1.tlua")
		...
		fs.processChanges([]FileChange{
			{
				...
				URI:          uri,
				Version:      1,
				Content:      "export local x = 1;",
				LanguageKind: lsproto.LanguageKind("unknownkind"),
			},
		})
	...
	t.Run("close then open in same batch marks as changed", func(t *testing.T) {
		...
		fh := fs.getFile(testURI1.FileName())
		assert.Equal(t, fh.Content(), "local x = 2;")
	})
}
