...
func (d *astDecoder) createChildrenNode(kind ast.Kind, data uint32, childIndices []int, commonData uint8) (*ast.Node, error) {
	...
	case ast.KindForOfStatement:
		...
		return d.factory.NewForOfStatement(initializer, expression, statement), nil
	case ast.KindNumericForStatement:
		it := newChildIter(childIndices)
		initializer := d.nodeAt(it.nextIf(mask, 0))
		from := d.nodeAt(it.nextIf(mask, 1))
		to := d.nodeAt(it.nextIf(mask, 2))
		step := d.nodeAt(it.nextIf(mask, 3))
		statement := d.nodeAt(it.nextIf(mask, 4))
		return d.factory.NewNumericForStatement(initializer, from, to, step, statement), nil
	case ast.KindRepeatStatement:
		it := newChildIter(childIndices)
		statements := d.nodeListAt(it.nextIf(mask, 0))
		expression := d.nodeAt(it.nextIf(mask, 1))
		return d.factory.NewRepeatStatement(statements, expression), nil
	case ast.KindBreakStatement:
		return d.factory.NewBreakStatement(), nil
	...
	case ast.KindGotoStatement:
		return d.factory.NewGotoStatement(d.singleChild(childIndices)), nil
	...
	case ast.KindFunctionDeclaration:
		it := newChildIter(childIndices)
		modifiers := d.modifierListAt(it.nextIf(mask, 0))
		target := d.nodeAt(it.nextIf(mask, 1))
		colonToken := d.nodeAt(it.nextIf(mask, 2))
		name := d.nodeAt(it.nextIf(mask, 3))
	...
	case ast.KindFunctionExpression:
		...
		typeNode := d.nodeAt(it.nextIf(mask, 4))
		body := d.nodeAt(it.nextIf(mask, 5))
		return d.factory.NewFunctionExpression(modifiers, name, typeParameters, parameters, typeNode, nil, body), nil
	...
	case ast.KindPropertyAccessExpression:
		...
		colonToken := d.nodeAt(it.nextIf(mask, 2))
		name := d.nodeAt(it.nextIf(mask, 3))
		return d.factory.NewPropertyAccessExpression(expression, questionDotToken, colonToken, name, 0), nil
	...
	case ast.KindTableEntry:
		return d.factory.NewTableEntry(d.singleChild(childIndices)), nil
	...
	case ast.KindMultiReturnType:
		return d.factory.NewMultiReturnTypeNode(d.singleNodeListChild(childIndices)), nil
	...
	case ast.KindTypeParameter:
		...
		dotDotDotToken := d.nodeAt(it.nextIf(mask, 1))
		name := d.nodeAt(it.nextIf(mask, 2))
		constraint := d.nodeAt(it.nextIf(mask, 3))
		expression := d.nodeAt(it.nextIf(mask, 4))
		defaultType := d.nodeAt(it.nextIf(mask, 5))
		return d.factory.NewTypeParameterDeclaration(modifiers, dotDotDotToken, name, constraint, expression, defaultType), nil
