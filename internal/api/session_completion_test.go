...
func TestCompletionSymbolTypeIsResolvable(t *testing.T) {
	...
	// The caret sits right after `people.`, requesting the object's members.
	...
	assert.NilError(t, err)
	assert.Assert(t, completions != nil, "expected a completion list for the object's members")
	...
	assert.Assert(t, sawPush, "member completions should include `push`")
}
