...
import (
	...
	"github.com/apyrr/tlua/internal/api"
...
func TestDocumentIdentifierUnmarshalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "uri object",
			input: `{"uri":"file:///foo.tlua"}`,
			uri:   "file:///foo.tlua",
		},
		{
			name:  "uri object with unknown fields",
			input: `{"uri":"file:///foo.tlua","extra":true}`,
			uri:   "file:///foo.tlua",
		},
		{
			name:  "empty object",
...
func TestNewDiagnosticResponseUsesUTF16Offsets(t *testing.T) {
	...
	text := "const 💩 = 1;"
	file := parser.ParseSourceFile(ast.SourceFileParseOptions{FileName: "/unicode.tlua"}, text, core.ScriptKindTS)
