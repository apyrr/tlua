...
import (
	...
	"github.com/apyrr/tlua/internal/vfs/vfsmatch"
)
...
func (r *Resolver) ResolveModuleName(moduleName string, containingFile string, resolutionMode core.ResolutionMode, redirectedReference ResolvedProjectReference) (*ResolvedModule, []DiagAndArgs) {
	...
	// Module names are Lua dotted names resolved against the project root and
	// node_modules; the moduleResolution option no longer selects a strategy.
	...
	if !r.isConfigLookup &&
		...
		for _, candidateDir := range candidateDirectories {
			if tspath.ContainsPath(candidateDir, finalPath, tspath.ComparePathsOptions{
				...
				jsAndDtsExtensions := []string{tspath.ExtensionJs, tspath.ExtensionJson, tspath.ExtensionDts}
...
// resolveLua resolves a Lua dotted module name (`a.b.c`) the way the runtime
// will: dots become directory separators and candidates are probed under the
// project root — the compile-time package.path — then under node_modules.
// A name is never resolved relative to the requiring file (that is Node's
// model, not Lua's), so the verbatim-emitted specifier means the same thing
// to the compiler and to the VM.
func (r *resolutionState) resolveLua() *ResolvedModule {
	if !tspath.IsLuaModuleName(r.name) {
		// A path-shaped specifier (./x, ../x, a/b, /abs) is not a Lua module
		// name and never resolves; the checker surfaces Cannot-find-module.
		if r.tracer != nil {
			r.tracer.write(diagnostics.Module_name_0_was_not_resolved, r.name)
		}
		return r.createResolvedModule(nil, false)
	}
	relativePath := tspath.LuaModuleNameToRelativePath(r.name)
	if resolved := r.probeLuaCandidates(tspath.CombinePaths(r.luaSearchRoot(), relativePath)); resolved != nil {
		return r.createResolvedModuleHandlingSymlink(resolved)
	}
	// node_modules fallback, anchored at the SEARCH ROOT and its ancestors —
	// never at the requiring file. Anchoring at the root keeps resolution a
	// pure function of the name (one name, one file, program-wide), which is
	// what package.loaded requires; walking the root's ancestors still finds
	// hoisted node_modules layouts. The dotted path maps into the package
	// tree; a package name may also reach an entrypoint via package.json
	// (types first, then main).
	result, _ := tspath.ForEachAncestorDirectory(r.luaSearchRoot(), func(directory string) (*resolved, bool) {
		nodeModules := tspath.CombinePaths(directory, "node_modules")
		// One DirectoryExists per ancestor instead of six file probes; the
		// tracking FS records the missing directory, so watch still
		// invalidates when a node_modules tree appears here later.
		if !r.resolver.host.FS().DirectoryExists(nodeModules) {
			return nil, false
		}
		base := tspath.CombinePaths(nodeModules, relativePath)
		if resolved := r.probeLuaCandidates(base); resolved != nil {
			return resolved, true
		}
		if r.resolver.host.FS().DirectoryExists(base) {
			if resolved := r.loadNodeModuleFromDirectory(extensionsImplementationFiles|extensionsDeclaration, base, true /*considerPackageJson*/); resolved != nil {
				return resolved, true
			}
		}
		return nil, false
	...
	if result != nil {
		return r.createResolvedModuleHandlingSymlink(result)
	}
	return r.createResolvedModule(nil, false)
}

// luaSearchRoot is the single configured search root (see
// CompilerOptions.GetLuaSearchRoot — synthesis uses the same anchor).
func (r *resolutionState) luaSearchRoot() string {
	return r.compilerOptions.GetLuaSearchRoot(r.resolver.host.GetCurrentDirectory())
}
...
// sees every miss and watch invalidates a failed resolution when the file
// appears later.
func (r *resolutionState) probeLuaCandidates(base string) *resolved {
	candidates := [...]struct {
		path      string
		extension string
	}{
	...
	for _, candidate := range candidates {
		if path, ok := r.tryFile(candidate.path); ok {
			return &resolved{path: path, extension: candidate.extension}
		}
	}
	return nil
}
...
func (r *resolutionState) tryAddingExtensions(extensionless string, extensions extensions, originalExtension string) *resolved {
	...
	default:
		if extensions&extensionsDeclaration != 0 && !tspath.IsDeclarationFileName(extensionless+originalExtension) {
			if resolved := r.tryExtension(".d"+originalExtension+tspath.ExtensionTs, extensionless, false); !resolved.shouldContinueSearching() {
				return resolved
			}
		}
		return continueSearching()
...
// tryFile probes exactly the candidate path. (moduleSuffixes is deleted: a
// compile-time suffix substitution has no runtime counterpart in package.path,
// so it could only make the checker and the VM disagree.)
...
func ResolveConfig(moduleName string, containingFile string, host ResolutionHost) *ResolvedModule {
	resolver := NewResolver(host, &core.CompilerOptions{Module: core.ModuleKindNodeNext}, "", "")
