...
// IsLuaModuleName reports whether name is a Lua dotted module name — the only
// specifier shape require() resolves. Dots separate segments and become
// directory separators during lookup; a path-shaped string (a separator
// anywhere, or an empty segment as in ".m", "m.", "a..b") is not a module
// name and never resolves.
func IsLuaModuleName(name string) bool {
...
// LuaModuleNameToRelativePath and LuaRelativePathToModuleName are the two
// halves of one mapping and must stay exact inverses: the resolver maps a
// require() name to the path it probes, and specifier synthesis maps a
// resolved path back to the name it emits.
func LuaModuleNameToRelativePath(name string) string {
	return strings.ReplaceAll(name, ".", "/")
}

// LuaRelativePathToModuleName converts a root-relative file path (extension
// already removed) to its dotted module name, or "" when the path cannot
// round-trip (a dot in any segment would be misread as a separator by the
// resolver's dots-to-slashes substitution).
func LuaRelativePathToModuleName(relativePath string) string {
	if relativePath == "" || strings.Contains(relativePath, ".") {
		return ""
	}
	name := strings.ReplaceAll(relativePath, "/", ".")
	if !IsLuaModuleName(name) {
		return ""
	}
	return name
}
