...
func WriteConfigFile(sys System, locale locale.Locale, reportDiagnostic DiagnosticReporter, options *collections.OrderedMap[string, any]) {
	...
	file := tspath.NormalizePath(tspath.CombinePaths(getCurrentDirectory, "tluaconfig.json"))
	if sys.FS().FileExists(file) {
		reportDiagnostic(ast.NewCompilerDiagnostic(diagnostics.A_tluaconfig_json_file_is_already_defined_at_Colon_0, file))
	} else {
		_ = sys.FS().WriteFile(file, generateTSConfig(options, locale))
		output := []string{"\n"}
		output = append(output, getHeader(sys, "Created a new tluaconfig.json")...)
