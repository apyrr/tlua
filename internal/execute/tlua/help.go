...
func getOptionsForHelp(commandLine *tsoptions.ParsedCommandLine) []*tsoptions.CommandLineOption {
	...
	opts = append(opts, &tsoptions.TluaBuildOption)
...
func getHeader(sys System, message string) []string {
	...
	const luaIcon = "     "
	const luaIconText = " Lua "
	const luaIconLength = len(luaIcon)

	luaIconFirstLine := colors.blueBackground(luaIcon)
	luaIconSecondLine := colors.blueBackground(colors.brightWhite(luaIconText))
	// If we have enough space, print tlua icon.
	if terminalWidth >= len(message)+luaIconLength {
		// right align of the icon is 120 at most.
		rightAlign := core.IfElse(terminalWidth > 120, 120, terminalWidth)
		leftAlign := rightAlign - luaIconLength
		header = append(header, fmt.Sprintf("%-*s", leftAlign, message), luaIconFirstLine, "\n")
		header = append(header, strings.Repeat(" ", leftAlign), luaIconSecondLine, "\n")
	} else {
...
func printEasyHelp(sys System, locale locale.Locale, simpleOptions []*tsoptions.CommandLineOption) {
	...
	msg := diagnostics.X_tlua_Colon_The_Lua_Compiler.Localize(locale) + " - " + diagnostics.Version_0.Localize(locale, core.Version())
	output = append(output, getHeader(sys, msg)...)

	output = append(output, colors.bold(diagnostics.COMMON_COMMANDS.Localize(locale)), "\n", "\n")

	example([]string{"tlua"}, diagnostics.Compiles_the_current_project_tluaconfig_json_in_the_working_directory)
	example([]string{"tlua app.tlua util.tlua"}, diagnostics.Ignoring_tluaconfig_json_compiles_the_specified_files_with_default_compiler_options)
	example([]string{"tlua -b"}, diagnostics.Build_a_composite_project_in_the_working_directory)
	example([]string{"tlua --init"}, diagnostics.Creates_a_tluaconfig_json_with_the_recommended_settings_in_the_working_directory)
	example([]string{"tlua -p ./path/to/tluaconfig.json"}, diagnostics.Compiles_the_Lua_project_located_at_the_specified_path)
	example([]string{"tlua --help --all"}, diagnostics.An_expanded_version_of_this_information_showing_all_possible_compiler_options)
	example([]string{"tlua --noEmit", "tlua --target esnext"}, diagnostics.Compiles_the_current_project_with_additional_settings)
...
func printAllHelp(sys System, locale locale.Locale, options []*tsoptions.CommandLineOption) {
	...
	beforeBuildOptions := diagnostics.Using_build_b_will_make_tlua_behave_more_like_a_build_orchestrator_than_a_compiler_This_is_used_to_trigger_building_composite_projects_which_you_can_learn_more_about_at_0.Localize(locale, "https://aka.ms/tsc-composite-builds")
	buildOptions := core.Filter(tsoptions.OptionsForBuild, func(option *tsoptions.CommandLineOption) bool {
		return option != &tsoptions.TluaBuildOption
	})
...
func PrintBuildHelp(sys System, locale locale.Locale, buildOptions []*tsoptions.CommandLineOption) {
	var output []string
	output = append(output, getHeader(sys, diagnostics.X_tlua_Colon_The_Lua_Compiler.Localize(locale)+" - "+diagnostics.Version_0.Localize(locale, core.Version()))...)
	before := diagnostics.Using_build_b_will_make_tlua_behave_more_like_a_build_orchestrator_than_a_compiler_This_is_used_to_trigger_building_composite_projects_which_you_can_learn_more_about_at_0.Localize(locale, "https://aka.ms/tsc-composite-builds")
