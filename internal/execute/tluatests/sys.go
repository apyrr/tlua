...
import (
	...
	"github.com/apyrr/tlua/internal/testutil/fsbaselineutil"
...
var tluaLibPath = "/home/src/tslibs/TS/Lib"

var tluaDefaultLibContent = stringtestutil.Dedent(`
...
func getTestLibPathFor(libName string) string {
	...
	if value, ok := tsoptions.LibMap.Get(libName); ok {
		...
		libFile = "lib." + libName + ".d.tlua"
	}
	return tluaLibPath + "/" + libFile
}
...
func newTestSys(tluaInput *tluaInput, forIncrementalCorrectness bool) *TestSys {
	cwd := tluaInput.cwd
	if cwd == "" {
		cwd = "/home/src/workspaces/project"
	}
	libPath := tluaLibPath
	if tluaInput.windowsStyleRoot != "" {
		libPath = tluaInput.windowsStyleRoot + libPath[1:]
	}
	currentWrite := &strings.Builder{}
	sys := NewTscSystem(tluaInput.files, !tluaInput.ignoreCase, cwd)
	...
	sys.tracer = harnessutil.NewTracerForBaselining(tspath.ComparePathsOptions{
		UseCaseSensitiveFileNames: !tluaInput.ignoreCase,
		CurrentDirectory:          cwd,
	}, currentWrite)
	sys.env = tluaInput.env
	...
	// Ensure the default library files are present
...
var (
	_ tlua.System             = (*TestSys)(nil)
	_ tlua.CommandLineTesting = (*TestSys)(nil)
)
...
func (s *TestSys) ensureLibPathExists(path string) {
	...
	if _, ok := s.fsFromFileMap().ReadFile(path); !ok {
		...
		err := s.fsFromFileMap().WriteFile(path, tluaDefaultLibContent)
		if err != nil {
...
func (s *TestSys) GetTrace(w io.Writer, locale locale.Locale) func(msg *diagnostics.Message, args ...any) {
	return func(msg *diagnostics.Message, args ...any) {
		...
		// With tlua -b building projects in parallel we cannot serialize the package.json lookup trace
