...
func createTestWatcher(t *testing.T) (*execute.Watcher, *TestSys) {
	...
	input := &tluaInput{
		files: FileMap{
			"/home/src/workspaces/project/a.tlua":          `local a: number = 1;`,
			"/home/src/workspaces/project/b.tlua":          `local a = require("a"); local b = a;`,
		...
		go func(i int) {
			...
			for j := range 10 {
				_ = sys.fsFromFileMap().WriteFile(
					"/home/src/workspaces/project/a.tlua",
					fmt.Sprintf("local a: number = %d;", i*10+j),
				)
				w.DoCycle()
			}
		}(i)
	...
	for i := range 4 {
		...
		go func(i int) {
			...
			for j := range 15 {
				_ = sys.fsFromFileMap().WriteFile(
					"/home/src/workspaces/project/a.tlua",
					fmt.Sprintf("local a: number = %d;", i*15+j),
				)
				w.DoCycle()
			}
		}(i)
...
func TestWatcherConcurrentFileChangesAndDoCycle(t *testing.T) {
	...
	for i := range 4 {
		...
		go func(i int) {
			defer wg.Done()
			for j := range 20 {
				path := fmt.Sprintf("/home/src/workspaces/project/gen_%d_%d.tlua", i, j)
				_ = sys.fsFromFileMap().WriteFile(path, fmt.Sprintf("local x%d_%d = %d;", i, j, j))
			}
		}(i)
	}
	...
	wg.Go(func() {
		for j := range 20 {
			_ = sys.fsFromFileMap().Remove(
				fmt.Sprintf("/home/src/workspaces/project/gen_0_%d.tlua", j),
			)
		}
	})
...
// TestWatcherRapidConfigChanges modifies tluaconfig.json rapidly from
...
func TestWatcherAlternatingModifyAndDoCycle(t *testing.T) {
	...
	wg.Go(func() {
		for j := range 100 {
			_ = sys.fsFromFileMap().WriteFile(
				"/home/src/workspaces/project/a.tlua",
				fmt.Sprintf("local a: number = %d;", j),
			)
		}
	})
...
func TestBuildWatchStopsWhenContextIsCancelled(t *testing.T) {
	...
	sys := newTestSys(&tluaInput{
		files: FileMap{
			// composite would force declaration emit, which fails with TS100054
			// for Lua modules and turns the build's exit status nonzero; plain
			// incremental keeps the build clean for the cancellation check.
			"/home/src/workspaces/project/tluaconfig.json": `{"compilerOptions":{"incremental":true},"files":["index.tlua"]}`,
	...
	resultCh := make(chan tlua.CommandLineResult, 1)
	go func() {
	...
	case result := <-resultCh:
		assert.Equal(t, result.Status, tlua.ExitStatusSuccess)
