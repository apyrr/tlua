	...
	testCases := []*tscInput{
		...
		},
		{
			...
			files: FileMap{
				...
				tscLibPath + "/lib.d.tlua": tscDefaultLibContent + "\n" + stringtestutil.Dedent(`
...
func TestTscLibraryResolution(t *testing.T) {
	...
	getTscLibraryResolutionFileMap := func(libReplacement bool) FileMap {
		...
		if libReplacement {
			files["/home/src/workspace/projects/node_modules/@typescript/lib-es5/index.d.tlua"] = tscDefaultLibContent
			files["/home/src/workspace/projects/node_modules/@typescript/lib-esnext/index.d.tlua"] = tscDefaultLibContent
...
func TestTscNoEmit(t *testing.T) {
	...
	getTscNoEmitChangesTestCases := func(commandLineArgs []string) []*tscInput {
		...
		// explanation.
		noChangeWithNoEmitWhileError := &tscEdit{
			caption:         "No Change run with noEmit",
			commandLineArgs: noChangeWithNoEmit.commandLineArgs,
			expectedDiff:    signaturePropagationDiff,
		}
		noChangeWithEmitWhileError := &tscEdit{
			caption:         "No Change run with emit",
			commandLineArgs: commandLineArgs,
			expectedDiff:    signaturePropagationDiff,
		}
