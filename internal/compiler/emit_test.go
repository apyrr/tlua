...
func generateLongLineTS(numProperties int) string {
	...
	b.WriteString("export local data: Record<string, number> = {")
			...
			p := compiler.NewProgram(compiler.ProgramOptions{
				Config: &tsoptions.ParsedCommandLine{
					ParsedConfig: &core.ParsedOptions{
						FileNames:       []string{"/dev/src/index.tlua"},
...
func BenchmarkEmitManyFiles(b *testing.B) {
	...
	for i := range numFiles {
		name := fmt.Sprintf("/dev/src/file_%d.tlua", i)
...
func BenchmarkEmitLongLinesWithLineBreaks(b *testing.B) {
	...
	sb.WriteString("export local data: Record<string, number> = {\n")
	for i := range numProperties {
		if i > 0 {
			sb.WriteString(",\n")
		}
