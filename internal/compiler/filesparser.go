...
func (t *parseTask) load(loader *fileLoader) {
	...
	if tspath.HasExtension(t.normalizedFilePath) {
		...
		// Reject files with an unsupported extension. `allowNonTsExtensions` relaxes this
		// for non-standard extensions, but only ones the parser can still handle: a file
		// whose extension yields no ScriptKind (e.g. a foreign `.d.ts`/`.ts`/`.js` pulled
		// in from node_modules) cannot be parsed and would panic the parser, so it must be
		// excluded regardless and surfaced as a diagnostic instead.
		if !loader.isSupportedExtension(canonicalFileName) &&
			(!allowNonTsExtensions || core.GetScriptKindFromFileName(t.normalizedFilePath) == core.ScriptKindUnknown) {
