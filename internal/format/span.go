...
// luaBlockOpenerLine returns the line of a Lua keyword-block's opening delimiter.
// A Lua block has no `{`; its opener is the token immediately preceding the first
// body statement -- `function()`/`then`/`do`. That line, not the block's own
// skip-trivia start (which is the first statement itself), is the indentation
// anchor: it lets the first statement indent one level and correctly handles a
// standalone `do ... end`, whose block range begins with `do` a line below its Pos().
func luaBlockOpenerLine(block *ast.Node, sourceFile *ast.SourceFile) int {
	if stmts := block.AsBlock().Statements; stmts != nil && len(stmts.Nodes) > 0 {
		firstStmtStart := scanner.GetTokenPosOfNode(stmts.Nodes[0], sourceFile, false)
		if opener := astnav.FindPrecedingToken(sourceFile, firstStmtStart); opener != nil {
			return getStartLineForNode(opener, sourceFile)
		}
	}
	// Empty block: no statement to indent, so anchor to the block's own start.
	return scanner.GetECMALineOfPosition(sourceFile, scanner.GetTokenPosOfNode(block, sourceFile, false))
}
...
func (w *formatSpanWorker) processChildNode(
	...
	// body statement rather than on a delimiter. Anchor it to its opening keyword
	// instead, mirroring a braced block whose `{` sits on the header line.
	if ast.IsLuaBlock(child) {
		childStartLine = luaBlockOpenerLine(child, w.sourceFile)
	}
...
func (i *dynamicIndenter) shouldAddDelta(line int, kind ast.Kind, container *ast.Node) bool {
	...
	// Lua's `end` (closes a keyword block) and `until` (closes a repeat body) sit at the parent level, like a `}`.
