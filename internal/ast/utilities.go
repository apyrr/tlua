...
func GetLuaLocals(container *Node) LuaLocalSymbolTable {
	...
	if data.LuaLocals == nil {
		data.LuaLocals = make(LuaLocalSymbolTable)
	}
	return data.LuaLocals
}

func ForEachVisibleLuaLocal(container *Node, reference *Node, cb func(*Symbol)) {
	if container == nil || reference == nil {
		return
	}
	ForEachVisibleLuaLocalAtPos(container, reference.Pos(), reference.End(), cb)
}

// ForEachVisibleLuaLocalAtPos is ForEachVisibleLuaLocal for a bare position:
// completions have a cursor position but no node there, and the enclosing
// scope node's own range (a source file starts at 0) must not be the
// visibility reference or every Lua local looks not-yet-declared.
func ForEachVisibleLuaLocalAtPos(container *Node, pos int, end int, cb func(*Symbol)) {
	if container == nil {
		return
	}
	for _, symbols := range container.LuaLocals() {
		for i := len(symbols) - 1; i >= 0; i-- {
			symbol := symbols[i]
			if IsLuaLocalVisibleAtPos(symbol, pos, end) {
				cb(symbol)
				break
			}
		}
...
func LookupLuaLocal(container *Node, name string, reference *Node, meaning SymbolFlags) *Symbol {
	if container == nil || reference == nil {
		return nil
	}
	symbols := container.LuaLocals()[name]
	for i := len(symbols) - 1; i >= 0; i-- {
		symbol := symbols[i]
		if symbol.Flags&meaning != 0 && IsLuaLocalVisibleAt(symbol, reference) {
			return symbol
		}
	}
	return nil
...
func IsLuaLocalVisibleAt(symbol *Symbol, reference *Node) bool {
	if reference == nil {
		return false
	}
	return IsLuaLocalVisibleAtPos(symbol, reference.Pos(), reference.End())
}

func IsLuaLocalVisibleAtPos(symbol *Symbol, pos int, end int) bool {
	if symbol == nil || symbol.ValueDeclaration == nil {
		return false
	}
	if IsFunctionDeclaration(symbol.ValueDeclaration) && IsLuaLocal(symbol.ValueDeclaration) {
		body := symbol.ValueDeclaration.Body()
		return symbol.ValueDeclaration.End() <= pos ||
			body != nil && body.Pos() <= pos && end <= body.End()
	}
	statement := FindAncestorKind(symbol.ValueDeclaration, KindVariableStatement)
	return statement != nil && statement.End() <= pos
}
...
func IsIdentifierName(node *Node) bool {
	...
	case KindPropertyDeclaration, KindPropertySignature, KindMethodDeclaration, KindMethodSignature, KindGetAccessor,
		KindSetAccessor, KindPropertyAssignment, KindPropertyAccessExpression:
...
func isLeftHandSideExpressionKind(kind Kind) bool {
	...
	case KindPropertyAccessExpression, KindElementAccessExpression, KindNewExpression, KindCallExpression,
		...
		KindPrivateIdentifier, KindRegularExpressionLiteral, KindNumericLiteral, KindStringLiteral,
		KindNoSubstitutionTemplateLiteral, KindTemplateExpression, KindFalseKeyword, KindNilKeyword, KindThisKeyword,
...
func IsLuaLocal(node *Node) bool {
	if GetCombinedNodeFlags(node)&NodeFlagsLuaLocal == 0 {
		return false
	}
	// A generic-for header list carries the LuaLocal flag only as the syntax
	// discriminator (see IsLuaGenericFor, which reads the raw flag); its
	// names are ordinary block-scoped loop variables, not Lua locals — they
	// must not skip block-scoped checks (use-before-declaration) or bind
	// into the Lua-local table (whose visibility rule requires a
	// VariableStatement ancestor).
	switch node.Kind {
	case KindVariableDeclaration:
		return node.Parent == nil || node.Parent.Parent == nil || !IsForOfStatement(node.Parent.Parent)
	case KindVariableDeclarationList:
		return node.Parent == nil || !IsForOfStatement(node.Parent)
	}
	return true
}

// LuaLocalValueList returns the value list expression of a Lua `local`
// declaration list — the last declaration's initializer — or nil.
func LuaLocalValueList(list *Node) *Node {
	declarations := list.AsVariableDeclarationList().Declarations.Nodes
	return declarations[len(declarations)-1].Initializer()
}

// HasLuaLocalValueList reports whether node is a name in a Lua `local` list
// whose value list assigns positionally: more than one name, or more than one
// value. The value list is held by the LAST declaration (so sibling node
// ranges stay ordered), but every name it covers counts as initialized.
func HasLuaLocalValueList(node *Node) bool {
	if node.Kind != KindVariableDeclaration || node.Parent == nil || node.Parent.Kind != KindVariableDeclarationList ||
		node.Parent.Flags&NodeFlagsLuaLocal == 0 {
		return false
	}
	valueList := LuaLocalValueList(node.Parent)
	if valueList == nil {
		return false
	}
...
func IsLuaFunctionBody(node *Node) bool {
	return node != nil && node.Flags&NodeFlagsLuaBlock != 0
}
...
func IsLuaBlock(node *Node) bool {
	return node != nil && node.Kind == KindBlock && node.Flags&NodeFlagsLuaBlock != 0
}

// IsLuaIf reports whether an IfStatement uses Lua `if ... then ... end`
// syntax. The parser wraps Lua if-arms in Lua-flagged Blocks; a TS `if` can
// acquire a Lua Block then-arm only via `if (c) do ... end` (a Lua do-block
// body), which this predicate deliberately classifies — and prints — as a
// Lua if (semantically identical; round-trips stably).
func IsLuaIf(node *Node) bool {
	return node.Kind == KindIfStatement && IsLuaBlock(node.AsIfStatement().ThenStatement)
}

// IsLuaWhile reports whether a WhileStatement uses Lua `while ... do ... end`
// syntax. Exact: after a while-condition, `do` always starts a Lua body, so a
// TS while can never own a Lua Block body.
func IsLuaWhile(node *Node) bool {
	return node.Kind == KindWhileStatement && IsLuaBlock(node.AsWhileStatement().Statement)
}

// IsLuaTableField reports whether a PropertyAssignment was parsed with Lua
// `[k] = v` table-field syntax rather than TS `[k]: v` (parser controlled;
// drives `=` printing).
...
// (`for k, v in explist do ... end`). Exact: only the Lua parse path builds a
// LuaLocal-flagged VariableDeclarationList initializer (TS for-of lists carry
// var/let/const/using flags; parser controlled).
func IsLuaGenericFor(node *Node) bool {
	if node.Kind != KindForOfStatement {
		return false
	}
	initializer := node.AsForOfStatement().Initializer
	return initializer != nil && initializer.Kind == KindVariableDeclarationList && initializer.Flags&NodeFlagsLuaLocal != 0
}

// IsLuaColonCall reports whether a CallExpression is a Lua method call
// `obj:name(args)` — sugar that passes the receiver as an implicit first
// argument. The parser builds the callee as a PropertyAccessExpression whose
// ColonToken records the sugar (only the call-rest path sets it, so a colon
// access never exists uncalled).
func IsLuaColonCall(node *Node) bool {
...
func CanHaveSymbol(node *Node) bool {
	...
	case KindArrowFunction, KindBinaryExpression, KindBindingElement, KindCallExpression, KindCallSignature,
		...
		KindConstructSignature, KindElementAccessExpression, KindExportAssignment,
...
func ForEachReturnStatement(body *Node, visitor func(stmt *Node) bool) bool {
	...
	traverse = func(node *Node) bool {
		...
		case KindReturnStatement:
			return visitor(node)
		case KindBlock, KindIfStatement, KindDoStatement, KindWhileStatement,
			KindForOfStatement, KindNumericForStatement, KindRepeatStatement, KindWithStatement, KindTryStatement, KindCatchClause:
...
// IsVarConst reports a Const-marked declaration list. There is no surface
// `const`; the producers are the ambient-global desugaring
// (`declare name: Type`) and factory-synthesized nodes.
...
// VarargParameterName is the text of the synthetic identifier the parser gives a
// Lua vararg parameter. The vararg has no binding name, but checker signatures
// assume every parameter owns a symbol, so it gets a zero-width name instead of
// a nil one. No source identifier can be spelled `...`, so the symbol is
// unreferenceable and cannot collide with a real declaration.
const VarargParameterName = "..."

// IsVarargSymbolName reports whether a *symbol* is a vararg's. Symbol-level code
// (the node builder, tuple labels, inlay hints, completions) cannot reach the
// declaration in every case -- the checker mints rest symbols of its own when it
// combines union and intersection signatures -- so it keys on the name. This is
// the one definition of that test: every site spelling it by hand is a site that
// can be forgotten, and a forgotten one prints `...args: T[]`, which is not tlua
// syntax and does not parse back.
func IsVarargSymbolName(name string) bool {
	return name == VarargParameterName
}

// IsVarargParameter reports whether node is a Lua vararg parameter: `...` or
// `...: T`. A named rest parameter keeps its DotDotDotToken for recovery, so the
// synthetic name is what distinguishes the two.
func IsVarargParameter(node *Node) bool {
	if node == nil || node.Kind != KindParameter {
		return false
	}
	param := node.AsParameterDeclaration()
	if param.DotDotDotToken == nil || param.Name() == nil || !IsIdentifier(param.Name()) {
		return false
	}
	return param.Name().Text() == VarargParameterName
}

// GetVarargParameter returns the vararg parameter of fn, or nil if fn is not a
// vararg function. A well-formed vararg is last, but every parameter is scanned:
// a misplaced one is already reported (a rest parameter must be last), and the
// body's `...` must still bind to it rather than cascade into "a '...' expression
// is only allowed inside a vararg function" on a function that visibly has one.
// When more than one parameter is a vararg, the LAST one wins: it is the one in
// (or nearest) the legal position, so it is what the body's `...` means.
func GetVarargParameter(fn *Node) *Node {
	if fn == nil {
		return nil
	}
	var vararg *Node
	for _, parameter := range fn.Parameters() {
		if IsVarargParameter(parameter) {
			vararg = parameter
		}
	}
	return vararg
...
// IsTrailingVarargParameter reports whether node is a vararg parameter in the
// legal (last) position. Only a trailing vararg is the signature's rest
// parameter; a misplaced one (already reported: a rest parameter must be last)
// is treated as a fixed parameter by the rest machinery, so its symbol keeps
// the annotation's element type un-wrapped.
func IsTrailingVarargParameter(node *Node) bool {
	if !IsVarargParameter(node) || node.Parent == nil {
		return false
	}
	parameters := node.Parent.Parameters()
	return len(parameters) != 0 && parameters[len(parameters)-1] == node
}

// IsPackTypeParameterDeclaration reports whether a type parameter is a Lua generic
// pack parameter, declared `<...A>`. Its value is a whole value pack, so `...A` and
// `...: A` spread it rather than repeating a single element type.
func IsPackTypeParameterDeclaration(node *Node) bool {
	return IsTypeParameterDeclaration(node) && node.AsTypeParameterDeclaration().DotDotDotToken != nil
}

// IsPackTypeReferencePosition reports whether a type node stands in a position that
// may spread a whole value pack: `...A` in a multiple-return list (a RestType whose
// parent is the multi-return node -- a rest element of a `[...]` tuple type, which
// survives parsing only in declaration files, is NOT a pack position and would
// silently misread the pack as a per-value element type), or `...: A` as a vararg
// parameter's annotation. A generic pack parameter is only legal in these
// positions; anywhere else it names several values in a single-value slot.
func IsPackTypeReferencePosition(node *Node) bool {
	parent := node.Parent
	if parent == nil {
		return false
	}
	switch parent.Kind {
	case KindRestType:
		return parent.AsRestTypeNode().Type == node && parent.Parent != nil && parent.Parent.Kind == KindMultiReturnType
	case KindParameter:
		return IsVarargParameter(parent) && parent.Type() == node
	}
	return false
}
...
func IsExpressionNode(node *Node) bool {
	switch node.Kind {
	case KindSuperKeyword, KindNilKeyword, KindTrueKeyword, KindFalseKeyword, KindRegularExpressionLiteral,
		...
		KindClassExpression, KindArrowFunction, KindVoidExpression, KindDeleteExpression,
		KindPrefixUnaryExpression, KindBinaryExpression, KindConditionalExpression,
		KindSpreadElement, KindTemplateExpression, KindOmittedExpression, KindJsxElement, KindJsxSelfClosingElement,
		KindJsxFragment, KindExpressionList, KindVarargExpression:
		return true
	...
	case KindIdentifier:
		...
		fallthrough
	case KindNumericLiteral, KindStringLiteral, KindNoSubstitutionTemplateLiteral, KindThisKeyword:
...
func IsInExpressionContext(node *Node) bool {
	parent := node.Parent
	switch parent.Kind {
	case KindVariableDeclaration, KindParameter, KindPropertyDeclaration, KindPropertySignature, KindPropertyAssignment, KindBindingElement:
		return parent.Initializer() == node
	case KindExpressionStatement, KindIfStatement, KindDoStatement, KindWhileStatement, KindReturnStatement, KindWithStatement,
		KindThrowStatement, KindTypeAssertionExpression, KindAsExpression, KindTemplateSpan, KindComputedPropertyName,
		KindSatisfiesExpression:
		return parent.Expression() == node
	case KindForOfStatement:
		// A generic-for's initializer is always a declaration list (the loop
		// names), never an expression; only the explist is in expression context.
		return parent.AsForOfStatement().Expression == node
	case KindNumericForStatement:
		s := parent.AsNumericForStatement()
...
func TryGetTextOfPropertyName(name *Node) (string, bool) {
	switch name.Kind {
	case KindIdentifier, KindPrivateIdentifier, KindStringLiteral, KindNumericLiteral,
		KindNoSubstitutionTemplateLiteral:
...
func IsBlockScope(node *Node, parentNode *Node) bool {
	switch node.Kind {
	case KindSourceFile, KindCatchClause, KindModuleDeclaration, KindForOfStatement,
		KindNumericForStatement, KindRepeatStatement,
...
func IsLabelName(node *Node) bool {
	return IsLabelOfLabelStatement(node) || IsJumpStatementTarget(node)
}

// StatementsOfLabelContainer returns the statement list that a label declared
// directly in node belongs to, or nil when node scopes no labels. A Lua label
// is scoped to its statement list, so this is the one definition of "block" for
// label visibility, duplicate detection, and the binder's label pre-scan.
func StatementsOfLabelContainer(node *Node) []*Node {
	switch node.Kind {
	case KindBlock, KindModuleBlock:
		return node.StatementList().Nodes
	case KindSourceFile:
...
func FindLabelStatement(statements []*Node, name string) *Node {
	for _, stmt := range statements {
		if stmt.Kind == KindLabelStatement && stmt.Label().Text() == name {
			return stmt
		}
	}
	return nil
...
// FindTargetLabel resolves the label a `goto name` at node targets: the nearest
// `::name::` declared in an enclosing block. A label is never visible across a
// function or module boundary — each compiles to its own Lua function — so the
// walk stops there, after searching the boundary's own statement list. This is
// the one resolution walk; the checker and the language service both use it,
// and the binder's activeLabelList resets at the same containers.
func FindTargetLabel(node *Node, name string) *Node {
	for current := node.Parent; current != nil; current = current.Parent {
		if label := FindLabelStatement(StatementsOfLabelContainer(current), name); label != nil {
			return label
		}
		if IsFunctionLikeOrClassStaticBlockDeclaration(current) || current.Kind == KindModuleBlock {
			break
		}
	}
	return nil
...
// The declared name in `::name::`.
func IsLabelOfLabelStatement(node *Node) bool {
	if !IsIdentifier(node) {
		return false
	}
	if !IsLabelStatement(node.Parent) {
		return false
	}
	return node == node.Parent.Label()
}

// The target name in `goto name`. Only goto jumps to a label; break and
// continue take no label in tlua.
func IsJumpStatementTarget(node *Node) bool {
	if !IsIdentifier(node) {
		return false
	}
	if !IsGotoStatement(node.Parent) {
		return false
	}
	return node == node.Parent.Label()
}
...
func IsInstantiatedModule(node *Node) bool {
	return GetModuleInstanceState(node) == ModuleInstanceStateInstantiated
}
...
func ForEachDynamicImportOrRequireCall(
	...
	for lastIndex >= 0 {
		...
		// `require("./m")` is Lua's module load, so a require call contributes a
		// module reference in every file, not just a JS one.
		if IsRequireCall(node, requireStringLiteralLikeArgument) {
...
func IsCheckJSEnabledForFile(sourceFile *SourceFile) bool {
	return sourceFile.CheckJsDirective != nil && sourceFile.CheckJsDirective.Enabled
}
...
func IsVariableLike(node *Node) bool {
	switch node.Kind {
	case KindBindingElement, KindParameter, KindPropertyAssignment, KindPropertyDeclaration,
...
// for a property name node. Numeric literal names map into the number-key
// namespace (Lua's t[1] != t["1"]); use TryGetTextOfPropertyName for the raw
// source text.
...
func ReplaceModifiers(factory *NodeFactory, node *Node, modifierArray *ModifierList) *Node {
	...
	case KindTypeParameter:
		return factory.UpdateTypeParameterDeclaration(
			...
			modifierArray,
			node.AsTypeParameterDeclaration().DotDotDotToken,
			node.Name(),
	...
	case KindFunctionDeclaration:
		return factory.UpdateFunctionDeclaration(
			...
			modifierArray,
			node.AsFunctionDeclaration().Target,
			node.AsFunctionDeclaration().ColonToken,
			node.Name(),
			node.TypeParameterList(),
			node.ParameterList(),
			node.Type(),
...
func IsTypeDeclaration(node *Node) bool {
	switch node.Kind {
	case KindTypeParameter, KindClassDeclaration, KindInterfaceDeclaration, KindTypeAliasDeclaration, KindJSTypeAliasDeclaration:
		return true
	case KindImportClause:
		return node.IsTypeOnly()
	case KindImportSpecifier, KindExportSpecifier:
...
func selectExpressionOfCallOrNewExpression(node *Node) *Node {
...
func IsCallExpressionTarget(node *Node, includeElementAccess bool, skipPastOuterExpressions bool) bool {
	return isCalleeWorker(node, IsCallExpression, selectExpressionOfCallOrNewExpression, includeElementAccess, skipPastOuterExpressions)
}

func IsNewExpressionTarget(node *Node, includeElementAccess bool, skipPastOuterExpressions bool) bool {
	return isCalleeWorker(node, IsNewExpression, selectExpressionOfCallOrNewExpression, includeElementAccess, skipPastOuterExpressions)
}

func IsCallOrNewExpressionTarget(node *Node, includeElementAccess bool, skipPastOuterExpressions bool) bool {
	return isCalleeWorker(node, IsCallOrNewExpression, selectExpressionOfCallOrNewExpression, includeElementAccess, skipPastOuterExpressions)
}
...
func IsPrimitiveLiteralValue(node *Node) bool {
	switch node.Kind {
...
func IsPotentiallyExecutableNode(node *Node) bool {
	if KindFirstStatement <= node.Kind && node.Kind <= KindLastStatement {
		if IsVariableStatement(node) {
			...
			if GetCombinedNodeFlags(declarationList)&(NodeFlagsBlockScoped|NodeFlagsLuaLocal) != 0 {
				return true
			}
	...
	return IsClassDeclaration(node) || IsModuleDeclaration(node)
}
