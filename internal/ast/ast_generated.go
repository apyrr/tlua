...
type (
	...
	ForOfStatementNode                = Node
	NumericForStatementNode           = Node
...
type (
	...
	BlockOrIfStatement             = Node // Block | IfStatement
	...
	DeclarationName                = Node // Identifier | PrivateIdentifier | StringLiteral | NumericLiteral | NoSubstitutionTemplateLiteral | ComputedPropertyName | BindingPattern | ElementAccessExpression
	ModuleName                     = Node // Identifier | StringLiteral
	ModuleExportName               = Node // Identifier | StringLiteral
	PropertyName                   = Node // Identifier | StringLiteral | NoSubstitutionTemplateLiteral | NumericLiteral | ComputedPropertyName | PrivateIdentifier
...
type IfStatement struct {
	...
	CompositeBase
	Expression    *Expression
	ThenStatement *BlockNode
	ElseStatement *BlockOrIfStatement // Optional
}

func (f *NodeFactory) NewIfStatement(expression *Expression, thenStatement *BlockNode, elseStatement *BlockOrIfStatement) *Node {
...
func (f *NodeFactory) UpdateIfStatement(node *IfStatement, expression *Expression, thenStatement *BlockNode, elseStatement *BlockOrIfStatement) *Node {
...
func (node *IfStatement) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateIfStatement(node, v.visitNode(node.Expression), v.visitNode(node.ThenStatement), v.visitEmbeddedStatement(node.ElseStatement))
}
...
func (f *NodeFactory) NewWhileStatement(expression *Expression, statement *BlockNode) *Node {
	data := &WhileStatement{}
	data.Expression = expression
...
func (f *NodeFactory) UpdateWhileStatement(node *WhileStatement, expression *Expression, statement *BlockNode) *Node {
...
func (f *NodeFactory) NewForOfStatement(initializer *ForInitializer, expression *Expression, statement *BlockNode) *Node {
	data := &ForOfStatement{}
	data.Initializer = initializer
	data.Expression = expression
	data.Statement = statement
	return f.newNode(KindForOfStatement, data)
}

func (f *NodeFactory) UpdateForOfStatement(node *ForOfStatement, initializer *ForInitializer, expression *Expression, statement *BlockNode) *Node {
	if initializer != node.Initializer || expression != node.Expression || statement != node.Statement {
		return updateNode(f.NewForOfStatement(initializer, expression, statement), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *ForOfStatement) ForEachChild(v Visitor) bool {
	return visit(v, node.Initializer) || visit(v, node.Expression) || visit(v, node.Statement)
}

func (node *ForOfStatement) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateForOfStatement(node, v.visitNode(node.Initializer), v.visitNode(node.Expression), v.visitIterationBody(node.Statement))
}

func (node *ForOfStatement) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewForOfStatement(node.Initializer, node.Expression, node.Statement), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewNumericForStatement(initializer *ForInitializer, from *Expression, to *Expression, step *Expression, statement *BlockNode) *Node {
	data := &NumericForStatement{}
	data.Initializer = initializer
	data.From = from
	data.To = to
	data.Step = step
	data.Statement = statement
	return f.newNode(KindNumericForStatement, data)
}

func (f *NodeFactory) UpdateNumericForStatement(node *NumericForStatement, initializer *ForInitializer, from *Expression, to *Expression, step *Expression, statement *BlockNode) *Node {
	if initializer != node.Initializer || from != node.From || to != node.To || step != node.Step || statement != node.Statement {
		return updateNode(f.NewNumericForStatement(initializer, from, to, step, statement), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *NumericForStatement) ForEachChild(v Visitor) bool {
...
func (node *NumericForStatement) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateNumericForStatement(node, v.visitNode(node.Initializer), v.visitNode(node.From), v.visitNode(node.To), v.visitNode(node.Step), v.visitIterationBody(node.Statement))
}

func (node *NumericForStatement) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewNumericForStatement(node.Initializer, node.From, node.To, node.Step, node.Statement), node.AsNode(), f.AsNodeFactory().hooks)
}

func (node *NumericForStatement) computeSubtreeFacts() SubtreeFacts {
	return propagateSubtreeFacts(node.Initializer) |
		propagateSubtreeFacts(node.From) |
		propagateSubtreeFacts(node.To) |
		propagateSubtreeFacts(node.Step) |
		propagateSubtreeFacts(node.Statement)
}

func IsNumericForStatement(node *Node) bool {
	return node.Kind == KindNumericForStatement
}
...
func (f *NodeFactory) NewRepeatStatement(statements *StatementList, expression *Expression) *Node {
	data := &RepeatStatement{}
	data.Statements = statements
	data.Expression = expression
	return f.newNode(KindRepeatStatement, data)
}

func (f *NodeFactory) UpdateRepeatStatement(node *RepeatStatement, statements *StatementList, expression *Expression) *Node {
	if statements != node.Statements || expression != node.Expression {
		return updateNode(f.NewRepeatStatement(statements, expression), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *RepeatStatement) ForEachChild(v Visitor) bool {
	return visitNodeList(v, node.Statements) || visit(v, node.Expression)
}

func (node *RepeatStatement) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateRepeatStatement(node, v.visitNodes(node.Statements), v.visitNode(node.Expression))
}

func (node *RepeatStatement) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewRepeatStatement(node.Statements, node.Expression), node.AsNode(), f.AsNodeFactory().hooks)
}

func (node *RepeatStatement) computeSubtreeFacts() SubtreeFacts {
	return propagateNodeListSubtreeFacts(node.Statements, propagateSubtreeFacts) |
		propagateSubtreeFacts(node.Expression)
}

func IsRepeatStatement(node *Node) bool {
	return node.Kind == KindRepeatStatement
}
...
func (f *NodeFactory) NewBreakStatement() *Node {
	data := &BreakStatement{}
	return f.newNode(KindBreakStatement, data)
}

func (node *BreakStatement) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewBreakStatement(), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewContinueStatement() *Node {
...
func (node *ContinueStatement) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewContinueStatement(), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewLabelStatement(label *IdentifierNode) *Node {
	data := &LabelStatement{}
	data.Label = label
	return f.newNode(KindLabelStatement, data)
}

func (f *NodeFactory) UpdateLabelStatement(node *LabelStatement, label *IdentifierNode) *Node {
	if label != node.Label {
		return updateNode(f.NewLabelStatement(label), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *LabelStatement) ForEachChild(v Visitor) bool {
	return visit(v, node.Label)
}

func (node *LabelStatement) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateLabelStatement(node, v.visitNode(node.Label))
}

func (node *LabelStatement) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewLabelStatement(node.Label), node.AsNode(), f.AsNodeFactory().hooks)
}

func IsLabelStatement(node *Node) bool {
	return node.Kind == KindLabelStatement
}
...
func (f *NodeFactory) NewGotoStatement(label *IdentifierNode) *Node {
	data := &GotoStatement{}
	data.Label = label
	return f.newNode(KindGotoStatement, data)
}

func (f *NodeFactory) UpdateGotoStatement(node *GotoStatement, label *IdentifierNode) *Node {
	if label != node.Label {
		return updateNode(f.NewGotoStatement(label), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *GotoStatement) ForEachChild(v Visitor) bool {
	return visit(v, node.Label)
}

func (node *GotoStatement) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateGotoStatement(node, v.visitNode(node.Label))
}

func (node *GotoStatement) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewGotoStatement(node.Label), node.AsNode(), f.AsNodeFactory().hooks)
}

func IsGotoStatement(node *Node) bool {
	return node.Kind == KindGotoStatement
}
...
func (f *NodeFactory) NewFunctionDeclaration(modifiers *ModifierList, target *Expression, colonToken *ColonToken, name *IdentifierNode, typeParameters *TypeParameterList, parameters *ParameterList, typeNode *TypeNode, fullSignature *TypeNode, body *FunctionBody) *Node {
...
func (f *NodeFactory) UpdateFunctionDeclaration(node *FunctionDeclaration, modifiers *ModifierList, target *Expression, colonToken *ColonToken, name *IdentifierNode, typeParameters *TypeParameterList, parameters *ParameterList, typeNode *TypeNode, fullSignature *TypeNode, body *FunctionBody) *Node {
	if modifiers != node.modifiers || target != node.Target || colonToken != node.ColonToken || name != node.name || typeParameters != node.TypeParameters || parameters != node.Parameters || typeNode != node.Type || fullSignature != node.FullSignature || body != node.Body {
		return updateNode(f.NewFunctionDeclaration(modifiers, target, colonToken, name, typeParameters, parameters, typeNode, fullSignature, body), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}
...
func (node *FunctionDeclaration) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateFunctionDeclaration(node, v.visitModifiers(node.modifiers), v.visitNode(node.Target), v.visitNode(node.ColonToken), v.visitNode(node.name), v.visitNodes(node.TypeParameters), v.visitParameters(node.Parameters), v.visitNode(node.Type), v.visitNode(node.FullSignature), v.visitFunctionBody(node.Body))
}

func (node *FunctionDeclaration) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewFunctionDeclaration(node.Modifiers(), node.Target, node.ColonToken, node.name, node.TypeParameters, node.Parameters, node.Type, node.FullSignature, node.Body), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewImportDeclaration(modifiers *ModifierList, importClause *ImportClauseNode, moduleSpecifier *Expression) *Node {
...
func (f *NodeFactory) NewJSImportDeclaration(modifiers *ModifierList, importClause *ImportClauseNode, moduleSpecifier *Expression) *Node {
	data := &ImportDeclaration{}
	data.modifiers = modifiers
	data.ImportClause = importClause
	data.ModuleSpecifier = moduleSpecifier
...
func (f *NodeFactory) UpdateImportDeclaration(node *ImportDeclaration, modifiers *ModifierList, importClause *ImportClauseNode, moduleSpecifier *Expression) *Node {
	if modifiers != node.modifiers || importClause != node.ImportClause || moduleSpecifier != node.ModuleSpecifier {
		switch node.Kind {
		case KindImportDeclaration:
			return updateNode(f.NewImportDeclaration(modifiers, importClause, moduleSpecifier), node.AsNode(), f.hooks)
		case KindJSImportDeclaration:
			return updateNode(f.NewJSImportDeclaration(modifiers, importClause, moduleSpecifier), node.AsNode(), f.hooks)
		default:
...
func (node *ImportDeclaration) ForEachChild(v Visitor) bool {
	return visitModifiers(v, node.modifiers) || visit(v, node.ImportClause) || visit(v, node.ModuleSpecifier)
}

func (node *ImportDeclaration) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateImportDeclaration(node, v.visitModifiers(node.modifiers), v.visitNode(node.ImportClause), v.visitNode(node.ModuleSpecifier))
}

func (node *ImportDeclaration) Clone(f NodeFactoryCoercible) *Node {
	switch node.Kind {
	case KindImportDeclaration:
		return cloneNode(f.AsNodeFactory().NewImportDeclaration(node.Modifiers(), node.ImportClause, node.ModuleSpecifier), node.AsNode(), f.AsNodeFactory().hooks)
	case KindJSImportDeclaration:
		return cloneNode(f.AsNodeFactory().NewJSImportDeclaration(node.Modifiers(), node.ImportClause, node.ModuleSpecifier), node.AsNode(), f.AsNodeFactory().hooks)
	default:
...
func IsKeywordExpression(node *Node) bool {
	switch node.Kind {
	case KindNilKeyword, KindTrueKeyword, KindFalseKeyword, KindThisKeyword, KindSuperKeyword, KindImportKeyword:
		return true
	}
	return false
}
...
func (f *NodeFactory) NewFunctionExpression(modifiers *ModifierList, name *IdentifierNode, typeParameters *TypeParameterList, parameters *ParameterList, typeNode *TypeNode, fullSignature *TypeNode, body *FunctionBody) *Node {
...
func (f *NodeFactory) UpdateFunctionExpression(node *FunctionExpression, modifiers *ModifierList, name *IdentifierNode, typeParameters *TypeParameterList, parameters *ParameterList, typeNode *TypeNode, fullSignature *TypeNode, body *FunctionBody) *Node {
	if modifiers != node.modifiers || name != node.name || typeParameters != node.TypeParameters || parameters != node.Parameters || typeNode != node.Type || fullSignature != node.FullSignature || body != node.Body {
		return updateNode(f.NewFunctionExpression(modifiers, name, typeParameters, parameters, typeNode, fullSignature, body), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}
...
func (node *FunctionExpression) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateFunctionExpression(node, v.visitModifiers(node.modifiers), v.visitNode(node.name), v.visitNodes(node.TypeParameters), v.visitParameters(node.Parameters), v.visitNode(node.Type), v.visitNode(node.FullSignature), v.visitFunctionBody(node.Body))
}

func (node *FunctionExpression) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewFunctionExpression(node.Modifiers(), node.name, node.TypeParameters, node.Parameters, node.Type, node.FullSignature, node.Body), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewExpressionList(elements *ElementList) *Node {
	data := &ExpressionList{}
	data.Elements = elements
	return f.newNode(KindExpressionList, data)
}

func (f *NodeFactory) UpdateExpressionList(node *ExpressionList, elements *ElementList) *Node {
	if elements != node.Elements {
		return updateNode(f.NewExpressionList(elements), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *ExpressionList) ForEachChild(v Visitor) bool {
	return visitNodeList(v, node.Elements)
}

func (node *ExpressionList) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateExpressionList(node, v.visitNodes(node.Elements))
}

func (node *ExpressionList) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewExpressionList(node.Elements), node.AsNode(), f.AsNodeFactory().hooks)
}

func (node *ExpressionList) computeSubtreeFacts() SubtreeFacts {
	return propagateNodeListSubtreeFacts(node.Elements, propagateSubtreeFacts)
}

func IsExpressionList(node *Node) bool {
	return node.Kind == KindExpressionList
}
...
func (f *NodeFactory) NewVarargExpression() *Node {
	data := &VarargExpression{}
	return f.newNode(KindVarargExpression, data)
}

func (node *VarargExpression) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewVarargExpression(), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (node *PropertyAccessExpression) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdatePropertyAccessExpression(node, v.visitNode(node.Expression), v.visitNode(node.QuestionDotToken), v.visitNode(node.ColonToken), v.visitNode(node.name), node.Flags)
}

func (node *PropertyAccessExpression) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewPropertyAccessExpression(node.Expression, node.QuestionDotToken, node.ColonToken, node.name, node.Flags), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewTableEntry(expression *Expression) *Node {
	data := &TableEntry{}
	data.Expression = expression
	return f.newNode(KindTableEntry, data)
}

func (f *NodeFactory) UpdateTableEntry(node *TableEntry, expression *Expression) *Node {
	if expression != node.Expression {
		return updateNode(f.NewTableEntry(expression), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *TableEntry) ForEachChild(v Visitor) bool {
	return visit(v, node.Expression)
}

func (node *TableEntry) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateTableEntry(node, v.visitNode(node.Expression))
}

func (node *TableEntry) Clone(f NodeFactoryCoercible) *Node {
...
func IsKeywordTypeNode(node *Node) bool {
	switch node.Kind {
	case KindAnyKeyword, KindBooleanKeyword, KindFunctionKeyword, KindIntrinsicKeyword, KindNeverKeyword, KindNumberKeyword, KindObjectKeyword, KindThreadKeyword, KindUserdataKeyword, KindCDataKeyword, KindStringKeyword, KindSymbolKeyword, KindSelfKeyword, KindNilKeyword, KindUnknownKeyword, KindVoidKeyword:
		return true
	}
	return false
}
...
type MultiReturnTypeNode struct {
	TypeNodeBase
	Elements *TypeList
}

func (f *NodeFactory) NewMultiReturnTypeNode(elements *TypeList) *Node {
	data := &MultiReturnTypeNode{}
	data.Elements = elements
	return f.newNode(KindMultiReturnType, data)
}

func (f *NodeFactory) UpdateMultiReturnTypeNode(node *MultiReturnTypeNode, elements *TypeList) *Node {
	if elements != node.Elements {
		return updateNode(f.NewMultiReturnTypeNode(elements), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *MultiReturnTypeNode) ForEachChild(v Visitor) bool {
	return visitNodeList(v, node.Elements)
}

func (node *MultiReturnTypeNode) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateMultiReturnTypeNode(node, v.visitNodes(node.Elements))
}

func (node *MultiReturnTypeNode) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewMultiReturnTypeNode(node.Elements), node.AsNode(), f.AsNodeFactory().hooks)
}

func IsMultiReturnTypeNode(node *Node) bool {
	return node.Kind == KindMultiReturnType
}
...
func (f *NodeFactory) UpdateFunctionTypeNode(node *FunctionTypeNode, modifiers *ModifierList, typeParameters *TypeParameterList, parameters *ParameterList, typeNode *TypeNode) *Node {
	if modifiers != node.modifiers || typeParameters != node.TypeParameters || parameters != node.Parameters || typeNode != node.Type {
		return updateNode(f.NewFunctionTypeNode(modifiers, typeParameters, parameters, typeNode), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}
...
func (f *NodeFactory) NewJSDocImportTag(tagName *IdentifierNode, importClause *ImportClauseNode, moduleSpecifier *Expression, comment *NodeList) *Node {
	data := &JSDocImportTag{}
	data.TagName = tagName
	data.ImportClause = importClause
	data.ModuleSpecifier = moduleSpecifier
...
func (f *NodeFactory) UpdateJSDocImportTag(node *JSDocImportTag, tagName *IdentifierNode, importClause *ImportClauseNode, moduleSpecifier *Expression, comment *NodeList) *Node {
	if tagName != node.TagName || importClause != node.ImportClause || moduleSpecifier != node.ModuleSpecifier || comment != node.Comment {
		return updateNode(f.NewJSDocImportTag(tagName, importClause, moduleSpecifier, comment), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}
...
func (node *JSDocImportTag) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateJSDocImportTag(node, v.visitNode(node.TagName), v.visitNode(node.ImportClause), v.visitNode(node.ModuleSpecifier), v.visitNodes(node.Comment))
}

func (node *JSDocImportTag) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewJSDocImportTag(node.TagName, node.ImportClause, node.ModuleSpecifier, node.Comment), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewExportDeclaration(modifiers *ModifierList, isTypeOnly bool, exportClause *NamedExportBindings, moduleSpecifier *Expression) *Node {
...
func (f *NodeFactory) UpdateExportDeclaration(node *ExportDeclaration, modifiers *ModifierList, isTypeOnly bool, exportClause *NamedExportBindings, moduleSpecifier *Expression) *Node {
	if modifiers != node.modifiers || isTypeOnly != node.IsTypeOnly || exportClause != node.ExportClause || moduleSpecifier != node.ModuleSpecifier {
		return updateNode(f.NewExportDeclaration(modifiers, isTypeOnly, exportClause, moduleSpecifier), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *ExportDeclaration) ForEachChild(v Visitor) bool {
	return visitModifiers(v, node.modifiers) || visit(v, node.ExportClause) || visit(v, node.ModuleSpecifier)
}

func (node *ExportDeclaration) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateExportDeclaration(node, v.visitModifiers(node.modifiers), node.IsTypeOnly, v.visitNode(node.ExportClause), v.visitNode(node.ModuleSpecifier))
}

func (node *ExportDeclaration) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewExportDeclaration(node.Modifiers(), node.IsTypeOnly, node.ExportClause, node.ModuleSpecifier), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewImportTypeNode(isTypeOf bool, argument *TypeNode, qualifier *EntityName, typeArguments *TypeList) *Node {
	data := &ImportTypeNode{}
...
func (f *NodeFactory) UpdateImportTypeNode(node *ImportTypeNode, isTypeOf bool, argument *TypeNode, qualifier *EntityName, typeArguments *TypeList) *Node {
	if isTypeOf != node.IsTypeOf || argument != node.Argument || qualifier != node.Qualifier || typeArguments != node.TypeArguments {
		return updateNode(f.NewImportTypeNode(isTypeOf, argument, qualifier, typeArguments), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}

func (node *ImportTypeNode) ForEachChild(v Visitor) bool {
	return visit(v, node.Argument) || visit(v, node.Qualifier) || visitNodeList(v, node.TypeArguments)
}

func (node *ImportTypeNode) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateImportTypeNode(node, node.IsTypeOf, v.visitNode(node.Argument), v.visitNode(node.Qualifier), v.visitNodes(node.TypeArguments))
}

func (node *ImportTypeNode) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewImportTypeNode(node.IsTypeOf, node.Argument, node.Qualifier, node.TypeArguments), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (f *NodeFactory) NewTypeParameterDeclaration(modifiers *ModifierList, dotDotDotToken *DotDotDotToken, name *IdentifierNode, constraint *TypeNode, expression *Expression, defaultType *TypeNode) *Node {
...
func (f *NodeFactory) UpdateTypeParameterDeclaration(node *TypeParameterDeclaration, modifiers *ModifierList, dotDotDotToken *DotDotDotToken, name *IdentifierNode, constraint *TypeNode, expression *Expression, defaultType *TypeNode) *Node {
	if modifiers != node.modifiers || dotDotDotToken != node.DotDotDotToken || name != node.name || constraint != node.Constraint || expression != node.Expression || defaultType != node.DefaultType {
		return updateNode(f.NewTypeParameterDeclaration(modifiers, dotDotDotToken, name, constraint, expression, defaultType), node.AsNode(), f.hooks)
	}
	return node.AsNode()
}
...
func (node *TypeParameterDeclaration) VisitEachChild(v *NodeVisitor) *Node {
	return v.Factory.UpdateTypeParameterDeclaration(node, v.visitModifiers(node.modifiers), v.visitNode(node.DotDotDotToken), v.visitNode(node.name), v.visitNode(node.Constraint), v.visitNode(node.Expression), v.visitNode(node.DefaultType))
}

func (node *TypeParameterDeclaration) Clone(f NodeFactoryCoercible) *Node {
	return cloneNode(f.AsNodeFactory().NewTypeParameterDeclaration(node.Modifiers(), node.DotDotDotToken, node.name, node.Constraint, node.Expression, node.DefaultType), node.AsNode(), f.AsNodeFactory().hooks)
}
...
func (n *Node) ForEachChild(v Visitor) bool {
	...
	case KindForOfStatement:
		return n.data.(*ForOfStatement).ForEachChild(v)
	case KindNumericForStatement:
		return n.data.(*NumericForStatement).ForEachChild(v)
	...
	case KindGotoStatement:
		return n.data.(*GotoStatement).ForEachChild(v)
	...
	case KindMultiReturnType:
		return n.data.(*MultiReturnTypeNode).ForEachChild(v)
...
func (n *Node) AsForOfStatement() *ForOfStatement {
	return n.data.(*ForOfStatement)
}

func (n *Node) AsNumericForStatement() *NumericForStatement {
	return n.data.(*NumericForStatement)
}

func (n *Node) AsRepeatStatement() *RepeatStatement {
	return n.data.(*RepeatStatement)
}
...
func (n *Node) AsGotoStatement() *GotoStatement {
	return n.data.(*GotoStatement)
}
...
func (n *Node) AsExpressionList() *ExpressionList {
	return n.data.(*ExpressionList)
}
...
func (n *Node) AsTableEntry() *TableEntry {
	return n.data.(*TableEntry)
}
...
func (n *Node) AsMultiReturnTypeNode() *MultiReturnTypeNode {
	return n.data.(*MultiReturnTypeNode)
}
...
func IsModifierKind(kind Kind) bool {
	switch kind {
	case KindAbstractKeyword, KindAccessorKeyword, KindSuspendKeyword, KindConstKeyword, KindDeclareKeyword, KindDefaultKeyword, KindExportKeyword, KindInKeyword, KindPrivateKeyword, KindProtectedKeyword, KindPublicKeyword, KindReadonlyKeyword, KindOutKeyword, KindOverrideKeyword, KindStaticKeyword:
		return true
	}
	return false
}
...
func IsPrefixUnaryOperator(kind Kind) bool {
	switch kind {
	case KindPlusToken, KindMinusToken, KindExclamationToken, KindHashToken:
		return true
	}
	return false
}
...
func IsConcatenationOperator(kind Kind) bool {
	switch kind {
...
func IsConcatenationOperatorOrHigher(kind Kind) bool {
	switch kind {
	case KindAsteriskAsteriskToken, KindAsteriskToken, KindSlashToken, KindPercentToken, KindPlusToken, KindMinusToken, KindDotDotToken:
		return true
	}
	return false
}
