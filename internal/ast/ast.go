...
func (n *Node) LuaLocals() LuaLocalSymbolTable {
	data := n.LocalsContainerData()
	if data != nil {
...
func (n *Node) Expression() *Node {
	...
	case KindTableEntry:
		return n.AsTableEntry().Expression
...
func (m *MutableNode) SetExpression(expr *Node) {
	...
	case KindTableEntry:
		n.AsTableEntry().Expression = expr
	case KindSpreadElement:
...
func (n *Node) StatementList() *NodeList {
	...
	case KindRepeatStatement:
		return n.AsRepeatStatement().Statements
	}
...
func (n *Node) CanHaveStatements() bool {
	switch n.Kind {
	case KindSourceFile, KindBlock, KindModuleBlock, KindRepeatStatement:
		return true
	default:
		return false
	}
...
func (n *Node) Label() *Node {
	...
	case KindLabelStatement:
		return n.AsLabelStatement().Label
	case KindGotoStatement:
		return n.AsGotoStatement().Label
	}
...
func (n *Node) ElementList() *NodeList {
	...
	case KindMultiReturnType:
		return n.AsMultiReturnTypeNode().Elements
	case KindExpressionList:
		return n.AsExpressionList().Elements
	}
...
func GetDeclarationFromName(name *Node) *Declaration {
	...
	case KindIdentifier:
		...
		if IsBinaryExpression(binExp) && GetAssignmentDeclarationKind(binExp) != JSDeclarationKindNone {
			// In Lua files the binder stamps a symbol on every member write, so the
			// symbol-presence test below no longer distinguishes declaring
			// assignments — this returns ordinary writes too. Consumers must judge
			// the result (e.g. against the target symbol's declarations) rather
			// than trust it as a declaration.
...
func declarationIsWriteAccess(decl *Node) bool {
	...
	case KindPropertyAssignment:
		// Destructuring assignment is deleted, so a property assignment's name
...
func accessKind(node *Node) AccessKind {
	...
	case KindArrayLiteralExpression:
		return accessKind(parent)
	case KindForOfStatement:
		if node == parent.AsForOfStatement().Initializer {
			return AccessKindWrite
		}
		return AccessKindRead
	default:
...
const (
	...
	AccessKindReadWrite                   // Reads from and writes to a variable. E.g.: `x += 1`.
)
...
type LuaLocalSymbolTable map[string][]*Symbol
...
func (node *ForOfStatement) computeSubtreeFacts() SubtreeFacts {
