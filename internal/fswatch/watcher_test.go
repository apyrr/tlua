...
func TestConsolidatedSymlinkChildMapsSharedLogicalPath(t *testing.T) {
	...
	event := Event{Kind: EventUpdate, Path: filepath.Join(logicalParent, "target", "file.tlua")}
	got := cb.mapEvent(event)
	want := filepath.Join(link, "file.tlua")
	if got.Path != want {
...
func TestConsolidatedChildWatchFiltersAgainstRequestedDir(t *testing.T) {
	...
	dw.events.update(filepath.Join(child, "file.tlua"))
	dw.events.update(filepath.Join(child, "nested", "file.tlua"))
	dw.events.update(filepath.Join(sibling, "file.tlua"))
	dw.triggerCallbacks()

	gotW := toWantEvents(got)
	want := []wantEvent{
		{EventUpdate, child},
		{EventUpdate, filepath.Join(child, "file.tlua")},
	}
	cmpEvents := func(a, b wantEvent) int {
		if a.Kind != b.Kind {
			return cmp.Compare(a.Kind, b.Kind)
...
func TestRecursiveWatchWithIgnoreDoesNotFilterByLogicalRoot(t *testing.T) {
	...
	var got []Event
	outsidePath := filepath.Join(t.TempDir(), "outside", "pkg", "index.tlua")
