...
func getOptionValue(t *testing.T, option *tsoptions.CommandLineOption, value string, cwd string) tsoptions.CompilerOptionsValue {
	...
	case tsoptions.CommandLineOptionTypeList, tsoptions.CommandLineOptionTypeListOrElement:
		...
		if len(errors) > 0 {
			if option.Name == "lib" {
				// The TS libraries are removed; tests naming one (vendored
				// fourslash tests mostly) skip like tests naming removed options.
				t.Skipf("lib value '%s' is removed", value)
			}
...
func (c *CompilationResult) getOutputPath(path string, ext string) string {
	...
	var outDir string
	if ext == tspath.ExtensionDts || (strings.HasSuffix(ext, tspath.ExtensionTs) && strings.Contains(ext, ".d.")) {
