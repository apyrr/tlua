...
func iterateErrorBaseline[T diagnosticwriter.Diagnostic](t *testing.T, inputFiles []*harnessutil.TestFile, inputDiagnostics []T, compareDiagnostics func(a, b T) int, pretty bool) []string {
	...
	for _, inputFile := range inputFiles {
		...
		for lineIndex, line := range lines {
			...
			for _, errDiagnostic := range fileErrors {
				...
				if end >= thisLineStart && (errStart < nextLineStart || lineIndex == len(lines)-1) {
					...
					// Calculate the start of the squiggle. Clamp to the line
					// length: a diagnostic can start past this line's end when
					// the source uses U+2028/U+2029 separators (which split the
					// text into short lines while positions run over the whole
					// file), which would otherwise slice out of range below.
					squiggleStart := min(max(0, relativeOffset), len(line))
		...
		sanitizedFileName := sanitizeTestFilePath(inputFile.UnitName)
		_, isDupe := dupeCase[sanitizedFileName]
		// Port of upstream checkDuplicatedFileName: record this file so a later
		// occurrence of the same (sanitized) name is recognized as a duplicate. Without
		// this, dupeCase stays empty and duplicate-filename errors are double-counted.
		if count, ok := dupeCase[sanitizedFileName]; ok {
			dupeCase[sanitizedFileName] = count + 1
		} else {
			dupeCase[sanitizedFileName] = 0
		}
		result = append(result, outputLines.String())
		if isDupe {
	...
	// Diagnostics in referenced library fixtures (e.g. tests/lib/react16.d.ts) that are
	// neither test inputs, default libs, nor tsconfig files. They are emitted in the
	// top-diagnostics summary above but are never walked per-file (their file is not an
	// input file), so they are not otherwise counted. tlua removes TypeScript features
	// (e.g. `class`) that such fixtures use, so an included fixture can now legitimately
	// contain errors — account for them here rather than assuming fixtures never error.
	numExternalFixtureDiagnostics := core.CountWhere(
		diagnostics,
		func(d T) bool {
			if d.File() == nil {
				return false
			...
			if isDefaultLibraryFile(fileName) || isBuiltFile(fileName) || isTsConfigFile(fileName) {
				return false
			}
			return !slices.ContainsFunc(inputFiles, func(f *harnessutil.TestFile) bool {
				return tspath.ComparePaths(
					removeTestPathPrefixes(fileName, false),
					removeTestPathPrefixes(f.UnitName, false),
	...
	assert.Check(t, cmp.Equal(totalErrorsReportedInNonLibraryNonTsconfigFiles+numLibraryDiagnostics+numTsconfigDiagnostics+numExternalFixtureDiagnostics, len(diagnostics)), "total number of errors")
