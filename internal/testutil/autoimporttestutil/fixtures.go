...
func (b *fileMapBuilder) AddNamedNodeModulesPackage(nodeModulesDir string, name string) NodeModulesPackageHandle {
	...
	// The package entrypoint is a .tlua module chunk: declaration files stay
	// global scripts in tlua, so a package that exposes module exports must
	// point "types" at a source file with a top-level return.
	packageJSONContent := fmt.Sprintf(`{"name":"%s","types":"index.tlua"}`, resolvedName)
	b.files[packageJSONPath] = packageJSONContent
	declarationPath := tspath.CombinePaths(pkgDir, "index.tlua")
	declarationContent := fmt.Sprintf("local %s: number = 1;\nreturn { %s = %s };\n", exportName, exportName, exportName)
...
func (b *fileMapBuilder) AddLocalProject(projectDir string, fileCount int) {
	...
	for i := 1; i <= fileCount; i++ {
		path := tspath.CombinePaths(dir, fmt.Sprintf("file%d.tlua", i))
		exportName := fmt.Sprintf("localExport%d_%d", b.nextProjectID, i)
		// A tlua module exports via its final top-level return statement.
		content := fmt.Sprintf("local %s = %d;\nreturn { %s = %s };\n", exportName, i, exportName, exportName)
