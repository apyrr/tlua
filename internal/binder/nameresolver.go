...
loop:
	for location != nil {
		...
		if ast.IsModuleDeclaration(location) && lastLocation != nil && location.Name() == lastLocation {
			// If lastLocation is the name of a namespace, skip the parent since it has its own locals that could
			// conflict.
			lastLocation = location
			location = location.Parent
		}
		result = ast.LookupLuaLocal(location, name, originalLocation, meaning)
		if result != nil {
			break loop
		}
