...
const (
	...
	ContainerFlagsPropagatesSelfType     ContainerFlags = 1 << 8
)
...
// it, nested blocks included, so entries are pushed for a statement list before
// any of it binds and popped after. Nearest match wins, so an inner block's
// label shadows an outer one of the same name.
type ActiveLabel struct {
	next *ActiveLabel
	// The join point every `goto name` flows into, along with the fall-through
	// that reaches the label statement. A loop label, because a goto below the
	// label adds its antecedent after the label itself has been bound.
	gotoTarget *ast.FlowLabel
	labelNode  *ast.Node
	name       string
	referenced bool
}
...
func (b *Binder) getDeclarationName(node *ast.Node) string {
	...
	if name != nil {
		if ast.IsAmbientModule(node) {
			// Only `declare global` survives; string-named ambient modules
			// (whose symbols were named "\"name\"") are deleted.
...
func (b *Binder) bindModuleDeclaration(node *ast.Node) {
	...
	// Every surviving ModuleDeclaration is a `declare global` augmentation
	// (string-named ambient modules are deleted from the grammar).
	if ast.IsAmbientModule(node) {
...
func (b *Binder) bindVariableDeclarationOrBindingElement(node *ast.Node) {
	...
	if name := node.Name(); name != nil && !ast.IsBindingPattern(name) {
		switch {
		case ast.IsLuaLocal(node):
			// Generic-for names are excluded by IsLuaLocal itself — they
			// bind block-scoped (Let) into the loop's Locals below.
			b.bindLuaLocalDeclaration(node)
...
func (b *Binder) bindLuaLocalDeclaration(node *ast.Node) {
	name := b.getDeclarationName(node)
	symbol := b.newSymbol(ast.SymbolFlagsNone, name)
	table := ast.GetLuaLocals(b.blockScopeContainer)
	table[name] = append(table[name], symbol)
	b.addDeclarationToSymbol(symbol, node, ast.SymbolFlagsBlockScopedVariable)
}

func (b *Binder) bindLuaLocalFunctionDeclaration(node *ast.Node) {
	name := b.getDeclarationName(node)
	symbol := b.newSymbol(ast.SymbolFlagsNone, name)
	table := ast.GetLuaLocals(b.blockScopeContainer)
	table[name] = append(table[name], symbol)
	b.addDeclarationToSymbol(symbol, node, ast.SymbolFlagsBlockScopedVariable|ast.SymbolFlagsFunction)
}
...
func (b *Binder) bindParameter(node *ast.Node) {
	...
	if ast.IsVarargParameter(node) {
		// The vararg has no binding name -- its synthetic `...` identifier exists
		// only so the parameter owns a symbol. Bind it anonymously: nothing can
		// spell the name, so an entry in the enclosing symbol table could never
		// be resolved and would only force scope walkers to filter it out (and a
		// second `...`, already reported as a misplaced rest, would collide with
		// the first and report "Duplicate identifier" against a name the user
		// never wrote). Signatures reach the symbol through the declaration.
		b.bindAnonymousDeclaration(node, ast.SymbolFlagsFunctionScopedVariable, ast.VarargParameterName)
	} else if ast.IsBindingPattern(decl.Name()) {
...
func (b *Binder) bindAnonymousDeclaration(node *ast.Node, symbolFlags ast.SymbolFlags, name string) {
	...
	if symbolFlags&ast.SymbolFlagsClassMember != 0 {
...
// check for reserved words used as identifiers in strict mode code. (`await` is an ordinary
// identifier in tlua: await expressions and their parsing contexts are removed.)
func (b *Binder) checkContextualIdentifier(node *ast.Node) {
	...
	if len(b.file.Diagnostics()) == 0 && node.Flags&ast.NodeFlagsAmbient == 0 && node.Flags&ast.NodeFlagsJSDoc == 0 && !ast.IsIdentifierName(node) {
		...
		// `yield` is an ordinary identifier in tlua (generators are removed), so
		// it is excluded from the strict-mode reserved words even though modules
		// are always strict — otherwise `local yield` would never be usable.
		if originalKeywordKind >= ast.KindFirstFutureReservedWord && originalKeywordKind <= ast.KindLastFutureReservedWord && originalKeywordKind != ast.KindYieldKeyword {
...
func (b *Binder) bindContainer(node *ast.Node, containerFlags ContainerFlags) {
	...
	if containerFlags&ContainerFlagsIsControlFlowContainer != 0 {
		...
		b.seenSelfType = false
		b.bindChildren(node)
		// Reset flags (for incremental scenarios)
		node.Flags &^= ast.NodeFlagsReachabilityAndEmitFlags | ast.NodeFlagsContainsSelf
		...
		if b.seenSelfType {
			node.Flags |= ast.NodeFlagsContainsSelf
		}
		...
		if containerFlags&ContainerFlagsPropagatesSelfType != 0 {
			b.seenSelfType = saveSeenSelfType || b.seenSelfType
		} else {
			b.seenSelfType = saveSeenSelfType
		}
	} else if containerFlags&ContainerFlagsIsInterface != 0 {
		saveSeenSelfType := b.seenSelfType
		b.seenSelfType = false
		b.bindChildren(node)
		// ContainsSelf cannot overlap with HasExtendedUnicodeEscape on Identifier.
		if b.seenSelfType {
			node.Flags |= ast.NodeFlagsContainsSelf
		} else {
			node.Flags &^= ast.NodeFlagsContainsSelf
...
func (b *Binder) bindChildren(node *ast.Node) {
	...
	// A label is a jump target, so it binds its flow even when the fall-through
	// that reaches it is dead: a goto elsewhere may still jump to it. Its flow
	// node is the join it establishes, not the incoming flow, so the label is
	// reachable exactly when some goto or the fall-through reaches it.
	if node.Kind == ast.KindLabelStatement {
		b.bindLabelStatement(node)
	...
	if b.currentFlow == b.unreachableFlow {
		...
		// A goto in dead code wires no flow edge, but it still names its label, so
		// mark the target referenced — otherwise a label whose only goto is
		// unreachable would be reported as unused.
		if node.Kind == ast.KindGotoStatement {
			if active := b.findActiveLabel(node.AsGotoStatement().Label.Text()); active != nil {
				active.referenced = true
			}
		}
		// A dead block still scopes its labels: pre-scan them so a label or goto
		// inside it gets the same bookkeeping (unused flags, goto references) as
		// dead statements in a live block.
		if statements := ast.StatementsOfLabelContainer(node); statements != nil {
			saveActiveLabelList := b.activeLabelList
			declared := b.declareLabels(statements)
	...
	case ast.KindNumericForStatement:
		b.bindNumericForStatement(node)
	case ast.KindRepeatStatement:
		b.bindRepeatStatement(node)
	...
	case ast.KindObjectLiteralExpression, ast.KindArrayLiteralExpression, ast.KindPropertyAssignment:
		b.inAssignmentPattern = saveInAssignmentPattern
		b.bindEachChild(node)
	default:
		b.bindEachChild(node)
...
// including one that appears before the label it targets — resolves. Duplicates
// keep the first entry; the checker reports them.
func (b *Binder) declareLabels(statements []*ast.Node) []*ActiveLabel {
	var declared []*ActiveLabel
	for _, node := range statements {
		if node.Kind != ast.KindLabelStatement {
			continue
		}
		label := node.AsLabelStatement().Label
		// A label from an enclosing block may be shadowed, so only this block's
		// own entries collide.
		if slices.ContainsFunc(declared, func(a *ActiveLabel) bool { return a.name == label.Text() }) {
			continue
		}
		active := &ActiveLabel{
			next:       b.activeLabelList,
			gotoTarget: b.createLoopLabel(),
			labelNode:  label,
			name:       label.Text(),
		}
		b.activeLabelList = active
		declared = append(declared, active)
	}
	return declared
}

// Pops the block's labels and flags the ones no goto targeted; the checker
// decides whether an unused label is worth reporting.
func (b *Binder) finishLabels(declared []*ActiveLabel, saved *ActiveLabel) {
	for _, label := range declared {
		if !label.referenced {
			label.labelNode.Flags |= ast.NodeFlagsUnreachable
		}
	}
	b.activeLabelList = saved
}
...
func (b *Binder) bindWhileStatement(node *ast.Node) {
	...
	preWhileLabel := b.createLoopLabel()
...
// the until-condition flows correctly both after the loop and back into it.
func (b *Binder) bindRepeatStatement(node *ast.Node) {
	stmt := node.AsRepeatStatement()
	preRepeatLabel := b.createLoopLabel()
	preConditionLabel := b.createBranchLabel()
	postRepeatLabel := b.createBranchLabel()
	b.addAntecedent(preRepeatLabel, b.currentFlow)
	b.currentFlow = preRepeatLabel
	saveBreakTarget := b.currentBreakTarget
	saveContinueTarget := b.currentContinueTarget
	...
	b.bindCondition(stmt.Expression, postRepeatLabel /*trueTarget: exit*/, preRepeatLabel /*falseTarget: loop*/)
	b.currentFlow = b.finishFlowLabel(postRepeatLabel)
}

func (b *Binder) bindForOfStatement(node *ast.Node) {
	stmt := node.AsForOfStatement()
...
// bindNumericForStatement binds a Lua `for i = from, to[, step] do ... end`.
// The bounds are evaluated once before the loop; the control variable's
// assignment happens each iteration, so its FlowAssignment lives inside the
// loop-label cycle. There is no user condition — the body may run zero times,
// so the zero-iteration exit edge is added before the initializer (the shape
// of bindForOfStatement, whose generic-for can also run zero times).
func (b *Binder) bindNumericForStatement(node *ast.Node) {
	stmt := node.AsNumericForStatement()
	preLoopLabel := b.createLoopLabel()
	postLoopLabel := b.createBranchLabel()
	b.bind(stmt.From)
	b.bind(stmt.To)
...
func (b *Binder) bindBreakStatement(node *ast.Node) {
	b.bindJumpFlow(b.currentBreakTarget)
}

func (b *Binder) bindContinueStatement(node *ast.Node) {
	b.bindJumpFlow(b.currentContinueTarget)
}
...
// Wires an unconditional jump: the target gains this flow as an antecedent, and
// whatever follows the jump is unreachable. Shared by break, continue and goto.
func (b *Binder) bindJumpFlow(flowLabel *ast.FlowLabel) {
	if flowLabel != nil {
...
// The label is a join point: fall-through reaches it, and so does every goto
// that targets it. Its flow label is left open (not finished) because a goto
// below the label still has to add its back-edge, exactly as a loop's
// pre-condition label stays open across the body.
func (b *Binder) bindLabelStatement(node *ast.Node) {
	stmt := node.AsLabelStatement()
	b.bind(stmt.Label)
	active := b.findActiveLabel(stmt.Label.Text())
	if active == nil || active.labelNode != stmt.Label {
		// Not the statement that declared the active entry: an embedded label
		// (never pre-scanned) or a same-block duplicate. The checker reports
		// both; joining the same-named label's flow here would wire a jump edge
		// no goto wrote, so leave the flow alone.
		return
	}
	// The label's gotoTarget is a loop label, and the reachability/narrowing
	// walkers read only its first antecedent as the loop's non-looping entry
	// (back-edges are the rest). A goto label has potentially MANY non-looping
	// entries — the fall-through plus every forward goto, all already bound
	// since gotos bind before the label they target — so slot 0 must join them
	// all: a branch label, which the walkers check antecedent-by-antecedent.
	// When nothing live enters (no forward goto, dead fall-through), seed the
	// dead fall-through directly, past addAntecedent's unreachable skip;
	// otherwise a later backward goto would make the loop label its own only
	// antecedent, an infinite cycle. The cost is that a label revived only by
	// a backward goto reports the code after it as unreachable — the same
	// conservatism a loop whose entry is dead has.
	if forward := active.gotoTarget.Antecedents; forward == nil {
		if b.currentFlow.Flags&ast.FlowFlagsUnreachable != 0 {
			active.gotoTarget.Antecedents = b.newFlowList(b.currentFlow, nil)
		} else {
			b.addAntecedent(active.gotoTarget, b.currentFlow)
		}
	} else {
		entry := b.createBranchLabel()
		entry.Antecedents = forward
		b.addAntecedent(entry, b.currentFlow)
		setFlowNodeReferenced(entry)
		active.gotoTarget.Antecedents = b.newFlowList(entry, nil)
	}
	b.currentFlow = active.gotoTarget
}

// An unresolved goto contributes no flow edge; the checker reports it. Leaving
// the flow untouched keeps the code after it reachable, so one bad goto does
// not cascade into unreachable-code errors.
func (b *Binder) bindGotoStatement(node *ast.Node) {
	stmt := node.AsGotoStatement()
	b.bind(stmt.Label)
	if active := b.findActiveLabel(stmt.Label.Text()); active != nil {
		active.referenced = true
		b.bindJumpFlow(active.gotoTarget)
	}
}
...
func (b *Binder) bindVariableDeclarationFlow(node *ast.Node) {
	...
	// list even though only the last declaration holds the initializer.
	if node.Initializer() != nil || ast.IsForOfStatement(node.Parent.Parent) || node.Parent.Parent.Kind == ast.KindNumericForStatement || ast.HasLuaLocalValueList(node) {
...
func GetContainerFlags(node *ast.Node) ContainerFlags {
	...
	case ast.KindArrowFunction:
		return ContainerFlagsIsContainer | ContainerFlagsIsControlFlowContainer | ContainerFlagsHasLocals | ContainerFlagsIsFunctionLike | ContainerFlagsIsFunctionExpression | ContainerFlagsPropagatesSelfType
