...
func (r *referenceResolver) GetReferencedExportContainer(node *ast.IdentifierNode, prefixLocals bool) *ast.Node /*SourceFile|ModuleDeclaration*/ {
	// When resolving the export for the name of a module
	// declaration, we need to start resolution at the declaration's container.
	// Otherwise, we could incorrectly resolve the export as the
	// declaration if it contains an exported member with the same name.
	startInDeclarationContainer := node.Parent != nil && node.Parent.Kind == ast.KindModuleDeclaration && node == node.Parent.Name()
	if symbol := r.getReferencedValueSymbol(node, startInDeclarationContainer); symbol != nil {
		...
		if parentSymbol != nil {
			...
			isMatchingContainer := func(n *ast.Node) bool {
				return n.Kind == ast.KindModuleDeclaration && r.getSymbolOfDeclaration(n) == parentSymbol
			}
