...
func getLanguageKind(filename string) lsproto.LanguageKind {
	if tspath.FileExtensionIsOneOf(filename, []string{tspath.ExtensionTs, tspath.ExtensionDts}) {
...
func isLibFile(fileName string) bool {
	...
	if strings.HasPrefix(baseName, "lib.") && strings.HasSuffix(baseName, tspath.ExtensionDts) {
		return true
	}
	return false
}
