...
local p1: IPromise<number> = null;
p/*1*/1.then(function (x/*2*/x) end); 
local p/*3*/2 = p1.then(function (x/*4*/x) return "hello"; end)
local p/*5*/3 = p2.then(function (x/*6*/x)
    return x/*7*/x;
end);`
	f, done := fourslash.NewFourslash(t, nil /*capabilities*/, content)
	defer done()
	f.VerifyQuickInfoAt(t, "1", "local p1: IPromise<number>", "")
	f.VerifyQuickInfoAt(t, "2", "(parameter) xx: number", "")
	f.VerifyQuickInfoAt(t, "3", "local p2: IPromise<string>", "")
	f.VerifyQuickInfoAt(t, "4", "(parameter) xx: number", "")
	f.VerifyQuickInfoAt(t, "5", "local p3: IPromise<string>", "")
