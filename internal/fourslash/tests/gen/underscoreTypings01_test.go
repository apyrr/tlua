// Code generated by convertFourslash; DO NOT EDIT.
// To modify this test, run "npm run makemanual underscoreTypings01"

package fourslash_test

import (
	"testing"

	"github.com/apyrr/tlua/internal/fourslash"
	. "github.com/apyrr/tlua/internal/fourslash/tests/util"
	"github.com/apyrr/tlua/internal/testutil"
)

func TestUnderscoreTypings01(t *testing.T) {
	fourslash.SkipIfFailing(t)
	t.Parallel()
	defer testutil.RecoverAndFail(t, "Panic on fourslash test")
	const content = `interface Iterator_<T, U> {
    (value: T, index: any, list: any): U;
}

interface WrappedArray<T> {
    map<U>(iterator: Iterator_<T, U>, context?: any): U[];
}

interface Underscore {
    <T>(list: T[]): WrappedArray<T>;
    map<T, U>(list: T[], iterator: Iterator_<T, U>, context?: any): U[];
}

declare _: Underscore;

local a: string[];
local /*1*/b = _.map(a, /*2*/x => x.length);    // Was typed any[], should be number[]
local /*3*/c = _(a).map(/*4*/x => x.length);
local /*5*/d = a.map(/*6*/x => x.length);

local aa: any[];
local /*7*/bb = _.map(aa, /*8*/x => x.length);
local /*9*/cc = _(aa).map(/*10*/x => x.length);
local /*11*/dd = aa.map(/*12*/x => x.length);

local e = a.map(x => x./*13*/`
	f, done := fourslash.NewFourslash(t, nil /*capabilities*/, content)
	defer done()
	f.VerifyQuickInfoAt(t, "1", "local b: number[]", "")
	f.VerifyQuickInfoAt(t, "2", "(parameter) x: string", "")
	f.VerifyQuickInfoAt(t, "3", "local c: number[]", "")
	f.VerifyQuickInfoAt(t, "4", "(parameter) x: string", "")
	f.VerifyQuickInfoAt(t, "5", "local d: number[]", "")
	f.VerifyQuickInfoAt(t, "6", "(parameter) x: string", "")
	f.VerifyQuickInfoAt(t, "7", "local bb: any[]", "")
	f.VerifyQuickInfoAt(t, "8", "(parameter) x: any", "")
	f.VerifyQuickInfoAt(t, "9", "local cc: any[]", "")
	f.VerifyQuickInfoAt(t, "10", "(parameter) x: any", "")
	f.VerifyQuickInfoAt(t, "11", "local dd: any[]", "")
	f.VerifyQuickInfoAt(t, "12", "(parameter) x: any", "")
	f.VerifyCompletions(t, "13", &fourslash.CompletionsExpectedList{
		IsIncomplete: false,
		ItemDefaults: &fourslash.CompletionsExpectedItemDefaults{
			CommitCharacters: &DefaultCommitCharacters,
			EditRange:        Ignored,
		},
		Items: &fourslash.CompletionsExpectedItems{
			Includes: []fourslash.CompletionsExpectedItem{
				"length",
			},
			Excludes: []string{
				"toFixed",
			},
		},
	})
}
