...
func TestFormattingOnInvalidCodes(t *testing.T) {
	...
	const content = `/*1*/local a;local c          , b;local  $d
/*2*/local $e
/*3*/local f
/*4*/a++;b++;
...
/*85*/    } ) ;`
	...
	f.FormatDocument(t, "")
	f.GoToMarker(t, "1")
	f.VerifyCurrentLineContent(t, `local a; local c, b; local $d`)
	f.GoToMarker(t, "2")
	f.VerifyCurrentLineContent(t, `local $e`)
	f.GoToMarker(t, "3")
	f.VerifyCurrentLineContent(t, `local f`)
	f.GoToMarker(t, "4")
