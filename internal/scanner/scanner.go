...
var textToKeyword = map[string]ast.Kind{
	...
	// tlua spells the logical operators the Lua way. `and`/`or`/`not` are
	// aliases of `&&`/`||`/`!`, scanning to the same token kinds, so precedence,
	// parsing, narrowing and emit are shared and the two spellings can never
	// drift apart. Being keywords, they are also reserved: `local and = 1` and
	// `x.and` no longer parse, exactly as in Lua.
	...
	"end":         ast.KindEndKeyword,
	// tlua removes TypeScript's module syntax: a chunk requires what it needs
	// and returns its value, so `export`, `default`, `from` and `require` are
	// ordinary identifiers. `import` stays reserved for the `import("./m")`
...
// TokenIsWordOperator reports whether the current token is an operator spelled
// as a word (`and`, `or`, `not`) rather than as punctuation. The two spellings
// share a token kind, but only the punctuation `!` is the non-null and
// definite-assignment operator, so those parser sites must ask.
func (s *Scanner) TokenIsWordOperator() bool {
	return s.tokenFlags&ast.TokenFlagsWordOperator != 0
}
...
func (s *Scanner) Scan() ast.Kind {
	...
	for {
		...
		case '!':
			if s.charAt(1) == '=' {
				// tlua accepts both `!=` and `~=` for not-equal. `!==` is not a
				// tlua operator; reject it and recover as the not-equal token.
				if s.charAt(2) == '=' {
					s.errorAt(diagnostics.X_0_is_not_a_valid_tlua_operator_Use_1_instead, s.pos, 3, "!==", "~=")
					s.pos += 3
				} else {
					s.pos += 2
				}
				s.token = ast.KindTildeEqualsToken
			} else {
				s.pos++
		...
		case '-':
			...
			if next == '=' {
				...
				if isLong {
					s.pos = contentStart
					commentClosed := s.scanLuaLongComment(level)
					if !commentClosed {
						s.tokenFlags |= ast.TokenFlagsUnterminated
						s.errorAt(diagnostics.X_0_expected, s.pos, 0, luaLongBracketClose(level))
					}
					if s.skipTrivia {
						continue
					}
				...
				// `--[` is a line comment unless the bracket begins a valid Lua
				// long-bracket opener, so malformed openers are ordinary text.
				for {
					s.scanASCIIWhile(func(b byte) bool {
						return b != '\n' && b != '\r'
					})
		...
		case '1', '2', '3', '4', '5', '6', '7', '8', '9':
			s.token = s.scanNumber()
		case ':':
			// `::` delimits a Lua label.
			if s.charAt(1) == ':' {
				s.pos += 2
				s.token = ast.KindColonColonToken
			} else {
				s.pos++
		...
		case '=':
			...
			if s.charAt(1) == '=' {
				// Lua equality is `==` (strict, no coercion). `===` is not a tlua
				// operator; reject it and recover as the equality token.
				if s.charAt(2) == '=' {
					s.errorAt(diagnostics.X_0_is_not_a_valid_tlua_operator_Use_1_instead, s.pos, 3, "===", "==")
					s.pos += 3
				} else {
		...
		case '^':
			s.pos++
			// TODO: Lua's `^` reuses the exponentiation token kind, whose name still
			// spells `**`. Kept for consistency with the other Lua operators that reuse
			// TS token kinds (`and`/`or`/`not`); revisit only as a project-wide decision
			// to name SyntaxKinds for Lua surface syntax rather than the inherited chars.
		...
		case '~':
			if s.charAt(1) == '=' {
				// Lua not-equal `~=` — the canonical not-equal operator.
				s.pos += 2
				s.token = ast.KindTildeEqualsToken
			} else {
				// Standalone `~` has no LuaJIT expression meaning.
		...
		case '#':
			...
			// Lua length operator. (tlua has no private identifiers; the normal
			// scanner never produces PrivateIdentifier.)
			s.pos++
...
// luaLongBracketStart recognizes '[' '='* '[' and returns the delimiter level
// and the first byte of its contents. Lua comments only call it after '--'.
func luaLongBracketStart(text string, pos int) (level int, contentStart int, ok bool) {
	...
	return level, pos + 1, true
}

func luaLongBracketClose(level int) string {
	return "]" + strings.Repeat("=", level) + "]"
}

func scanLuaLongBracketEnd(text string, pos int, level int) (end int, closed bool) {
	closer := luaLongBracketClose(level)
	if offset := strings.Index(text[pos:], closer); offset >= 0 {
		return pos + offset + len(closer), true
	}
	return len(text), false
}

func (s *Scanner) scanLuaLongComment(level int) bool {
	closer := luaLongBracketClose(level)
	for s.pos < s.end {
		if strings.HasPrefix(s.text[s.pos:], closer) {
			s.pos += len(closer)
			return true
		}
		ch, size := s.charAndSize()
...
// and returns KindNumericLiteral. tlua has no bigint, so there is no `n` suffix.
func (s *Scanner) scanNumericValue() ast.Kind {
...
// isWordOperatorKind reports whether kind is one a word-spelled operator
// resolves to. The word operators are the only names in textToKeyword that map
// to a punctuation kind rather than a keyword kind; every other lookup yields a
// keyword, KindIdentifier (GetIdentifierToken's miss) or KindUnknown (the raw
// map's miss). This is the single definition of the property: parser gating
// (TokenIsWordOperator) and emit gating (IsBareWritableName) must agree on it.
func isWordOperatorKind(kind ast.Kind) bool {
	return kind != ast.KindUnknown && kind != ast.KindIdentifier && !ast.IsKeywordKind(kind)
}

// identifierToken resolves just-scanned identifier text to its token kind. A
// word-spelled operator (`and`, `or`, `not`) scans to the same kind as its
...
// IsWordOperatorText reports whether text is one of tlua's word-spelled
// operators: `and`, `or` and `not`. They scan to punctuation kinds rather than
// keyword kinds, so unlike an ordinary keyword such as `while` they are legal
// in no identifier or member-name position: `t.and` and `import { and }` do not
// parse, and a property named `and` must be written `t["and"]`.
func IsWordOperatorText(text string) bool {
	return isWordOperatorKind(textToKeyword[text])
}

func GetViableKeywordSuggestions() []string {
	result := make([]string, 0, len(textToKeyword))
	for text := range textToKeyword {
		// The word operators are punctuation, so they can never head the
		// statement this suggestion recovers, and offering them turns the typo
		// `notify foo` into "did you mean 'not ify'?".
		if len(text) > 2 && !IsWordOperatorText(text) {
			result = append(result, text)
		}
	}
	// textToKeyword is a map, so iteration order is nondeterministic; sort so keyword
	// suggestions (e.g. the "did you mean" space suggestion) are stable across runs.
	slices.Sort(result)
	return result
}
	...
	for {
		...
		case '-':
			if pos+1 < textLen && text[pos+1] == '-' {
				...
				pos += 2
				if level, contentStart, ok := luaLongBracketStart(text, pos); ok {
					pos, _ = scanLuaLongBracketEnd(text, contentStart, level)
				} else {
					for pos < textLen {
						ch, size := utf8.DecodeRuneInString(text[pos:])
						if stringutil.IsLineBreak(ch) {
...
Single-line comment ranges include their leading `//` or `--` but not the ending line
break. Multi-line comment ranges include their complete TypeScript or Lua delimiters.
*/
