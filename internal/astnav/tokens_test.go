...
import (
	...
	"github.com/apyrr/tlua/internal/testutil/jstest"
	"gotest.tools/v3/assert"
)

var testFiles = []string{
	filepath.Join(repo.TestDataPath(), "tests/cases/astnav/mapCode.tlua"),
}
...
func tsGetTokensAtPositions(t testing.TB, fileText string, positions []int) []*tokenInfo {
	dir := t.TempDir()
	err := os.WriteFile(filepath.Join(dir, "file.tlua"), []byte(fileText), 0o644)
	assert.NilError(t, err)
	...
	script := `
		...
		export default (ts) => {
			...
			const fileText = fs.readFileSync("file.tlua", "utf8");
