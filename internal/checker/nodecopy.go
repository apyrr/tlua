...
func (b *NodeBuilderImpl) reuseName(node *ast.Node, isMethod bool) *ast.Node {
	...
	// Numeric literal names (and computed names over numeric literals) denote
	// number keys; keep them as-is so the reused name rebinds to the same key.
	if ast.IsNumericLiteral(res) {
		return res
	}
	if ast.IsComputedPropertyName(res) && ast.IsNumericLiteral(res.Expression()) {
		return res
	}

	kind := classifyPropertyName(text, isMethod)
	...
	visitExistingNodeTreeSymbolsWorker := func(node *ast.Node) *ast.Node {
		...
		if node.Kind == ast.KindJSDocNullableType {
			unionMembers := []*ast.Node{
				...
				factory.NewLiteralTypeNode(factory.NewKeywordExpression(ast.KindNilKeyword)),
			}
		...
		if node.Kind == ast.KindJSDocOptionalType {
			unionMembers := []*ast.Node{
				...
				factory.NewKeywordTypeNode(ast.KindNilKeyword),
			}
			return factory.NewUnionTypeNode(factory.NewNodeList(unionMembers))
		}
