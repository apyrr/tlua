	...
	if target.flags&TypeFlagsTypeVariable != 0 {
		...
		if simplified != target {
			...
			if indexType.flags&TypeFlagsInstantiable != 0 {
				simplified := c.distributeIndexOverObjectType(c.getSimplifiedType(target.AsIndexedAccessType().objectType, false /*writing*/), indexType, target.AsIndexedAccessType().accessFlags, false /*writing*/)
						...
						if allTypeFlags&TypeFlagsString == 0 {
							...
							choose := func(left *Type, right *Type) *Type {
								...
								case right.flags&TypeFlagsBooleanLiteral != 0 && core.IfElse(getBooleanLiteralValue(right), "true", "false") == str:
									return right
								case left.flags&TypeFlagsNil != 0:
									return left
								case right.flags&TypeFlagsNil != 0 && right.AsIntrinsicType().intrinsicName == str:
									return right
								default:
									return left
								}
...
func (c *Checker) applyToReturnTypes(source *Signature, target *Signature, callback func(s *Type, t *Type)) {
	...
	if c.couldContainTypeVariables(targetReturnType) {
		...
		// Lua: a function's return is a value pack, so a plain single-value return
		// (`() => number`) or a `void` return relates to a generic pack target
		// `(...R)` as a one-element or empty pack. Adjust the source here so R infers
...
// packTargetSpreadsPackParameter reports whether t is a value pack with a variadic
// element that spreads a generic pack parameter, i.e. a `(...R)` / `(boolean, ...R)`
// return whose R is a `<...R>` parameter. Only then does a plain callee return need
// re-expressing as a pack (getReturnPackForInference); a fixed multi-return target
// infers unchanged.
func (c *Checker) packTargetSpreadsPackParameter(t *Type) bool {
	// The combined-flags bit test rejects the common fixed multi-return target
	// without resolving type arguments (every tlua multi-return is a pack, so this
	// runs for ordinary callbacks too).
	if !isPackType(t) || t.TargetTupleType().combinedFlags&ElementFlagsVariadic == 0 {
		return false
	}
	elementInfos := t.TargetTupleType().elementInfos
	elementTypes := c.getElementTypes(t)
	for i, info := range elementInfos {
		if info.flags&ElementFlagsVariadic != 0 && i < len(elementTypes) && isPackTypeParameter(elementTypes[i]) {
			return true
		}
	}
	return false
...
// nonPackReturnAsPack expresses a non-pack return type as the value pack Lua
// semantics give it: `void` is the empty pack (zero values), and any other single
// value is a one-element pack. It is the single definition of that adjustment —
// signature-return relating (compareSignaturesRelated) and pack-parameter inference
// (applyToReturnTypes) both use it, so the two cannot drift apart.
func (c *Checker) nonPackReturnAsPack(t *Type) *Type {
	if t.flags&TypeFlagsVoid != 0 {
		return c.createPackTypeEx(nil, nil, false /*collapse*/)
	}
...
func (c *Checker) inferFromIndexTypes(n *InferenceState, source *Type, target *Type) {
	...
	sourceIsInferable := c.isObjectTypeWithInferableIndex(source)
	for _, targetInfo := range indexInfos {
		if getInferenceInfoForType(n, targetInfo.keyType) != nil {
			// The key is itself an inference target, and nothing is assignable to an
			...
			if len(valueTypes) != 0 {
				c.inferWithPriority(n, c.getUnionType(valueTypes), targetInfo.valueType, priority)
			}
			continue
		}
		if sourceIsInferable {
...
func (c *Checker) inferToMappedType(n *InferenceState, source *Type, target *Type, constraintType *Type) bool {
	...
	if constraintType.flags&TypeFlagsTypeParameter != 0 {
		...
		indexTypes := core.Map(c.getIndexInfosOfType(source), func(info *IndexInfo) *Type { return info.valueType })
