...
import (
	...
	"github.com/apyrr/tlua/internal/pseudochecker"
...
func newNodeBuilderImpl(ch *Checker, e *printer.EmitContext, idToSymbol map[*ast.IdentifierNode]*ast.Symbol) *NodeBuilderImpl {
	...
	b := &NodeBuilderImpl{f: e.Factory.AsNodeFactory(), ch: ch, e: e, idToSymbol: idToSymbol, pc: pseudochecker.NewPseudoChecker(ch.exactOptionalPropertyTypes)}
...
func (b *NodeBuilderImpl) isExpandableType(t *Type, isAlias bool) bool {
	...
	if objectFlags&ObjectFlagsAnonymous != 0 && t.symbol != nil &&
		t.symbol.Flags&(ast.SymbolFlagsClass|ast.SymbolFlagsValueModule|ast.SymbolFlagsFunction|ast.SymbolFlagsMethod) != 0 {
		return true
	}
	return false
}
	...
	if ast.IsImportTypeNode(root) {
		...
		if b.ctx.flags&nodebuilder.FlagsUseInstantiationExpressions != 0 && typeRef.TypeArguments != nil && len(typeRef.TypeArguments.Nodes) != 0 {
			...
			for _, id := range getAccessStack(ref) {
				expr = b.f.NewPropertyAccessExpression(expr, nil, nil /*colonToken*/, id, ast.NodeFlagsNone)
			}
			return expr
		}
...
func (b *NodeBuilderImpl) symbolToNode(symbol *ast.Symbol, meaning ast.SymbolFlags) *ast.Node {
	if b.ctx.internalFlags&nodebuilder.InternalFlagsWriteComputedProps != 0 {
		...
		if b.ch.valueSymbolLinks.Has(symbol) {
			...
			if nameType != nil && nameType.flags&TypeFlagsUniqueESSymbol != 0 {
...
func (b *NodeBuilderImpl) createAccessFromSymbolChain(chain []*ast.Symbol, index int, stopper int, overrideTypeArguments *ast.NodeList) *ast.Node {
	...
	if index > stopper {
		...
		return b.createExpressionWithTypeArguments(b.f.NewPropertyAccessExpression(b.createAccessExpression(lhs), nil, nil /*colonToken*/, identifier, ast.NodeFlagsNone), typeParameterNodes)
	}
	return identifier
}
...
func (b *NodeBuilderImpl) createExpressionFromSymbolChain(chain []*ast.Symbol, index int) *ast.Expression {
	...
	if index == 0 || canUsePropertyAccess(symbolName) {
		...
		if index > 0 {
			result := b.f.NewPropertyAccessExpression(b.createExpressionFromSymbolChain(chain, index-1), nil, nil /*colonToken*/, identifier, ast.NodeFlagsNone)
...
// numberKeyDisplayText renders a number key's decimal value as it appears in a
// type-literal member position: a negative key is bracketed ("[-1]") because a
// bare "-1:" does not parse, while a non-negative key prints plainly.
func numberKeyDisplayText(value string) string {
	...
	if b.ch.valueSymbolLinks.Has(symbol) {
		...
		if nameType.flags&TypeFlagsStringOrNumberLiteral != 0 {
			...
			case string:
				...
				return v
			case jsnum.Number:
				return numberKeyDisplayText(v.String())
			}
		}
...
func (b *NodeBuilderImpl) getNameOfSymbolAsWritten(symbol *ast.Symbol) string {
	...
	// Decode number-key names for display (e.g. tuple element symbols).
...
func (b *NodeBuilderImpl) getSpecifierForModuleSymbol(symbol *ast.Symbol, overrideImportMode core.ResolutionMode) string {
	// String-named ambient modules are deleted, so every module symbol is
	// backed by a source file — but a file module symbol's name is still its
	// quoted, extension-less path, which is the preferred display form.
	if b.ctx.enclosingFile == nil {
		if strings.HasPrefix(symbol.Name, "\"") && strings.HasSuffix(symbol.Name, "\"") {
...
func (b *NodeBuilderImpl) typeParameterToDeclarationWithConstraint(typeParameter *Type, constraintNode *ast.TypeNode) *ast.TypeParameterDeclarationNode {
	...
	if typeParameter.AsTypeParameter().isPack {
		// A pack parameter prints as `...A`. Its implicit `(...any)` constraint is
		// not written source, so it is suppressed rather than shown as `extends`.
...
func (b *NodeBuilderImpl) createMappedTypeNodeFromType(t *Type) *ast.TypeNode {
	...
	if b.ctx.flags&nodebuilder.FlagsGenerateNamesForShadowedTypeParams != 0 && b.isHomomorphicMappedTypeWithNonHomomorphicInstantiation(mapped) {
		...
		return b.f.NewConditionalTypeNode(
			...
			b.f.NewInferTypeNode(b.f.NewTypeParameterDeclaration(nil, nil, newTypeVariable.AsTypeReferenceNode().TypeName.Clone(b.f), originalConstraintNode, nil, nil)),
			result,
		...
		return b.f.NewConditionalTypeNode(
			...
			b.f.NewInferTypeNode(b.f.NewTypeParameterDeclaration(nil, nil, newTypeVariable.AsTypeReferenceNode().TypeName.Clone(b.f), b.f.NewTypeOperatorNode(ast.KindKeyOfKeyword, b.typeToTypeNode(b.ch.getModifiersTypeFromMappedType(t))), nil, nil)),
			result,
			b.f.NewKeywordTypeNode(ast.KindNeverKeyword),
		)
	}
...
func (b *NodeBuilderImpl) typePredicateToTypePredicateNode(predicate *TypePredicate) *ast.Node {
	...
	if predicate.kind == TypePredicateKindAssertsIdentifier {
		assertsModifier = b.f.NewToken(ast.KindAssertsKeyword)
	}
	// `this`-based predicates are removed; the parameter name is always an
	// ordinary identifier.
...
func (b *NodeBuilderImpl) typeParameterToDeclaration(parameter *Type) *ast.Node {
	...
	// A pack parameter's `(...any)` constraint is implicit, never written source, so
	// there is no node to build (the callee would only discard it).
	if !isPackTypeParameter(parameter) {
...
func (b *NodeBuilderImpl) symbolToParameterDeclaration(parameterSymbol *ast.Symbol, preserveModifierFlags bool) *ast.Node {
	...
	// A Lua vararg prints as `...: E` -- the pack's element type, never the array
	// the symbol carries, and never with the synthetic name. Keyed on that name, so
	// every rest symbol the *checker* mints must carry it too (see
	// combineUnionOrIntersectionParameters, which would otherwise leak
	// `...args: string[]`: not tlua syntax, and the array where the element type
	// belongs). A named rest in source keeps printing as written -- it is the
	// rejected legacy form (100034), and its diagnostics should quote what the user
	// typed.
	if ast.IsVarargSymbolName(parameterSymbol.Name) {
		// Going through serializeTypeForDeclaration would re-derive the type from
		// the symbol and print the un-unwrapped array back out.
		name := b.f.NewIdentifier(ast.VarargParameterName)
		b.idToSymbol[name] = parameterSymbol
		// A generic pack vararg `...: A` carries the pack A itself, not an array, so
		// it prints as `...: A`. Every other vararg prints its element type.
		annotationType := parameterType
		if !isPackTypeParameter(parameterType) {
			annotationType = b.ch.getVarargElementTypeForDisplay(parameterType)
		}
		node := b.f.NewParameterDeclaration(nil, b.f.NewToken(ast.KindDotDotDotToken), name, nil, b.typeToTypeNode(annotationType), nil)
		b.ctx.approximateLength += len(ast.VarargParameterName) + 3
		return node
	}
...
func (b *NodeBuilderImpl) signatureToSignatureDeclarationHelper(signature *Signature, kind ast.Kind, options *SignatureToSignatureDeclarationOptions) *ast.Node {
	...
	case kind == ast.KindFunctionExpression:
		// TODO: assert name is Identifier
		node = b.f.NewFunctionExpression(modifierList, name, typeParamList, paramList, returnTypeNode, nil /*fullSignature*/, b.f.NewBlock(b.f.NewNodeList([]*ast.Node{}), false))
...
func (b *NodeBuilderImpl) indexInfoToObjectComputedNamesOrSignatureDeclaration(indexInfo *IndexInfo, printed map[*ast.Node]bool, typeNode *ast.TypeNode) []*ast.Node {
	if len(indexInfo.components) > 0 {
		...
		if allComponentComputedNamesSerializable {
			...
			newComponents := core.Filter(indexInfo.components, func(c *ast.Node) bool {
				...
				// -- and members already printed: a member whose union-typed key feeds
				// several indexes belongs to each, but names one member, not one per index.
				return !b.ch.hasLateBindableName(c) && !printed[c]
			})
			bailed := false
...
// classifyPropertyName classifies a STRING-key property name. Numeric-looking
// string names print as string literals ("1" stays distinct from the number
// key 1); number keys never reach here — they are built directly as numeric
// literal names.
func classifyPropertyName(name string, isMethod bool) propertyNameNodeKind {
...
func (b *NodeBuilderImpl) createPropertyNameNodeForIdentifierOrLiteral(name string, singleQuote bool, isMethod bool, symbol *ast.Symbol) *ast.Node {
	switch classifyPropertyName(name, isMethod) {
...
// createNumberKeyPropertyNameNode builds a property-name node for the number
// key whose canonical decimal text is value: a numeric literal, or a computed
// [-N] for negative keys. NaN/Infinity keys have no numeric-literal spelling
// and print as identifier names (accepted edge; Lua rejects such keys at
// runtime anyway).
func (b *NodeBuilderImpl) createNumberKeyPropertyNameNode(value string, symbol *ast.Symbol) *ast.Node {
	if strings.HasPrefix(value, "-") {
...
func (b *NodeBuilderImpl) getPropertyNameNodeForSymbol(symbol *ast.Symbol) *ast.Node {
	...
	fromNameType := b.getPropertyNameNodeForSymbolFromNameType(symbol, singleQuote, isMethod)
	if fromNameType != nil {
		return fromNameType
	}
	...
	return b.createPropertyNameNodeForIdentifierOrLiteral(name, singleQuote, isMethod, symbol)
}

// See getNameForSymbolFromNameType for a stringy equivalent
func (b *NodeBuilderImpl) getPropertyNameNodeForSymbolFromNameType(symbol *ast.Symbol, singleQuote bool, isMethod bool) *ast.Node {
	if !b.ch.valueSymbolLinks.Has(symbol) {
		return nil
	}
	...
	if nameType.flags&TypeFlagsStringOrNumberLiteral != 0 {
		...
		case jsnum.Number:
			// Number keys print as numeric literal names.
			return b.createNumberKeyPropertyNameNode(v.String(), symbol)
		case string:
			// String keys print quoted unless they are identifiers, so the
			// string key "1" stays distinct from the number key 1.
			return b.createPropertyNameNodeForIdentifierOrLiteral(v, singleQuote, isMethod, symbol)
		}
	}
	if nameType.flags&TypeFlagsUniqueESSymbol != 0 {
...
func (b *NodeBuilderImpl) conditionalTypeToTypeNode(_t *Type) *ast.TypeNode {
	...
	if b.ctx.flags&nodebuilder.FlagsGenerateNamesForShadowedTypeParams != 0 && t.root.isDistributive && t.checkType.flags&TypeFlagsTypeParameter == 0 {
		...
		syntheticExtendsNode := b.f.NewInferTypeNode(b.f.NewTypeParameterDeclaration(nil, nil, newId, nil, nil, nil))
...
func (b *NodeBuilderImpl) typeReferenceToTypeNode(t *Type) *ast.TypeNode {
	...
	if t.Target() == b.ch.luaTableType || t.Target() == b.ch.luaReadonlyTableType {
		// `T[]` denotes a number-key table; print it back as `T[]`. The
		// targets are symbol-less, so there is no `Array` name to write —
		// FlagsWriteArrayAsGenericType does not apply.
		elementType := b.typeToTypeNode(typeArguments[0])
		arrayType := b.f.NewArrayTypeNode(elementType)
		if t.Target() == b.ch.luaTableType {
			return arrayType
		} else {
			return b.f.NewTypeOperatorNode(ast.KindReadonlyKeyword, arrayType)
		}
	} else if t.Target().objectFlags&ObjectFlagsTuple != 0 {
		// A zero-value pack means "returns nothing" — display it as void
		// rather than as an empty tuple.
		if t.Target().AsTupleType().isPack && b.ch.getTypeReferenceArity(t) == 0 {
		...
		if len(typeArguments) > 0 {
			...
			if tupleConstituentNodes != nil {
				// A Lua multi-return pack displays as a bare comma-separated
				// list (`number, string`), never as a bracketed tuple; a
				...
				if t.Target().AsTupleType().isPack {
					for i := 0; i < len(tupleConstituentNodes.Nodes); i++ {
						...
						case flags&ElementFlagsVariable != 0:
							tupleConstituentNodes.Nodes[i] = b.f.NewRestTypeNode(tupleConstituentNodes.Nodes[i])
						case flags&ElementFlagsOptional != 0:
							tupleConstituentNodes.Nodes[i] = b.f.NewOptionalTypeNode(tupleConstituentNodes.Nodes[i])
						}
					}
					packTypeNode := b.f.NewMultiReturnTypeNode(tupleConstituentNodes)
					b.e.SetEmitFlags(packTypeNode, printer.EFSingleLine)
					return packTypeNode
				}
				for i := 0; i < len(tupleConstituentNodes.Nodes); i++ {
...
func (b *NodeBuilderImpl) typeToTypeNode(t *Type) *ast.TypeNode {
	...
	if t.flags&TypeFlagsNil != 0 {
		b.ctx.approximateLength += 9
		return b.f.NewKeywordTypeNode(ast.KindNilKeyword)
	}
	...
	if inTypeAlias == 0 && t.alias != nil && (b.ctx.flags&nodebuilder.FlagsUseAliasDefinedOutsideCurrentScope != 0 || b.ch.IsTypeSymbolAccessible(t.alias.Symbol(), b.ctx.enclosingDeclaration)) {
		...
		if !b.shouldExpandType(t, true /*isAlias*/) {
			...
			// The inert global Array prints honestly as `Array<T>`; only the
			// synthesized table target (handled in typeReferenceToTypeNode)
			// sugars to `T[]`.
	...
	// A metatable type prints as its flattened structural shape: the metatable pairing is a
	// checking-time fact with no denotable spelling, so it does not survive display or emit.
	if objectFlags&(ObjectFlagsAnonymous|ObjectFlagsMapped|ObjectFlagsMetatable) != 0 {
		debug.Assert(t.Flags()&TypeFlagsObject != 0)
...
func (b *NodeBuilderImpl) createAccessExpression(node *ast.Node) *ast.Expression {
	...
	case ast.IsQualifiedName(node):
		return b.f.NewPropertyAccessExpression(b.createAccessExpression(node.AsQualifiedName().Left), nil /*questionDotToken*/, nil /*colonToken*/, b.f.DeepCloneNode(node.AsQualifiedName().Right), ast.NodeFlagsNone)
