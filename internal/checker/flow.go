...
func (c *Checker) getTypeAtFlowAssignment(f *FlowState, flow *ast.FlowNode) FlowType {
	...
	if c.containsMatchingReference(f.reference, node) {
		...
		if ast.IsVariableDeclaration(node) && (ast.IsInJSFile(node) || ast.IsVarConst(node)) {
...
// the paired type -- the same replacement checkLuaMetatableCall gives the call expression's own
// value, so interning makes the two one. Returning FlowType{} continues flow analysis at the
// antecedent.
func (c *Checker) getTypeAtLuaSetmetatableCall(f *FlowState, flow *ast.FlowNode, kind luaMetatableCallKind) FlowType {
	node := flow.Node
	args := node.Arguments()
	if len(args) < 2 || !c.isMatchingReference(f.reference, c.getReferenceCandidate(args[0])) {
		return FlowType{}
	}
	// Checking the metatable argument can re-enter this very flow node through a loop's back edge
	// (e.g. `while c do setmetatable(t, t.mt) end`): checkExpressionCached clears the flow-loop
	// stack, so the ordinary loop guard does not fire. The sentinel resolves that cycle to "no
	// narrowing", the way getResolvedSignature's resolvingSignature does for signatures.
	if slices.Contains(c.luaSetmetatableFlowStack, node) {
		return FlowType{}
	}
	...
	flowType := c.getTypeAtFlowNode(f, flow.Antecedent)
	t := c.finalizeEvolvingArrayType(flowType.t)
	// Replacement, not intersection: getSetmetatableResultType unpairs t first and the paired
	// result is assignable to t by construction. A protected antecedent refuses a new metatable
	// (the expression path reports it), so leave its type alone -- unless this is
	// debug.setmetatable, which bypasses the protection and replaces like any other.
	if kind == luaMetatableCallSet && someType(t, c.isProtectedMetatableType) {
		return flowType
	}
	narrowedType := c.getSetmetatableResultType(t, metatableType)
	if narrowedType == nil || narrowedType == t {
		return flowType
	}
	return c.newFlowType(narrowedType, flowType.incomplete)
}
...
func (c *Checker) narrowTypeByTypePredicate(f *FlowState, t *Type, predicate *TypePredicate, callExpression *ast.Node, assumeTrue bool) *Type {
	...
	if predicate.t != nil && !(IsTypeAny(t) && (predicate.t == c.globalObjectType || predicate.t == c.globalFunctionType)) {
		...
		if predicateArgument != nil {
			...
			if c.optionalChainContainsReference(predicateArgument, f.reference) && (assumeTrue && !c.hasTypeFacts(predicate.t, TypeFactsEQUndefined) || !assumeTrue && everyType(predicate.t, c.IsNullableType)) {
...
func (c *Checker) narrowTypeByTruthiness(f *FlowState, t *Type, expr *ast.Node, assumeTrue bool) *Type {
	...
	if assumeTrue && c.optionalChainContainsReference(expr, f.reference) {
		t = c.getAdjustedTypeWithFacts(t, TypeFactsNEUndefinedOrNull)
	}
	access := c.getDiscriminantPropertyAccess(f, expr, t)
	if access != nil {
...
func (c *Checker) narrowTypeByCallExpression(f *FlowState, t *Type, callExpression *ast.Node, assumeTrue bool) *Type {
	...
	if c.hasMatchingArgument(callExpression, f.reference) {
		...
		if predicate != nil && predicate.kind == TypePredicateKindIdentifier {
...
func (c *Checker) narrowTypeByEquality(t *Type, operator ast.Kind, value *ast.Node, assumeTrue bool) *Type {
	...
	// tlua equality is strict (`==`/`~=`); the loose `==`/`!=` with coercion and
	// the null/undefined double-match do not exist, so there is no `doubleEquals`
	// path — every comparison narrows strictly.
	if operator == ast.KindTildeEqualsToken {
		assumeTrue = !assumeTrue
	}
	valueType := c.getTypeOfExpression(value)
	if valueType.flags&TypeFlagsNullable != 0 {
		return c.getAdjustedTypeWithFacts(t, core.IfElse(assumeTrue, TypeFactsEQUndefined, TypeFactsNEUndefined))
	}
	if assumeTrue {
		...
		filteredType := c.filterType(t, func(t *Type) bool {
			return c.areTypesComparable(t, valueType)
		})
...
// narrowTypeByLuaTypeGuard narrows by an `==`/`~=` comparison of a `type(x)` or
// `io.type(x)` call against a tag literal.
func (c *Checker) narrowTypeByLuaTypeGuard(f *FlowState, t *Type, guard luaGuardKind, argument *ast.Node, operator ast.Kind, literal *ast.Node, assumeTrue bool) *Type {
	if operator == ast.KindTildeEqualsToken {
		assumeTrue = !assumeTrue
	}
	target := c.getReferenceCandidate(argument)
	if !c.isMatchingReference(f.reference, target) {
		// Guarding `x?.y` on any result other than the one a nil argument produces
		// implies that x is present.
		if c.optionalChainContainsReference(target, f.reference) && assumeTrue != isLuaAbsentGuardResult(guard, literal) {
			t = c.getAdjustedTypeWithFacts(t, TypeFactsNEUndefinedOrNull)
		}
		...
		if propertyAccess != nil {
			return c.narrowTypeByDiscriminant(t, propertyAccess, func(t *Type) *Type {
				return c.narrowTypeByLuaGuardLiteral(t, guard, literal, assumeTrue)
			})
		}
		return t
	}
...
func (c *Checker) narrowTypeByDiscriminantProperty(t *Type, access *ast.Node, operator ast.Kind, value *ast.Node, assumeTrue bool) *Type {
	if (operator == ast.KindEqualsEqualsToken || operator == ast.KindTildeEqualsToken) && t.flags&TypeFlagsUnion != 0 {
		...
		if keyPropertyName != "" {
			if accessedName, ok := c.getAccessedPropertyName(access); ok && keyPropertyName == accessedName {
				...
				if candidate != nil {
					if assumeTrue && operator == ast.KindEqualsEqualsToken || !assumeTrue && operator == ast.KindTildeEqualsToken {
						return candidate
					}
...
func (c *Checker) narrowTypeByDiscriminant(t *Type, access *ast.Node, narrowType func(t *Type) *Type) *Type {
	...
	removeNullable := (optionalChain || isNonNullAccess(access)) && c.maybeTypeOfKind(t, TypeFlagsNullable)
	nonNullType := t
	if removeNullable {
...
func (c *Checker) narrowTypeByBooleanComparison(f *FlowState, t *Type, expr *ast.Node, boolValue *ast.Node, operator ast.Kind, assumeTrue bool) *Type {
	assumeTrue = (assumeTrue != (boolValue.Kind == ast.KindTrueKeyword)) != (operator != ast.KindTildeEqualsToken)
...
func (c *Checker) getNarrowedTypeEx(t *Type, candidate *Type, assumeTrue bool, checkDerived bool, disjoint bool) *Type {
	...
	key := NarrowedTypeKey{t, candidate, assumeTrue, checkDerived, disjoint}
	if narrowedType, ok := c.narrowedTypes[key]; ok {
		return narrowedType
	}
	narrowedType := c.getNarrowedTypeWorker(t, candidate, assumeTrue, checkDerived, disjoint)
	c.narrowedTypes[key] = narrowedType
	return narrowedType
}

func (c *Checker) getNarrowedTypeWorker(t *Type, candidate *Type, assumeTrue bool, checkDerived bool, disjoint bool) *Type {
	if !assumeTrue {
		if t == candidate {
			return c.neverType
		}
		...
		trueType := c.getNarrowedTypeEx(t, candidate, true /*assumeTrue*/, false /*checkDerived*/, disjoint)
	...
	case c.isTypeAssignableTo(candidate, t):
		return candidate
	case disjoint:
		// A total miss against a partitioning candidate is uninhabited, not an
		// asserted intersection: no table is ever a thread.
		return c.neverType
	}
...
func (c *Checker) narrowTypeByOptionalChainContainment(f *FlowState, t *Type, operator ast.Kind, value *ast.Node, assumeTrue bool) *Type {
	// We are in a branch of obj?.foo == value (or obj?.foo ~= value). tlua unifies null
	// and undefined into a single nil, so there is one absence type to remove from obj:
	// When operator is == and type of value excludes nil, nil is removed from obj in true branch.
	// When operator is ~= and type of value excludes nil, nil is removed from obj in false branch.
	// When operator is == and type of value is nil, nil is removed from obj in false branch.
	// When operator is ~= and type of value is nil, nil is removed from obj in true branch.
	equalsOperator := operator == ast.KindEqualsEqualsToken
	nullableFlags := TypeFlagsNil
	valueType := c.getTypeOfExpression(value)
...
func (c *Checker) luaGlobalReferenceSymbol(node *ast.Node) *ast.Symbol {
	if !ast.IsAccessExpression(node) {
		return nil
	}
	root := luaEntityRoot(node.Expression())
	if !ast.IsIdentifier(root) || root.Text() != c.luaGlobalsSymbol.Name {
		return nil
	}
	links := c.symbolNodeLinks.Get(node)
	if links.luaGlobalReferenceSymbolComputed {
		return links.luaGlobalReferenceSymbol
	}
	links.luaGlobalReferenceSymbolComputed = true
	if !c.isLuaEnvironmentExpression(node.Expression()) {
		return nil
	}
	name, ok := c.getAccessedPropertyName(node)
	if !ok || ast.IsNumberKeyName(name) {
		return nil
	}
	links.luaGlobalReferenceSymbol = c.luaEnvironmentExport(name)
	return links.luaGlobalReferenceSymbol
}

func (c *Checker) luaGlobalReferenceMatches(global *ast.Symbol, node *ast.Node) bool {
	if global == nil {
		return false
	}
...
func (c *Checker) isMatchingReference(source *ast.Node, target *ast.Node) bool {
	...
	if (sourceGlobal != nil && sourceGlobal == targetGlobal) ||
		c.luaGlobalReferenceMatches(sourceGlobal, target) ||
		c.luaGlobalReferenceMatches(targetGlobal, source) {
		return true
	}
	switch source.Kind {
...
func writeFlowCacheKeySuffix(b *keyBuilder, declaredType *Type, initialType *Type, flowContainer *ast.Node) {
	b.writeByte(':')
...
func (c *Checker) tryGetElementAccessExpressionName(node *ast.ElementAccessExpression) (string, bool) {
	switch {
	case ast.IsNumericLiteral(node.ArgumentExpression):
...
func (c *Checker) tryGetNameFromEntityNameExpression(node *ast.Node) (string, bool) {
	...
	if symbol == nil || !c.isConstantReference(node) {
		return "", false
	}
	declaration := symbol.ValueDeclaration
	if declaration == nil {
...
func (c *Checker) isConstantReference(node *ast.Node) bool {
	...
	case ast.KindObjectBindingPattern, ast.KindArrayBindingPattern:
		...
		return ast.IsVariableDeclaration(rootDeclaration) && c.isVarConst(rootDeclaration)
	}
	return false
}
...
// Return a new type in which occurrences of the string and number primitives and placeholder template
...
func (c *Checker) replacePrimitivesWithLiterals(typeWithPrimitives *Type, typeWithLiterals *Type) *Type {
	if c.maybeTypeOfKind(typeWithPrimitives, TypeFlagsString|TypeFlagsTemplateLiteral|TypeFlagsNumber) &&
		c.maybeTypeOfKind(typeWithLiterals, TypeFlagsStringLiteral|TypeFlagsTemplateLiteral|TypeFlagsStringMapping|TypeFlagsNumberLiteral) {
...
func (c *Checker) getExplicitTypeOfSymbol(symbol *ast.Symbol, diagnostic *ast.Diagnostic) *Type {
	...
	if symbol.Flags&(ast.SymbolFlagsVariable|ast.SymbolFlagsProperty) != 0 {
		...
		if declaration != nil {
			...
			// point; the branch was unreachable and has been removed.)
			if diagnostic != nil {
...
func (c *Checker) getInitialTypeOfVariableDeclaration(node *ast.Node) *Type {
	// A name in a multi-name Lua local list starts at its position's value —
	...
	if node.Parent.Parent.Kind == ast.KindNumericForStatement {
		return c.numberType
	}
	if ast.IsForOfStatement(node.Parent.Parent) {
		// Every ForOfStatement is a Lua generic-for; the name takes its
		// position in the iterator function's return pack.
		statement := node.Parent.Parent
		return c.getLuaGenericForNameType(statement, slices.Index(node.Parent.AsVariableDeclarationList().Declarations.Nodes, node))
	}
	return c.errorType
}
...
func (c *Checker) getInitialTypeOfBindingElement(node *ast.Node) *Type {
	...
	default:
		// Binding-pattern rests no longer parse, so an array binding element is
		// always a positional one.
...
func (c *Checker) getTypeOfDestructuredArrayElement(t *Type, index int) *Type {
	...
	if elementType := c.checkIteratedTypeOrElementType(IterationUseDestructuring, t, c.nilType, nil /*errorNode*/); elementType != nil {
...
func (c *Checker) getFlowTypeInConstructor(symbol *ast.Symbol, constructor *ast.Node) *Type {
	...
	reference := c.factory.NewPropertyAccessExpression(c.factory.NewKeywordExpression(ast.KindThisKeyword), nil, nil /*colonToken*/, accessName, ast.NodeFlagsNone)
...
func (c *Checker) extendAssignmentPosition(node *ast.Node, declaration *ast.Node) int {
	...
	for node != nil && node.Pos() > declaration.Pos() {
		...
		case ast.KindVariableStatement, ast.KindExpressionStatement, ast.KindIfStatement, ast.KindDoStatement, ast.KindWhileStatement,
			...
			ast.KindWithStatement, ast.KindTryStatement, ast.KindClassDeclaration:
			pos = node.End()
		}
		node = node.Parent
	}
