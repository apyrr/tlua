...
var JsxNames = struct {
	...
	// tlua has no namespaces, so the classic `namespace JSX` member protocol
	// is a set of flat global interfaces instead.
	IntrinsicElements:                      "JsxIntrinsicElements",
	ElementClass:                           "JsxElementClass",
	ElementAttributesPropertyNameContainer: "JsxElementAttributesProperty",
	ElementChildrenAttributeNameContainer:  "JsxElementChildrenAttribute",
	Element:                                "JsxElement",
	ElementType:                            "JsxElementType",
	IntrinsicAttributes:                    "JsxIntrinsicAttributes",
	IntrinsicClassAttributes:               "JsxIntrinsicClassAttributes",
	LibraryManagedAttributes:               "JsxLibraryManagedAttributes",
}
...
func (c *Checker) checkJsxPreconditions(errorNode *ast.Node) {
	...
	if c.noImplicitAny && c.getJsxElementTypeAt(errorNode) == nil {
		c.error(errorNode, diagnostics.JSX_element_implicitly_has_type_any_because_the_global_type_JsxElement_does_not_exist)
	}
}
...
func (c *Checker) getContextualTypeForChildJsxExpression(node *ast.Node, child *ast.JsxChild, contextFlags ContextFlags) *Type {
	...
	jsxChildrenPropertyName := c.getJsxElementChildrenPropertyName(node)
...
func (c *Checker) elaborateJsxComponents(node *ast.Node, source *Type, target *Type, relation *Relation, diagnosticOutput *[]*ast.Diagnostic) bool {
	...
	for _, prop := range node.Properties() {
		// A spread attribute no longer parses, but KindJsxSpreadAttribute is still a
		// live AST kind (the factory and the API decoder build one), and it has no
		// name -- so this guard stands between a stray one and a nil dereference.
	...
	if ast.IsJsxOpeningElement(node.Parent) && ast.IsJsxElement(node.Parent.Parent) {
		...
		childrenPropName := c.getJsxElementChildrenPropertyName(node)
...
func (c *Checker) createJsxAttributesTypeFromAttributesProperty(openingLikeElement *ast.Node, checkMode CheckMode) *Type {
	...
	// hasSpreadAnyType / typeToIntersect are gone with the spread attribute: they
	// existed only to model `{...anyValue}` and an invalid spread operand.
	...
	jsxChildrenPropertyName := c.getJsxElementChildrenPropertyName(openingLikeElement)
	...
	if !isJsxOpenFragment {
		...
		for _, attributeDecl := range attributes.Properties() {
			...
			// A spread attribute no longer parses. It is not merely ignored here: the
			// kind is still constructible, and silently skipping one would drop its
			// properties from the attributes type. Assert instead of guessing.
			debug.Assert(ast.IsJsxAttribute(attributeDecl), "JSX spread attributes no longer parse")
	...
	if parentHasSemanticJsxChildren(openingLikeElement) {
		...
		if jsxChildrenPropertyName != ast.InternalSymbolNameMissing && jsxChildrenPropertyName != "" {
...
func (c *Checker) getUninstantiatedJsxSignaturesOfType(elementType *Type, caller *ast.Node) []*Signature {
	...
	if elementType.flags&TypeFlagsStringLiteral != 0 {
		...
		if intrinsicType == nil {
			c.error(caller, diagnostics.Property_0_does_not_exist_on_type_1, getStringLiteralValue(elementType), JsxNames.IntrinsicElements)
			return nil
		}
...
func (c *Checker) getJsxPropsTypeFromCallSignature(sig *Signature, context *ast.Node) *Type {
	...
	propsType = c.getJsxManagedAttributesFromLocatedAttributes(context, propsType)
...
func (c *Checker) getJsxPropsTypeFromClassType(sig *Signature, context *ast.Node) *Type {
	forcedLookupLocation := c.getJsxElementPropertiesName(context)
	var attributesType *Type
	...
	attributesType = c.getJsxManagedAttributesFromLocatedAttributes(context, attributesType)
...
func (c *Checker) getJsxManagedAttributesFromLocatedAttributes(context *ast.Node, attributesType *Type) *Type {
	managedSym := c.getJsxLibraryManagedAttributes(context)
	if managedSym != nil {
...
func (c *Checker) getJsxLibraryManagedAttributes(location *ast.Node) *ast.Symbol {
	return c.getJsxTypeSymbol(JsxNames.LibraryManagedAttributes, location)
}

func (c *Checker) getJsxElementTypeSymbol(location *ast.Node) *ast.Symbol {
	return c.getJsxTypeSymbol(JsxNames.ElementType, location)
}
...
func (c *Checker) getJsxElementPropertiesName(location *ast.Node) string {
	return c.getNameFromJsxElementAttributesContainer(JsxNames.ElementAttributesPropertyNameContainer, location)
}

func (c *Checker) getJsxElementChildrenPropertyName(location *ast.Node) string {
	...
	return c.getNameFromJsxElementAttributesContainer(JsxNames.ElementChildrenAttributeNameContainer, location)
}
...
func (c *Checker) getNameFromJsxElementAttributesContainer(nameOfAttribPropContainer string, location *ast.Node) string {
	// JsxElementAttributesProperty | JsxElementChildrenAttribute [symbol]
	{
		jsxElementAttribPropInterfaceSym := c.getJsxTypeSymbol(nameOfAttribPropContainer, location)
		if jsxElementAttribPropInterfaceSym != nil {
			...
			if len(propertiesOfJsxElementAttribPropInterface) > 1 && len(jsxElementAttribPropInterfaceSym.Declarations) != 0 {
				...
				c.error(jsxElementAttribPropInterfaceSym.Declarations[0], diagnostics.The_global_type_0_may_not_have_more_than_one_property, nameOfAttribPropContainer)
			}
		}
	}
...
func (c *Checker) createSignatureForJSXIntrinsic(node *ast.Node, result *Type) *Signature {
	elementType := c.getJsxType(JsxNames.Element, node)
...
func (c *Checker) getIntrinsicTagSymbol(node *ast.Node) *ast.Symbol {
	...
	if !c.isErrorType(intrinsicElementsType) {
		...
		// Wasn't found
		c.error(node, diagnostics.Property_0_does_not_exist_on_type_1, tagName.Text(), JsxNames.IntrinsicElements)
		links.resolvedSymbol = c.unknownSymbol
		return links.resolvedSymbol
	}
	if c.noImplicitAny {
		c.error(node, diagnostics.JSX_element_implicitly_has_type_any_because_no_interface_0_exists, JsxNames.IntrinsicElements)
	}
	links.resolvedSymbol = c.unknownSymbol
	return links.resolvedSymbol
}
...
func (c *Checker) getJsxStatelessElementTypeAt(location *ast.Node) *Type {
	...
	return c.getUnionType([]*Type{jsxElementType, c.nilType})
}
...
func (c *Checker) getJsxElementTypeTypeAt(location *ast.Node) *Type {
	sym := c.getJsxElementTypeSymbol(location)
	if sym == nil {
		return nil
	}
...
// getJsxType resolves one of the flat JSX protocol types (JsxElement,
// JsxIntrinsicElements, ...). tlua has no namespaces, so the protocol is a
// set of flat interfaces resolved by ordinary name lookup from the location
// (so a module-local fixture works), falling back to the globals.
func (c *Checker) getJsxType(name string, location *ast.Node) *Type {
	if typeSymbol := c.getJsxTypeSymbol(name, location); typeSymbol != nil {
		return c.getDeclaredTypeOfSymbol(typeSymbol)
	}
	return c.errorType
}

func (c *Checker) getJsxTypeSymbol(name string, location *ast.Node) *ast.Symbol {
	return c.resolveName(location, name, ast.SymbolFlagsType, nil /*nameNotFoundMessage*/, false /*isUse*/, false /*excludeGlobals*/)
}
