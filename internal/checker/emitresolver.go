...
func (r *EmitResolver) determineIfDeclarationIsVisible(node *ast.Node) bool {
	...
	case ast.KindVariableDeclaration,
		...
		if isLuaLocalDeclaration(node) {
			return false
		}
		// A dotted function declaration (`function M.f() ... end`) is a member of M,
		// not a declaration in its own right, so it never emits standalone.
...
func isLuaLocalDeclaration(node *ast.Node) bool {
	return ast.IsVariableDeclaration(node) && ast.IsLuaLocal(node) ||
		ast.IsFunctionDeclaration(node) && ast.IsLuaLocal(node)
}
...
func (r *EmitResolver) markLinkedAliases(node *ast.Node) {
	...
	for exportSymbol != nil {
		...
		for _, declaration := range exportSymbol.Declarations {
			if isLuaLocalDeclaration(declaration) {
				continue
			}
...
func (r *EmitResolver) IsDefinitelyReferenceToGlobalSymbolObject(node *ast.Node) bool {
	...
	if node.Expression().Name().Text() != "Symbol" {
		return false
	}
	r.checkerMu.Lock()
	defer r.checkerMu.Unlock()
	// `_G.Symbol.something`, including repeated `_G` self-access, must be rooted
	// at the built-in environment rather than a lexical shadow.
...
func (r *EmitResolver) GetReferencedExportContainer(node *ast.IdentifierNode, prefixLocals bool) *ast.Node /*SourceFile|ModuleDeclaration*/ {
	if !ast.IsParseTreeNode(node) {
		return nil
	}
...
func (r *EmitResolver) CreateLiteralConstValue(emitContext *printer.EmitContext, node *ast.Node, tracker nodebuilder.SymbolTracker) *ast.Node {
	...
	var literalResult *ast.Node
	if t == r.checker.trueType {
		literalResult = emitContext.Factory.NewKeywordExpression(ast.KindTrueKeyword)
	} else if t == r.checker.falseType {
		literalResult = emitContext.Factory.NewKeywordExpression(ast.KindFalseKeyword)
	}
	if literalResult != nil {
