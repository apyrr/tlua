...
func isCompoundLikeAssignment(assignment *ast.Node) bool {
	...
	// Upstream gates on isShiftOperatorOrHigher; `..` now occupies shift's old
	// precedence slot (relational < concatenation < additive), so it belongs in
	// the gate too -- a `t[k] = t[k] .. s` build is compound-like just as `+` is.
	return right.Kind == ast.KindBinaryExpression && isConcatenationOperatorOrHigher(right.AsBinaryExpression().OperatorToken.Kind)
}
...
func CompareTypes(t1, t2 *Type) int {
	...
	switch {
	case t1.flags&(TypeFlagsAny|TypeFlagsUnknown|TypeFlagsString|TypeFlagsNumber|TypeFlagsBoolean|TypeFlagsESSymbol|TypeFlagsVoid|TypeFlagsNil|TypeFlagsNever|TypeFlagsNonPrimitive) != 0:
	...
	case t1.flags&TypeFlagsObject != 0:
		...
		if t1.objectFlags&ObjectFlagsReference != 0 && t2.objectFlags&ObjectFlagsReference != 0 {
			...
			// this should be fine.
			// Compared, not subtracted: the kind mask reaches bit 31, which a subtraction of
			// int-converted flags would sign-flip on a 32-bit int.
			if c := cmp.Compare(t1.objectFlags&ObjectFlagsObjectTypeKindMask, t2.objectFlags&ObjectFlagsObjectTypeKindMask); c != 0 {
				return c
			}
			if c := compareTypeMappers(t1.AsObjectType().mapper, t2.AsObjectType().mapper); c != 0 {
				return c
	...
	case t1.flags&TypeFlagsUniqueESSymbol != 0:
		// Unique symbols are ordered by their symbol.
		if c := t1.checker.compareSymbols(t1.symbol, t2.symbol); c != 0 {
			return c
		}
...
func getSortOrderFlags(t *Type) int { return int(t.flags) }
...
func isConcatenationOperatorOrHigher(kind ast.Kind) bool {
	return kind == ast.KindDotDotToken || isAdditiveOperatorOrHigher(kind)
}
...
func isRelationalOperatorOrHigher(kind ast.Kind) bool {
	return isRelationalOperator(kind) || isConcatenationOperatorOrHigher(kind)
}

func isEqualityOperator(kind ast.Kind) bool {
	return kind == ast.KindEqualsEqualsToken || kind == ast.KindTildeEqualsToken
}
...
func isLogicalOperatorOrHigher(kind ast.Kind) bool {
	return ast.IsLogicalBinaryOperator(kind) || isEqualityOperatorOrHigher(kind)
}
...
// tryGetNameFromType maps a literal or unique-symbol type to the symbol-table
// (identity) name it denotes. Number-literal types map into the number-key
// namespace (Lua's t[1] != t["1"]).
...
// `<...A>`). Its value is a whole value pack, so `...A`/`...: A` spread it and it
// may not stand in a single-value position.
func isPackTypeParameter(t *Type) bool {
...
func (c *Checker) isConstantVariable(symbol *ast.Symbol) bool {
	return symbol.Flags&ast.SymbolFlagsVariable != 0 && (c.getDeclarationNodeFlagsFromSymbol(symbol)&ast.NodeFlagsConst) != 0
}
...
func (c *Checker) isMutableLocalVariableDeclaration(declaration *ast.Node) bool {
	// Return true if symbol is a non-exported and non-global `let` variable, or a Lua local.
	return ast.IsLuaLocal(declaration) || declaration.Parent.Flags&ast.NodeFlagsLet != 0 && !(ast.GetCombinedModifierFlags(declaration)&ast.ModifierFlagsExport != 0 || declaration.Parent.Parent.Kind == ast.KindVariableStatement && ast.IsGlobalSourceFile(declaration.Parent.Parent.Parent))
}
...
func containsNonMissingUndefinedType(c *Checker, t *Type) bool {
	...
	return candidate.flags&TypeFlagsNil != 0 && candidate != c.missingType
}
...
func CreateModeMismatchDetails(program Program, file *ast.SourceFile) DiagnosticDetails {
	// Without the Node module-variant extensions there is no extension-based
	// escape hatch to suggest; only the package.json "type" field remains.
