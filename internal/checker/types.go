...
// Links for declared types (type parameters, classes, and interfaces)
...
type LuaGenericForLinks struct {
	iteratorResult *luaGenericForIteratorResult
}

// LuaBuiltinLinks caches a call's Lua builtin refinement (nil negatives
// included) so flow walks do not re-derive it per reference.
type LuaBuiltinLinks struct {
	refined  *Type
	resolved bool
}
...
const (
	...
	// 1 << 10 was NodeCheckFlagsEnumValuesComputed.
...
type SymbolNodeLinks struct {
	...
	// Nil is a cacheable result for dynamic, shadowed, and filtered `_G` accesses.
	luaGlobalReferenceSymbol         *ast.Symbol
	luaGlobalReferenceSymbolComputed bool
}
...
const (
	...
	TypeFlagsNil     TypeFlags = 1 << 2
	// bit 3 (was TypeFlagsNull) is free: tlua merges null into nil, so no type
	// carries a distinct null flag.
	TypeFlagsVoid   TypeFlags = 1 << 4
	TypeFlagsString TypeFlags = 1 << 5
	TypeFlagsNumber TypeFlags = 1 << 6
	// bit 7 (was TypeFlagsBigInt) is free: tlua has no bigint.
	...
	// bit 12 (was TypeFlagsBigIntLiteral) is free: tlua has no bigint.
	TypeFlagsBooleanLiteral TypeFlags = 1 << 13
	TypeFlagsUniqueESSymbol TypeFlags = 1 << 14 // unique symbol
	// 1 << 15 and 1 << 16 were EnumLiteral and Enum.
	...
	TypeFlagsNullable                      = TypeFlagsNil // nil is the only absence type; "nullable" means "can be nil"
	TypeFlagsLiteral                       = TypeFlagsStringLiteral | TypeFlagsNumberLiteral | TypeFlagsBooleanLiteral
	TypeFlagsUnit                          = TypeFlagsLiteral | TypeFlagsUniqueESSymbol | TypeFlagsNullable
	TypeFlagsFreshable                     = TypeFlagsLiteral
	...
	TypeFlagsIntrinsic                     = TypeFlagsAny | TypeFlagsUnknown | TypeFlagsString | TypeFlagsNumber | TypeFlagsESSymbol | TypeFlagsVoid | TypeFlagsNil | TypeFlagsNever | TypeFlagsNonPrimitive
	TypeFlagsStringLike                    = TypeFlagsString | TypeFlagsStringLiteral | TypeFlagsTemplateLiteral | TypeFlagsStringMapping
	TypeFlagsNumberLike                    = TypeFlagsNumber | TypeFlagsNumberLiteral
	...
	TypeFlagsVoidLike                      = TypeFlagsVoid | TypeFlagsNil
	TypeFlagsPrimitive                     = TypeFlagsStringLike | TypeFlagsNumberLike | TypeFlagsBooleanLike | TypeFlagsESSymbolLike | TypeFlagsVoidLike
	TypeFlagsDefinitelyNonNullable         = TypeFlagsStringLike | TypeFlagsNumberLike | TypeFlagsBooleanLike | TypeFlagsESSymbolLike | TypeFlagsObject | TypeFlagsNonPrimitive
	TypeFlagsDisjointDomains               = TypeFlagsNonPrimitive | TypeFlagsStringLike | TypeFlagsNumberLike | TypeFlagsBooleanLike | TypeFlagsESSymbolLike | TypeFlagsVoidLike
	...
	TypeFlagsSingleton                     = TypeFlagsAny | TypeFlagsUnknown | TypeFlagsString | TypeFlagsNumber | TypeFlagsBoolean | TypeFlagsESSymbol | TypeFlagsVoid | TypeFlagsNil | TypeFlagsNever | TypeFlagsNonPrimitive
	// 'TypeFlagsNarrowable' types are types where narrowing actually narrows.
	// This *should* be every type other than null, undefined, void, and never
	TypeFlagsNarrowable = TypeFlagsAny | TypeFlagsUnknown | TypeFlagsStructuredOrInstantiable | TypeFlagsStringLike | TypeFlagsNumberLike | TypeFlagsBooleanLike | TypeFlagsESSymbol | TypeFlagsUniqueESSymbol | TypeFlagsNonPrimitive
...
const (
	...
	ObjectFlagsObjectTypeKindMask = ObjectFlagsClassOrInterface | ObjectFlagsReference | ObjectFlagsTuple | ObjectFlagsAnonymous | ObjectFlagsMapped | ObjectFlagsReverseMapped | ObjectFlagsEvolvingArray | ObjectFlagsInstantiationExpressionType | ObjectFlagsSingleSignatureType | ObjectFlagsMetatable
	...
	// Bit 31 is the last free bit of the TypeFlags.Object domain. Further metamethods
	// (__newindex, __call, operators) extend MetatableType rather than claiming a kind.
	ObjectFlagsMetatable = 1 << 31 // Lua table paired with its metatable (a setmetatable result)
...
func (t *Type) AsMetatableType() *MetatableType             { return t.data.(*MetatableType) }
...
type LiteralType struct {
	TypeBase
	value       any   // string | jsnum.Number | bool
...
type TupleType struct {
	...
	readonly      bool
	isPack        bool // Lua multi-return value pack; interned separately from real tuples
}
...
// A Lua table paired with its metatable, produced by setmetatable(table, metatable). Its
// members resolve to the table's own shape with the metatable's __index filling the gaps,
// so every consumer sees ordinary resolved members; getmetatable reads metatableType back.
type MetatableType struct {
	...
	protectedType      *Type // metatableType's committed __metatable, or nil: non-nil marks the metatable protected
}
...
// `this is T` / `asserts this is T` predicates are removed with the `this`
// type; only the identifier-based predicate kinds remain.
const (
	TypePredicateKindIdentifier TypePredicateKind = iota
