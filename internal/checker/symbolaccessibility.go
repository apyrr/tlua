...
func symbolTableIDFromLuaLocals(node *ast.Node) symbolTableID {
	return stKindLuaLocals | symbolTableID(ast.GetNodeId(node))
}
...
func (c *Checker) getAccessibleSymbolChainEx(ctx accessibleSymbolChainContext) []*ast.Symbol {
	...
	cacheLocation := firstRelevantLocation
	if hasLuaLocalScope(ctx.enclosingDeclaration) {
		cacheLocation = ctx.enclosingDeclaration
	}
	links := c.symbolContainerLinks.Get(ctx.symbol)
	linkKey := accessibleChainCacheKey{ctx.useOnlyExternalAliasing, cacheLocation, ctx.meaning}
...
func (c *Checker) trySymbolTable(
	...
	// If there's no result and we're looking at the global symbol table, treat `_G` like an alias and try to lookup thru that
	if isGlobals {
		return c.getCandidateListForSymbol(ctx, c.luaGlobalsSymbol, c.luaGlobalsSymbol, ignoreQualification)
	}
	return nil
}
...
func (c *Checker) canQualifySymbol(
	...
	if !c.needsQualification(symbolFromSymbolTable, ctx.enclosingDeclaration, meaning) {
		return true
	}
	// A global augmentation's binder parent is the syntax-only `global` module,
	// not a valid source qualifier. Let the global-table fallback supply `_G`.
	if parent := symbolFromSymbolTable.Parent; parent != nil && core.Some(parent.Declarations, ast.IsGlobalScopeAugmentation) {
		return false
	}
	// If symbol needs qualification, make sure that its parent is accessible.
	return len(c.getAccessibleSymbolChainEx(accessibleSymbolChainContext{symbolFromSymbolTable.Parent, ctx.enclosingDeclaration, getQualifiedLeftMeaning(meaning), ctx.useOnlyExternalAliasing, ctx.visitedSymbolTablesMap})) > 0
}
...
func (c *Checker) someSymbolTableInScope(
	...
	for location := enclosingDeclaration; location != nil; location = location.Parent {
		if luaLocals := luaLocalSymbolTableForAccessibility(location, enclosingDeclaration); luaLocals != nil {
			if callback(luaLocals, symbolTableIDFromLuaLocals(location.AsNode()), false, true, location) {
				return true
			}
		}
...
func luaLocalSymbolTableForAccessibility(container *ast.Node, reference *ast.Node) ast.SymbolTable {
	var table ast.SymbolTable
	ast.ForEachVisibleLuaLocal(container, reference, func(symbol *ast.Symbol) {
		if table == nil {
			table = make(ast.SymbolTable)
		}
...
func hasLuaLocalScope(location *ast.Node) bool {
	for ; location != nil; location = location.Parent {
		if len(location.LuaLocals()) != 0 {
			return true
		}
	}
	return false
