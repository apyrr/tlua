...
func (b *NodeBuilderImpl) pseudoTypeToNode(t *pseudochecker.PseudoType) *ast.Node {
	...
	case pseudochecker.PseudoTypeKindUnion:
		...
		appendTypeNode = func(node *ast.Node) {
			...
			if node.Kind == ast.KindNilKeyword {
				if hasUndefined {
					return
				}
				hasUndefined = true
	...
	case pseudochecker.PseudoTypeKindSingleCallSignature:
		...
		return b.f.NewFunctionTypeNode(nil, typeParams, params, returnType)
	...
	case pseudochecker.PseudoTypeKindObjectLiteral:
		...
		return result
	case pseudochecker.PseudoTypeKindStringLiteral, pseudochecker.PseudoTypeKindNumericLiteral:
...
func (b *NodeBuilderImpl) pseudoParametersEquivalentToParameters(params []*pseudochecker.PseudoParameter, targetSig *Signature, reportErrors bool, nonParamErrorLocation *ast.Node) bool {
	...
	for i, p := range params {
		...
		paramType := b.ch.getTypeOfParameter(targetParam)
		if decl := targetParam.ValueDeclaration; decl != nil && ast.IsVarargParameter(decl) {
			// A vararg's annotation is the pack's *element* type, while its symbol
			// carries the array built from that. The pseudotype is taken straight from
			// the annotation, so it must be compared against the element -- otherwise
			// nothing ever matches, and every vararg is reported as needing the
			// explicit type annotation it already has.
			if element := b.ch.getElementTypeOfArrayType(paramType); element != nil {
				paramType = element
			}
		}
...
func (b *NodeBuilderImpl) pseudoReturnTypeMatchesPredicate(rt *pseudochecker.PseudoType, predicate *TypePredicate) bool {
	...
	predicateIsAsserts := predicate.kind == TypePredicateKindAssertsIdentifier
