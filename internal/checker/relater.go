...
func (c *Checker) isSimpleTypeRelatedTo(source *Type, target *Type, relation *Relation, errorReporter ErrorReporter) bool {
	...
	// `nil` (the merged null/undefined) is assignable to `nil` and `void`.
...
func (c *Checker) getVariances(t *Type) []VarianceFlags {
	...
	// The synthesized number-key table targets are symbol-less, so they must take
	// this fast path — getVariancesWorker would deref their nil symbol.
	if t == c.globalArrayType || t == c.globalReadonlyArrayType || t == c.luaTableType || t == c.luaReadonlyTableType || t.objectFlags&ObjectFlagsTuple != 0 {
...
func (c *Checker) compareSignaturesRelated(source *Signature, target *Signature, checkMode SignatureCheckMode, reportErrors bool, errorReporter ErrorReporter, compareTypes TypeComparer, reportUnreliableMarkers *TypeMapper) Ternary {
	...
	if checkMode&SignatureCheckModeIgnoreReturnTypes == 0 {
		...
		// Lua multi-return adjustment for signature compatibility. Callers
		// through the target consume only the values the target declares, so
		// extra source values are dropped (covariant truncation) — in contrast
		// to return statements, which are strict about the declared arity.
		switch {
		case c.maybePackType(sourceReturnType) && !c.maybePackType(targetReturnType):
			// Only the first value is consumed through the target.
			sourceReturnType = c.adjustMultiReturn(sourceReturnType)
		case c.maybePackType(targetReturnType):
			switch {
			case isPackType(sourceReturnType) && isPackType(targetReturnType):
				sourceReturnType = c.truncatePackForRelation(sourceReturnType, targetReturnType)
			case isPackType(sourceReturnType):
				// Union-of-packs target: compare as-is.
			case !c.maybePackType(sourceReturnType):
				// Zero values (void) are assignable when the target requires none;
				// one value leaves the target's remaining values reading as nil.
...
func (c *Checker) isValidDeclarationForTupleLabel(d *ast.Node) bool {
	// A vararg's name is synthetic, so it labels nothing: using it would print
	// tuples as `[a: number, ......: boolean[]]`.
	return ast.IsNamedTupleMember(d) || ast.IsParameterDeclaration(d) && !ast.IsVarargParameter(d) && d.Name() != nil && ast.IsIdentifier(d.Name())
}
...
// truncatePackForRelation drops trailing source pack values beyond the target
// pack's fixed arity — sound for signature assignment because call sites
// through the target never consume them.
func (c *Checker) truncatePackForRelation(source *Type, target *Type) *Type {
	targetTuple := target.TargetTupleType()
	if targetTuple.combinedFlags&ElementFlagsVariable != 0 {
		return source
	}
	sourceTuple := source.TargetTupleType()
	arity := len(targetTuple.elementInfos)
	if sourceTuple.combinedFlags&ElementFlagsVariable != 0 || len(sourceTuple.elementInfos) <= arity {
		return source
	}
	return c.createPackTypeEx(c.getTypeArguments(source)[:arity], sourceTuple.elementInfos[:arity], false /*collapse*/)
}
...
func (c *Checker) getTupleElementLabel(elementInfo TupleElementInfo, restSymbol *ast.Symbol, index int) string {
	...
	if restSymbol != nil && ast.IsVarargSymbolName(restSymbol.Name) {
		// A vararg has no name to derive labels from. Its unbounded tail keeps the
		// `...` spelling, which the display sites recognize; deriving from the
		// synthetic name instead would print unlabeled fixed elements as `..._0`.
		if elementInfo.flags&ElementFlagsVariable != 0 {
			return ast.VarargParameterName
		}
...
func (c *Checker) getUnionOrIntersectionTypePredicate(signatures []*Signature, isUnion bool) *TypePredicate {
	...
	for _, sig := range signatures {
		...
		if pred != nil {
			...
			if pred.kind != TypePredicateKindIdentifier || last != nil && !c.typePredicateKindsMatch(last, pred) {
				return nil
			}
			last = pred
...
func (c *Checker) createTypePredicateFromTypePredicateNode(node *ast.Node, signature *Signature) *TypePredicate {
	...
	// `this is T` predicates are removed with the `this` type; the parameter name
	// is now always an ordinary identifier.
...
func (r *Relater) getUndefinedStrippedTargetIfNeeded(source *Type, target *Type) *Type {
	if source.flags&TypeFlagsUnion != 0 && target.flags&TypeFlagsUnion != 0 && source.Types()[0].flags&TypeFlagsNil == 0 && target.Types()[0].flags&TypeFlagsNil != 0 {
		return r.c.extractTypesOfKind(target, ^TypeFlagsNil)
	}
	return target
}
...
func (r *Relater) typeRelatedToSomeType(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	if target.flags&TypeFlagsUnion != 0 {
		...
		if r.relation != r.c.comparableRelation && target.objectFlags&ObjectFlagsPrimitiveUnion != 0 &&
			source.flags&(TypeFlagsStringLiteral|TypeFlagsBooleanLiteral|TypeFlagsNumberLiteral) != 0 {
			...
			// We exclude the comparable relation because it must be checked in both directions.
...
func (r *Relater) structuredTypeRelatedToWorker(source *Type, target *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	default:
		...
		case source.objectFlags&ObjectFlagsReference != 0 && target.objectFlags&ObjectFlagsReference != 0 && source.Target() == target.Target() && !isTupleType(source) && !r.c.isMarkerType(source) && !r.c.isMarkerType(target):
			// An empty array literal must be assignable to a `never[]`; this check ensures that
			// regardless of the element type synthesized for the empty array literal.
		...
		for _, t := range target.Types() {
			for i := range sourcePropertiesFiltered {
				...
				related := r.propertyRelatedTo(source, target, sourceProperty, targetProperty, func(*ast.Symbol) *Type { return combination[i] },
					false /*reportErrors*/, IntersectionStateNone, true /*skipOptional*/)
...
func (r *Relater) isPropertySymbolTypeRelated(sourceProp *ast.Symbol, targetProp *ast.Symbol, getTypeOfSourceProperty func(sym *ast.Symbol) *Type, reportErrors bool, intersectionState IntersectionState) Ternary {
	targetIsOptional := targetProp.CheckFlags&ast.CheckFlagsPartial != 0
...
func (r *Relater) indexSignaturesRelatedTo(source *Type, target *Type, sourceIsPrimitive bool, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	for _, targetInfo := range indexInfos {
		...
		default:
			related = r.typeRelatedToIndexInfo(source, targetInfo, sourceIsPrimitive, reportErrors, intersectionState)
		}
		if related == TernaryFalse {
			return TernaryFalse
		}
...
func (r *Relater) typeRelatedToIndexInfo(source *Type, targetInfo *IndexInfo, sourceIsPrimitive bool, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	// A primitive is not a table, and must never satisfy an index signature through its
	// members. By this point source is the primitive's *apparent* type -- the `Number`
	// or `String` interface -- whose members carry no table key at all, so the check
	// below would find nothing to reject and pass it vacuously.
	//
	...
	if !sourceIsPrimitive && intersectionState&IntersectionStateSource == 0 && (r.relation != r.c.strictSubtypeRelation || source.objectFlags&ObjectFlagsFreshLiteral != 0) && r.c.isObjectTypeWithInferableIndex(source) {
...
// Return true if the type's own properties may stand in for an index signature it does
// not declare. Every declared object type qualifies -- in Lua a record, an interface,
// and a class instance are all just tables, and `pairs` over any of them is idiomatic
// (TypeScript restricts this to object/type literals). Excluded: a type with call or
// construct signatures (a Lua function is not a table), and a synthesized placeholder
// with no symbol -- the implicit empty object behind `v || {}` must keep failing an
// index it does not declare.
func (c *Checker) isObjectTypeWithInferableIndex(t *Type) bool {
	...
	if t.objectFlags&ObjectFlagsReverseMapped != 0 {
		return c.isObjectTypeWithInferableIndex(t.AsReverseMappedType().source)
	}
	// A metatable type has no symbol of its own, but it is still a table: it satisfies an
	// index signature through its members exactly as the table it was built from does.
	if t.objectFlags&ObjectFlagsMetatable != 0 {
		return c.isObjectTypeWithInferableIndex(t.AsMetatableType().tableType)
	}
	return t.objectFlags&ObjectFlagsJSLiteral != 0 ||
		t.flags&TypeFlagsObject != 0 && t.symbol != nil && !c.typeHasCallOrConstructSignatures(t)
}
...
func (r *Relater) membersRelatedToIndexInfo(source *Type, targetInfo *IndexInfo, reportErrors bool, intersectionState IntersectionState) Ternary {
	...
	for _, prop := range props {
		...
		if r.c.isApplicableIndexType(r.c.getLiteralTypeFromProperty(prop, TypeFlagsStringOrNumberLiteralOrUnique, false), keyType) {
			...
			var t *Type
			if r.c.exactOptionalPropertyTypes || propType.flags&TypeFlagsNil != 0 || keyType == r.c.numberType || prop.Flags&ast.SymbolFlagsOptional == 0 {
				t = propType
			} else {
...
func (c *Checker) isTypeDerivedFrom(source *Type, target *Type) bool {
	...
	default:
		...
		// `isArrayType` now denotes the `T[]` number-key table, which is
		// unrelated to the inert global ReadonlyArray, so that clause would
		// conflate the two distinct types (e.g. wrongly narrowing an
		// interface extending ReadonlyArray to a `T[]` table). Only the plain
		// base-type relation applies.
		return c.hasBaseType(source, c.getTargetType(target))
	}
}
