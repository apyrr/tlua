...
// GetSymbolsInScopeAtPosition walks the scope chain from location with an
// explicit cursor position as the Lua-local visibility reference. There is
// deliberately no position-free variant: using the scope node's own range (a
// source file starts at 0) hides every position-gated Lua local declared
// after it, which is exactly the bug that kept locals out of completions.
func (c *Checker) GetSymbolsInScopeAtPosition(location *ast.Node, pos int, meaning ast.SymbolFlags) []*ast.Symbol {
	return c.getSymbolsInScope(location, pos, pos, meaning)
}
...
func (c *Checker) getSymbolsInScope(location *ast.Node, refPos int, refEnd int, meaning ast.SymbolFlags) []*ast.Symbol {
	...
	copyLuaLocals := func(container *ast.Node, meaning ast.SymbolFlags) {
		if meaning != 0 {
			ast.ForEachVisibleLuaLocalAtPos(container, refPos, refEnd, func(symbol *ast.Symbol) {
				copySymbol(symbol, meaning)
			})
		}
	}
	...
	populateSymbols := func() {
		for location != nil {
			copyLuaLocals(location, meaning)
...
func (c *Checker) GetContextualTypeForArrayLiteralAtPosition(contextualArrayType *Type, arrayLiteral *ast.Node, position int) *Type {
	...
	return c.getContextualTypeForElementExpression(contextualArrayType, elementIndex, -1 /*length*/)
}
...
func (c *Checker) GetPropertySymbolsFromContextualType(node *ast.Node, contextualType *Type, unionSymbolOk bool) []*ast.Symbol {
	...
	if name == "" || name == ast.InternalSymbolNameMissing {
		return nil
	}
