...
func (options *CompilerOptions) GetModuleResolutionKind() ModuleResolutionKind {
	// The moduleResolution option is gone; the resolution kind is derived
	// purely from the module kind (bundler unless module is node16/nodenext).
...
func (options *CompilerOptions) AllowImportingTsExtensionsFrom(fileName string) bool {
	return tspath.IsDeclarationFileName(fileName)
}
...
// name<->file round-trip breaks.
func (options *CompilerOptions) GetLuaSearchRoot(currentDirectory string) string {
	if options.RootDir != "" {
		return options.RootDir
	}
	if options.ConfigFilePath != "" {
		return tspath.GetDirectoryPath(options.ConfigFilePath)
	}
...
func (options *CompilerOptions) GetIsolatedModules() bool {
	return options.IsolatedModules == TSTrue
}
