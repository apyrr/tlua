	...
	for i := 0; i < len(s); {
		...
		if c < utf8.RuneSelf {
			...
			case c == q:
				b.WriteByte('\\')
				b.WriteByte(q)
			case c == '$' && quoteChar == QuoteCharBacktick && i+1 < len(s) && s[i+1] == '{':
				// `${` begins a substitution in a backtick template literal.
				b.WriteString(`\$`)
			case c < 0x20:
				b.WriteString(`\x`)
				b.WriteByte(lowerHexDigit(c >> 4))
				b.WriteByte(lowerHexDigit(c & 0xf))
			default:
				b.WriteByte(c)
			}
			i++
		...
		// Non-ASCII: keep valid UTF-8 verbatim, but escape a stray/invalid byte (e.g. a lone
		// WTF-8 surrogate sentinel) as `\xHH` so the output stays valid UTF-8 and round-trips.
...
func getLiteralText(node *ast.LiteralLikeNode, sourceFile *ast.SourceFile, flags getLiteralTextFlags) string {
	...
	// String literals never reach here: getLiteralTextOfNode intercepts them and emits a Lua
	// string literal directly (so `\u{..}` and other JS-only escapes cannot leak into output).
	switch node.Kind {
