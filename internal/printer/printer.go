...
func (p *Printer) getLiteralTextOfNode(node *ast.LiteralLikeNode, sourceFile *ast.SourceFile, flags getLiteralTextFlags) string {
	// tlua: every string literal is re-emitted from its cooked value as a Lua string literal, so
	// the output is valid Lua regardless of the original JS spelling (e.g. a `\u{..}` escape that
	// Lua cannot parse). This is the single seam through which real, synthetic (textSource), and
	// name-context string literals all pass, so it also covers `.d.tlua` and LS type display.
	if ast.IsStringLiteral(node) {
		text := node.Text()
		quoteChar := QuoteCharDoubleQuote
		if textSourceNode, ok := p.emitContext.textSource[node]; ok && textSourceNode != nil {
			// A synthetic string sourced from another node: resolve its text at emit time (a
			// captured value can be stale, e.g. after an identifier rename).
...
func (p *Printer) writeCommentRangeWorker(text string, lineMap []core.TextPos, kind ast.Kind, loc core.TextRange) {
	rawComment := text[loc.Pos():loc.End()]
	commentText := formatLuaComment(kind, rawComment)
	if kind == ast.KindSingleLineCommentTrivia {
		p.writeComment(commentText)
		return
	}
	if commentText != rawComment {
		start := loc.Pos()
		text = text[:start] + commentText + text[loc.End():]
		loc = core.NewTextRange(start, start+len(commentText))
...
func formatLuaComment(kind ast.Kind, comment string) string {
	if kind == ast.KindSingleLineCommentTrivia {
		if strings.HasPrefix(comment, "//") || strings.HasPrefix(comment, "--") {
			comment = comment[2:]
		}
		// The canonical prefix supplies the conventional separator itself.
	...
	case strings.HasPrefix(comment, "--["):
		pos := 3
		for pos < len(comment) && comment[pos] == '=' {
			pos++
		}
		if pos < len(comment) && comment[pos] == '[' {
			level := pos - 3
			inner = comment[pos+1:]
			closer := "]" + strings.Repeat("=", level) + "]"
			if strings.HasSuffix(inner, closer) {
				inner = inner[:len(inner)-len(closer)]
			}
		}
	}

	level := 0
	for strings.Contains(inner, "]"+strings.Repeat("=", level)+"]") {
		level++
	}
	equals := strings.Repeat("=", level)
	return "--[" + equals + "[" + inner + "]" + equals + "]"
}
...
func (p *Printer) emitNoSubstitutionTemplateLiteral(node *ast.NoSubstitutionTemplateLiteral) {
	state := p.enterNode(node.AsNode())
	// tlua: Lua has no backtick strings; a template with no substitutions is just a string.
	p.writer.WriteStringLiteral(luaStringLiteral(node.Text, QuoteCharDoubleQuote))
	p.exitNode(node.AsNode(), state)
}
...
func (p *Printer) emitModifierList(parentNode *ast.Node, modifiers *ast.ModifierList) int {
	if modifiers == nil || len(modifiers.Nodes) == 0 {
		return parentNode.Pos()
	}
	// Decorators were removed from tlua, so a ModifierList only ever holds modifiers.
...
func (p *Printer) emitTypeParameter(node *ast.TypeParameterDeclaration) {
	...
	if node.DotDotDotToken != nil {
		// A generic pack parameter is spelled `...A`.
...
func (p *Printer) emitParameter(node *ast.ParameterDeclaration) {
	...
	p.emitModifierList(node.AsNode(), node.Modifiers())
	p.emitTokenNode(node.DotDotDotToken)
	// A vararg has no name in source; its synthetic `...` identifier stands in
	// only so the parameter owns a symbol. Emitting it would print `... ...`.
	if !ast.IsVarargParameter(node.AsNode()) {
...
func (p *Printer) emitTypeAnnotation(node *ast.TypeNode) {
	...
	p.writePunctuation(":")
	p.writeSpace()
	p.emitTypeNodeOutsideExtendsWithMultiReturnParens(node)
}
...
// emitSignatureSkippingSelf prints the signature of a colon method declaration.
// Its first parameter is the `self` the parser synthesized from the colon, which
// is implicit in the source and so must stay implicit in the output.
func (p *Printer) emitSignatureSkippingSelf(node *ast.Node) {
	n := node.FunctionLikeData()
	p.emitTypeParameters(node, n.TypeParameters)
...
// emitLuaBlockStatements prints the indented statement list of a Lua block;
// callers write the surrounding keywords and terminator.
func (p *Printer) emitLuaBlockStatements(body *ast.Block) {
	p.generateNames(body.AsNode())
	p.increaseIndent()
	detachedState := p.emitDetachedCommentsBeforeStatementList(body.AsNode(), body.Statements.Loc)
	statementOffset := p.emitPrologueDirectives(body.Statements)
...
func (p *Printer) emitLuaFunctionBody(body *ast.Block) {
	p.emitContext.AddEmitFlags(body.AsNode(), EFNoSourceMap)
...
func (p *Printer) emitLuaFunctionBodyNode(node *ast.BlockNode) {
	debug.Assert(node != nil, "Lua function bodies must have a block")
...
func (p *Printer) emitTypeNodeOutsideExtendsWithMultiReturnParens(node *ast.TypeNode) {
	// A multiple return type is always written parenthesized here.
	if node.Kind == ast.KindMultiReturnType {
		p.writePunctuation("(")
...
func (p *Printer) emitReturnType(node *ast.TypeNode, allowReturnPack bool) {
	if node == nil {
		return
	}
	p.writePunctuation("=>")
	p.writeSpace()
	if allowReturnPack && node.Kind == ast.KindMultiReturnType {
		p.writePunctuation("(")
...
func (p *Printer) emitMultiReturnType(node *ast.MultiReturnTypeNode) {
	state := p.enterNode(node.AsNode())
	// A bare comma-separated type list — Lua multiple return values, no brackets.
	p.emitList((*Printer).emitTupleElementType, node.AsNode(), node.Elements, LFSingleLineTupleTypeElements)
	p.exitNode(node.AsNode(), state)
}
...
func (p *Printer) emitTypeNode(node *ast.TypeNode, precedence ast.TypePrecedence) {
	...
	case ast.KindMultiReturnType:
		p.emitMultiReturnType(node.AsMultiReturnTypeNode())
...
func (p *Printer) emitFunctionExpression(node *ast.FunctionExpression) {
	...
	// The parser only produces anonymous function expressions (Lua's
	// constructor form); a name occurs only on factory-synthesized nodes.
	if name := node.Name(); name != nil {
		p.writeSpace()
		p.emitIdentifierNameNode(name)
	}
	indented := p.shouldEmitIndented(node.AsNode())
	p.increaseIndentIf(indented)
	p.pushNameGenerationScope(node.AsNode())
...
// emitArrowFunction prints a tlua lambda as the Lua function constructor it
// compiles to: `(x) => body` becomes `function(x) return body end`, and a
// braced body inlines its statements before `end`. A concise body is typed as
// a single value (packs in arrow return positions are unsupported), but a
// direct call in Lua tail position would propagate its whole pack --
// parenthesize it so the runtime truncates to the one value the type names.
func (p *Printer) emitArrowFunction(node *ast.ArrowFunction) {
	...
	p.emitTypeAnnotation(node.Type)
	switch {
	case ast.IsBlock(node.Body):
		p.emitLuaFunctionBody(node.Body.AsBlock())
	case ast.IsExpression(node.Body):
		p.writeSpace()
		p.writeKeyword("return")
		p.writeSpace()
		if ast.SkipPartiallyEmittedExpressions(node.Body).Kind == ast.KindCallExpression {
			p.writePunctuation("(")
			p.emitExpression(node.Body, ast.OperatorPrecedenceLowest)
			p.writePunctuation(")")
		} else {
			p.emitExpressionNoASI(node.Body, ast.OperatorPrecedenceLowest)
		}
		p.writeSpace()
		p.writeKeyword("end")
	default:
		panic(fmt.Sprintf("unexpected ConciseBody: %v", node.Body.Kind))
	}
	p.popNameGenerationScope(node.AsNode())
	p.decreaseIndentIf(indented)
	p.exitNode(node.AsNode(), state)
...
func (p *Printer) getBinaryExpressionPrecedence(node *ast.BinaryExpression) (leftPrec ast.OperatorPrecedence, rightPrec ast.OperatorPrecedence) {
	...
	case ast.OperatorPrecedenceLogicalAND:
		rightPrec = ast.OperatorPrecedenceEquality
	case ast.OperatorPrecedenceEquality:
		rightPrec = ast.OperatorPrecedenceRelational
	case ast.OperatorPrecedenceRelational:
		rightPrec = ast.OperatorPrecedenceConcatenation
	case ast.OperatorPrecedenceConcatenation:
		// Lua `..` is right-associative: bump the left operand so a same-precedence
		// left is parenthesized while a same-precedence right is not.
		leftPrec = ast.OperatorPrecedenceAdditive
	...
	case ast.OperatorPrecedenceMultiplicative:
		...
		// The next level up is Unary (exponentiation now binds tighter still), so a
		// unary right operand needs no parens: `a * -b` stays `a * -b`, not `a * (-b)`.
		rightPrec = ast.OperatorPrecedenceUnary
	case ast.OperatorPrecedenceExponentiation:
		// Lua exponentiation is right-associative and admits a unary right operand.
...
func (p *Printer) emitTemplateExpression(node *ast.TemplateExpression) {
	...
	if head := node.Head.Text(); head != "" {
		sep()
		p.writer.WriteStringLiteral(luaStringLiteral(head, QuoteCharDoubleQuote))
	}
	for _, spanNode := range node.TemplateSpans.Nodes {
		...
		p.write("tostring")
		p.writePunctuation("(")
		p.emitExpression(span.Expression, ast.OperatorPrecedenceDisallowComma)
		p.writePunctuation(")")
		if lit := span.Literal.Text(); lit != "" {
			sep()
			p.writer.WriteStringLiteral(luaStringLiteral(lit, QuoteCharDoubleQuote))
		}
	}
	if !wroteAny {
		// Unreachable for a parsed TemplateExpression (the grammar guarantees a head plus
...
func (p *Printer) emitExpressionListElement(node *ast.Expression) {
	// DisallowComma keeps a nested parenthesized comma expression parenthesized.
	p.emitExpression(node, ast.OperatorPrecedenceDisallowComma)
}
...
func (p *Printer) emitVarargExpression(node *ast.Node) {
...
func (p *Printer) emitVariableStatement(node *ast.VariableStatement) {
	state := p.enterNode(node.AsNode())
	if ast.IsLuaLocal(node.DeclarationList) {
		debug.Assert(node.Modifiers() == nil, "Lua local declarations cannot have modifiers")
	}
	p.emitModifierList(node.AsNode(), node.Modifiers())
	if isAmbientGlobalShape(node) {
		// The canonical output for an ambient global is the Luau-style
		// `declare name: Type;` — never a declaration keyword. Each declarator
		// gets its own `declare`-prefixed statement so the output re-parses
		// (there is no multi-declarator ambient spelling).
		declarations := node.DeclarationList.AsVariableDeclarationList().Declarations.Nodes
		for i, declaration := range declarations {
			...
			p.emitVariableDeclaration(declaration.AsVariableDeclaration())
		}
	} else {
		// Reachable only for non-ambient lists (the parser produces LuaLocal
		// ones; transforms rebuild ambient lists as Const, so a declare
		// modifier never reaches the `local` keyword below).
...
// isAmbientGlobalShape reports whether a variable statement prints in the
// Luau-style `declare name: Type;` form: a declare-modified list whose every
// declarator is an uninitialized, typed identifier. (Initialized or untyped
// declarators only arise from parser error recovery; those keep the plain
// list emit and their errors.)
func isAmbientGlobalShape(node *ast.VariableStatement) bool {
	if !ast.HasSyntacticModifier(node.AsNode(), ast.ModifierFlagsAmbient) {
		return false
	}
	list := node.DeclarationList.AsVariableDeclarationList()
	if len(list.Declarations.Nodes) == 0 {
		return false
	}
	for _, declaration := range list.Declarations.Nodes {
		decl := declaration.AsVariableDeclaration()
		if decl.Initializer != nil || decl.Type == nil || !ast.IsIdentifier(decl.Name()) {
			return false
		}
	}
	return true
...
func (p *Printer) emitExpressionStatement(node *ast.ExpressionStatement) {
	...
	if p.currentSourceFile != nil && p.currentSourceFile.ScriptKind == core.ScriptKindJSON {
		...
		// An arrow prints as `function ... end` too, so at statement start it
		// would reparse as a function declaration just like a function
		// expression would.
		case ast.KindFunctionExpression, ast.KindArrowFunction, ast.KindObjectLiteralExpression:
			p.emitExpression(node.Expression, ast.OperatorPrecedenceParentheses)
		default:
			p.emitExpression(node.Expression, ast.OperatorPrecedenceComma)
		}
...
// emitLuaIfChain prints a Lua `if ... then ... [elseif ...] [else ...] end`
// statement; nested Lua ifs in the else slot print as `elseif` arms sharing
// the single closing `end`, mirroring the parse shape.
func (p *Printer) emitLuaIfChain(node *ast.IfStatement, chained bool) {
	state := p.enterNode(node.AsNode())
	if chained {
		p.writeKeyword("elseif")
	} else {
		p.emitToken(ast.KindIfKeyword, node.Pos(), WriteKindKeyword, node.AsNode())
	}
	p.writeSpace()
	p.emitExpression(node.Expression, ast.OperatorPrecedenceLowest)
	p.writeSpace()
	p.writeKeyword("then")
	p.emitLuaBlockStatements(node.ThenStatement.AsBlock())
	switch {
...
// semicolon guards Lua's call-gluing ambiguity, matching do-while style).
func (p *Printer) emitRepeatStatement(node *ast.RepeatStatement) {
	state := p.enterNode(node.AsNode())
	p.generateAllNames(node.Statements)
	p.emitToken(ast.KindRepeatKeyword, node.Pos(), WriteKindKeyword, node.AsNode())
	p.increaseIndent()
	p.emitList((*Printer).emitStatement, node.AsNode(), node.Statements, LFMultiLineFunctionBodyStatements)
	p.decreaseIndent()
	p.writeLine()
	p.writeKeyword("until")
	p.writeSpace()
	p.emitExpression(node.Expression, ast.OperatorPrecedenceLowest)
	p.writeTrailingSemicolon()
	p.exitNode(node.AsNode(), state)
...
// emitNumericForStatement prints a Lua `for i[: T] = from, to[, step] do ...
// end`. The single control declaration prints directly — never through
// emitVariableDeclarationList, which writes `local` for Lua lists.
func (p *Printer) emitNumericForStatement(node *ast.NumericForStatement) {
	state := p.enterNode(node.AsNode())
	p.emitToken(ast.KindForKeyword, node.Pos(), WriteKindKeyword, node.AsNode())
	p.writeSpace()
	p.emitVariableDeclaration(node.Initializer.AsVariableDeclarationList().Declarations.Nodes[0].AsVariableDeclaration())
	p.writeSpace()
	p.writePunctuation("=")
	p.writeSpace()
	p.emitExpression(node.From, ast.OperatorPrecedenceDisallowComma)
	p.writePunctuation(",")
	p.writeSpace()
	p.emitExpression(node.To, ast.OperatorPrecedenceDisallowComma)
	if node.Step != nil {
		p.writePunctuation(",")
		p.writeSpace()
		p.emitExpression(node.Step, ast.OperatorPrecedenceDisallowComma)
	}
	p.writeSpace()
	p.writeKeyword("do")
	p.emitLuaBlockStatements(node.Statement.AsBlock())
...
// emitLuaGenericForStatement prints `for k, v in explist do ... end`. The
// name list prints its declarations directly (no `local`).
func (p *Printer) emitLuaGenericForStatement(node *ast.ForOfStatement) {
	state := p.enterNode(node.AsNode())
	p.emitToken(ast.KindForKeyword, node.Pos(), WriteKindKeyword, node.AsNode())
	p.writeSpace()
	initializer := node.Initializer.AsVariableDeclarationList()
	p.emitList((*Printer).emitVariableDeclarationNode, node.Initializer, initializer.Declarations, LFVariableDeclarationList)
	p.writeSpace()
...
func (p *Printer) emitLabelStatement(node *ast.LabelStatement) {
	state := p.enterNode(node.AsNode())
	p.emitToken(ast.KindColonColonToken, node.Pos(), WriteKindPunctuation, node.AsNode())
	p.emitLabelIdentifier(node.Label.AsIdentifier())
	p.emitToken(ast.KindColonColonToken, node.Label.End(), WriteKindPunctuation, node.AsNode())
	p.exitNode(node.AsNode(), state)
}

// Prints Lua `goto name;`.
func (p *Printer) emitGotoStatement(node *ast.GotoStatement) {
	state := p.enterNode(node.AsNode())
...
func (p *Printer) emitVariableDeclarationList(node *ast.VariableDeclarationList) {
	state := p.enterNode(node.AsNode())
	// `local` is the only declaration keyword. Const lists occur too (the
	// ambient-global desugaring and factory-synthesized nodes) but the flags
	// are internal scope markers with no spelling of their own.
	if ast.IsLuaLocal(node.AsNode()) {
		debug.Assert(len(node.Declarations.Nodes) >= 1, "Lua local declaration lists must have at least one declaration")
	}
	p.writeKeyword("local")
	p.writeSpace()
...
func (p *Printer) emitFunctionDeclaration(node *ast.FunctionDeclaration) {
	...
	p.writeSpace()
	if node.Target != nil {
		// `function a.b.f(...)`: the target chain precedes the declaration's name.
		// A colon instead of a dot marks the method form, whose `self` parameter
		// the parser synthesized and which must therefore not be printed back.
		p.emitExpression(node.Target, ast.OperatorPrecedenceMember)
		if node.ColonToken != nil {
			p.writePunctuation(":")
		} else {
			p.writePunctuation(".")
		}
	...
	p.pushNameGenerationScope(node.AsNode())
	if node.ColonToken != nil {
		p.emitSignatureSkippingSelf(node.AsNode())
	} else {
		p.emitSignature(node.AsNode())
	}
	p.emitLuaFunctionBodyOrSemicolon(node.Body)
...
func (p *Printer) emitInterfaceDeclaration(node *ast.InterfaceDeclaration) {
	state := p.enterNode(node.AsNode())
	// A `local interface` is module-private; `local` and modifiers are mutually
	// exclusive, so emit one or the other (mirrors emitFunctionDeclaration).
	if ast.IsLuaLocal(node.AsNode()) {
		p.writeKeyword("local")
		p.writeSpace()
	} else {
...
func (p *Printer) emitTypeAliasDeclaration(node *ast.TypeAliasDeclaration) {
	state := p.enterNode(node.AsNode())
	// A `local type` is module-private; `local` and modifiers are mutually exclusive.
	if ast.IsLuaLocal(node.AsNode()) {
		p.writeKeyword("local")
		p.writeSpace()
	} else {
...
func (p *Printer) emitStatement(node *ast.Statement) {
	...
	case ast.KindForOfStatement:
		p.emitLuaGenericForStatement(node.AsForOfStatement())
	case ast.KindNumericForStatement:
		p.emitNumericForStatement(node.AsNumericForStatement())
	case ast.KindRepeatStatement:
		p.emitRepeatStatement(node.AsRepeatStatement())
...
// emitTableEntry prints a Lua positional table entry: the bare expression.
func (p *Printer) emitTableEntry(node *ast.TableEntry) {
	state := p.enterNode(node.AsNode())
	p.emitExpression(node.Expression, ast.OperatorPrecedenceDisallowComma)
	p.exitNode(node.AsNode(), state)
}
...
func formatSynthesizedComment(comment SynthesizedComment) string {
	return formatLuaComment(comment.Kind, comment.Text)
}
