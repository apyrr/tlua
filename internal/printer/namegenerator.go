...
func (g *NameGenerator) generateNameForNode(node *ast.Node, privateName bool, flags GeneratedIdentifierFlags, prefix string, suffix string) string {
	...
	case ast.KindModuleDeclaration:
		if privateName || len(prefix) > 0 || len(suffix) > 0 {
			panic("Generated name for a module cannot be private and may have neither a prefix nor suffix")
		}
		return g.generateNameForModule(node)
...
func (g *NameGenerator) generateNameForModule(node *ast.Node) string {
	name := g.GetTextOfNode(node.Name())
	// Use the module name itself if it is unique, otherwise make a unique variation.
