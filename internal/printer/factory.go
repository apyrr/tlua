...
func (f *NodeFactory) NewStrictEqualityExpression(left *ast.Expression, right *ast.Expression) *ast.Expression {
	return f.NewBinaryExpression(nil /*modifiers*/, left, nil /*typeNode*/, f.NewToken(ast.KindEqualsEqualsToken), right)
}

func (f *NodeFactory) NewStrictInequalityExpression(left *ast.Expression, right *ast.Expression) *ast.Expression {
	return f.NewBinaryExpression(nil /*modifiers*/, left, nil /*typeNode*/, f.NewToken(ast.KindTildeEqualsToken), right)
}
...
func (f *NodeFactory) CreateExpressionFromEntityName(node *ast.Node) *ast.Expression {
	if ast.IsQualifiedName(node) {
		...
		propAccess := f.NewPropertyAccessExpression(left, nil, nil /*colonToken*/, right, ast.NodeFlagsNone)
...
func (f *NodeFactory) NewMethodCall(object *ast.Node, methodName *ast.Node, argumentsList []*ast.Node) *ast.Node {
	...
	if ast.IsCallExpression(object) && (object.Flags&ast.NodeFlagsOptionalChain != 0) {
		return f.NewCallExpression(
			f.NewPropertyAccessExpression(object, nil, nil /*colonToken*/, methodName, ast.NodeFlagsNone),
			nil,
			nil,
...
func (f *NodeFactory) GetNamespaceMemberName(ns *ast.IdentifierNode, name *ast.IdentifierNode, opts NameOptions) *ast.IdentifierNode {
	...
	qualifiedName := f.NewPropertyAccessExpression(ns, nil /*questionDotToken*/, nil /*colonToken*/, name, ast.NodeFlagsNone)
...
func (f *NodeFactory) NewAssignHelper(attributesSegments []*ast.Expression, scriptTarget core.ScriptTarget) *ast.Expression {
	return f.NewCallExpression(f.NewPropertyAccessExpression(f.NewIdentifier("Object"), nil, nil /*colonToken*/, f.NewIdentifier("assign"), ast.NodeFlagsNone), nil, nil, f.NewNodeList(attributesSegments), ast.NodeFlagsNone)
}
...
func (f *NodeFactory) NewESDecorateClassElementAccessGetMethod(
	...
	if nameComputed {
		...
		accessor = f.NewPropertyAccessExpression(f.NewIdentifier("obj"), nil, nil /*colonToken*/, nameExpr, ast.NodeFlagsNone)
	}
