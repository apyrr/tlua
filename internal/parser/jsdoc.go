...
func (p *Parser) parsePropertyAccessEntityNameExpression() *ast.Node {
	...
	for p.parseOptional(ast.KindDotToken) {
		...
		node = p.finishNode(p.factory.NewPropertyAccessExpression(node, nil, nil /*colonToken*/, name, ast.NodeFlagsNone), pos)
	}
	return node
}
...
func (p *Parser) parseTemplateTagTypeParameter() *ast.Node {
	...
	return p.finishNode(p.factory.NewTypeParameterDeclaration(modifiers, nil /*dotDotDotToken*/, name, nil /*constraint*/, nil /*expression*/, defaultType), typeParameterPos)
}
