...
function parseEnvBoolean(name, defaultValue = false) {
    name = "TLUA_HEREBY_" + name.toUpperCase();
...
function getReleaseBuildFlags(versionOverride) {
    ...
    if (versionOverride) {
        ldflags += ` -X github.com/apyrr/tlua/internal/core.version=${versionOverride}`;
    }
 ...
 */
function buildTlua(opts) {
    opts ||= {};
    const out = opts.out ?? "./built/local/";
    const env = { ...goBuildEnv, ...opts.env };
    return $({ cancelSignal: opts.abortSignal, env })`go build ${goBuildFlags} ${opts.extraFlags ?? []} ${goBuildTags("noembed")} -o ${out} ./cmd/tlua`;
}

export const tluaBuild = task({
    name: "tlua:build",
    description: "Builds the tlua binary.",
    run: async () => {
        await buildTlua({ extraFlags: options.release ? getReleaseBuildFlags() : [] });
    },
});

export const local = task({
    name: "local",
    dependencies: [lib, tluaBuild],
});
...
export const buildWatch = task({
    name: "build:watch",
    description: "Builds the tlua binary and watches for changes.",
    run: async () => {
        await watchDebounced("build:watch", async (paths, abortSignal) => {
            ...
            if (goChanged) {
                console.log("Building tlua...");
                await buildTlua({ abortSignal });
            }
        }, {
            paths: ["cmd", "internal"],
...
/** @type {EnumDef[]} */
const enumDefs = [
    { name: "SymbolFlags", goPrefix: "SymbolFlags", goFile: "internal/ast/symbolflags.go", outDir: "_packages/tlua/src/enums" },
    { name: "TypeFlags", goPrefix: "TypeFlags", goFile: "internal/checker/types.go", outDir: "_packages/tlua/src/enums" },
    { name: "ObjectFlags", goPrefix: "ObjectFlags", goFile: "internal/checker/types.go", outDir: "_packages/tlua/src/enums" },
    { name: "SignatureFlags", goPrefix: "SignatureFlags", goFile: "internal/checker/types.go", outDir: "_packages/tlua/src/enums" },
    { name: "SignatureKind", goPrefix: "SignatureKind", goFile: "internal/checker/types.go", outDir: "_packages/tlua/src/enums" },
    { name: "ElementFlags", goPrefix: "ElementFlags", goFile: "internal/checker/types.go", outDir: "_packages/tlua/src/enums" },
    { name: "TypePredicateKind", goPrefix: "TypePredicateKind", goFile: "internal/checker/types.go", outDir: "_packages/tlua/src/enums" },
    { name: "DiagnosticCategory", goPrefix: "Category", goFile: "internal/diagnostics/diagnostics.go", outDir: "_packages/tlua/src/enums" },
    { name: "SyntaxKind", goPrefix: "Kind", goFile: "internal/ast/kind_generated.go", outDir: "_packages/tlua/src/enums" },
    { name: "NodeFlags", goPrefix: "NodeFlags", goFile: "internal/ast/nodeflags.go", outDir: "_packages/tlua/src/enums" },
    { name: "OuterExpressionKinds", goPrefix: "OEK", goFile: "internal/ast/utilities.go", outDir: "_packages/tlua/src/enums" },
    { name: "ModifierFlags", goPrefix: "ModifierFlags", goFile: "internal/ast/modifierflags.go", outDir: "_packages/tlua/src/enums" },
    { name: "ModuleKind", goPrefix: "ModuleKind", goFile: "internal/core/compileroptions.go", outDir: "_packages/tlua/src/enums" },
    { name: "ModuleResolutionKind", goPrefix: "ModuleResolutionKind", goFile: "internal/core/compileroptions.go", outDir: "_packages/tlua/src/enums" },
    { name: "NewLineKind", goPrefix: "NewLineKind", goFile: "internal/core/compileroptions.go", outDir: "_packages/tlua/src/enums" },
    { name: "JsxEmit", goPrefix: "JsxEmit", goFile: "internal/core/compileroptions.go", outDir: "_packages/tlua/src/enums" },
    { name: "TokenFlags", goPrefix: "TokenFlags", goFile: "internal/ast/tokenflags.go", outDir: "_packages/tlua/src/enums" },
    { name: "NodeBuilderFlags", goPrefix: "Flags", goFile: "internal/nodebuilder/types.go", outDir: "_packages/tlua/src/enums" },
    { name: "CompletionItemKind", goPrefix: "CompletionItemKind", goFile: "internal/lsp/lsproto/lsp_generated.go", outDir: "_packages/tlua/src/enums" },
    ...
    { name: "InternalSymbolName", goPrefix: "InternalSymbolName", goFile: "internal/ast/symbol.go", outDir: "_packages/tlua/src/enums", stringEnum: true, valueReplacements: { InternalSymbolNamePrefix: "__" } },
];
...
const vendorJsonrpcDir = "_packages/tlua/vendor/vscode-jsonrpc";
...
async function runTests() {
    ...
    if (baselineTrackingEnabled) {
        const tmpDir = tmp.dirSync({ prefix: "tlua-baseline-tracking-", unsafeCleanup: true });
    ...
    try {
        const testEnv = {
            ...goTestEnv,
            ...(trackingDir ? { TLUA_BASELINE_TRACKING_DIR: trackingDir } : {}),
        };
...
export const testAPI = task({
    name: "test:api",
    description: "Runs the tlua JS API tests.",
    dependencies: [lib, tluaBuild, buildAPITests],
    run: runTestAPI,
});
 ...
 * @typedef {{ vscodeTarget: string; sourceDir: string; extensionDir: string; vsixPath: string }} VsixExtension
 * @typedef {{ GOOS: string; GOARCH: string }} GoDistTarget
 * @typedef {{ os: OS; arch: Arch; vsix?: boolean; alpine?: boolean }} Platform
 */
void 0;

/** @type {VsixExtensionPackage[]} */
const vsixExtensionPackages = [
    { name: "tlua", sourceDir: extensionDir },
];
 ...
 */
const platforms = [
    { os: "win32", arch: "x64", vsix: true },
    { os: "win32", arch: "arm64", vsix: true },
...
async function runCheckPlatforms() {
    ...
    if (errors.length) {
        throw new Error(`tlua platform list is out of sync with 'go tool dist list':\n${errors.map(e => `  - ${e}`).join("\n")}`);
    }
}

/**
 * Recursively strips `@tlua/source` export conditions from a package.json object.
...
export const buildTluaPackages = task({
    name: "tlua:build-packages",
    hiddenFromTaskList: true,
    run: runBuildTluaPackages,
});

async function runBuildTluaPackages() {
    await rimraf(builtNpm);

    const platforms = getPlatforms();

    const inputDir = "./_packages/tlua";
    ...
    const platformBuilders = platforms.map(({ npmDir, npmPackageName, nodeOs, nodeArch, goos, goarch }) => async () => {
        ...
        const readme = [
            `# \`${npmPackageName}\``,
            "",
            `This package provides ${nodeOs}-${nodeArch} support for [${mainTluaPackage.npmPackageName}](https://www.npmjs.com/package/${mainTluaPackage.npmPackageName}).`,
        ];
        ...
        const exeName = tluaExeName(nodeOs);
        await buildTlua({
 ...
 * @param {string} nodeOs
 */
function tluaExeName(nodeOs) {
    // The npm package's JS shim and the Go binary intentionally share the tlua name.
    return nodeOs === "win32" ? "tlua.exe" : "tlua";
}

export const packTluaPackages = task({
    name: "tlua:pack-packages",
    hiddenFromTaskList: true,
    dependencies: options.forRelease ? undefined : [buildTluaPackages],
    run: runPackTluaPackages,
});

async function runPackTluaPackages() {
    const platforms = getPlatforms();
    await Promise.all([mainTluaPackage, ...platforms].map(async ({ npmDir, npmTarball }) => {
    ...
    const publishManifest = {
        stages: [
            ...
            [
                {
                    filename: path.basename(mainTluaPackage.npmTarball),
                },
            ],
        ],
    };
...
async function runPackVsixExtensions() {
    ...
    await Promise.all(extensions.map(async ({ npmDir, vscodeTarget, sourceDir, extensionDir: thisExtensionDir, vsixPath }) => {
        ...
        packageJson.bundledTluaVersion = getVersion();
...
export const tlua = task({
    name: "tlua",
    hiddenFromTaskList: true,
    dependencies: options.forRelease ? undefined : [packTluaPackages, packVsixExtensions],
