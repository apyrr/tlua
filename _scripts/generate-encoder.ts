 ...
 *   - _packages/tlua/src/api/node/protocol.generated.ts
 */
...
function generateGoEncoder(): string {
    ...
    w.write('"fmt"');
    w.write("");
    w.write('"github.com/apyrr/tlua/internal/ast"');
    w.pop();
    w.write(")");
    w.write("");
...
function emitRemoteNodeClassOpen(w: CodeWriter) {
    ...
    w.write(`        //`);
    w.write(`        // This is a MethodSignature with mask 0b00111011. The possible properties are`);
    w.write(`        // ["modifiers", "name", "postfixToken", "typeParameters", "parameters", "type"]`);
    w.write(`        // (it has modifiers, name, typeParameters, parameters, and type).`);
    w.write(`        //`);
    w.write(`        // | Bit   | 5    | 4          | 3              | 2            | 1    | 0         |`);
    w.write(`        // | ----- | ---- | ---------- | -------------- | ------------ | ---- | --------- |`);
    w.write(`        // | Value | 1    | 1          | 1              | 0            | 1    | 1         |`);
    w.write(`        // | Name  | type | parameters | typeParameters | postfixToken | name | modifiers |`);
    w.write(`        //`);
    w.write(`        // We are trying to get the index of "parameters" (bit = 4).`);
    ...
    w.write(`        // | Bit   | 5    | 4          | 3              | 2            | 1    | 0         |`);
    w.write(`        // | ----- | ---- | ---------- | -------------- | ------------ | ---- | --------- |`);
    w.write(`        // | Value | 0    | 0          | 0              | 1            | 0    | 0         |`);
    w.write(`        //`);
    w.write(`        // Counting the 1s gives us the number of *missing properties* before the \`order\`th property. If every property`);
    w.write(`        // were present, we would have \`parameters = children[4]\`, but since \`postfixToken\` is missing,`);
    w.write(`        // we have \`parameters = children[4 - 1]\`.`);
...
export default function main() {
    ...
    writeAndFormat(
        path.join(ROOT, "_packages/tlua/src/api/node/protocol.generated.ts"),
        tsProtocol + "\n",
        "dprint fmt",
    );
    ...
    writeAndFormat(
        path.join(ROOT, "_packages/tlua/src/api/node/encoder.generated.ts"),
        tsEncoder + "\n",
        "dprint fmt",
    );
    ...
    writeAndFormat(
        path.join(ROOT, "_packages/tlua/src/api/node/node.generated.ts"),
        tsNode + "\n",
        "dprint fmt",
    );
