...
/**
 * When-clause context set when the current file is managed by the tlua server.
 */
export class ManagedFileContextManager implements vscode.Disposable {
    private static readonly contextName = "tlua.isManagedFile";
    ...
    private isManagedFile(editor: vscode.TextEditor): boolean {
        ...
        return isSupportedLanguageMode(editor.document)
            || isTsConfigFileName(editor.document.fileName);
    }
